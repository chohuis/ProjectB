...
| 항목 | 블로커 | 해소 조건 | 그룹 |
|---|---|---|---|
| 코치·구단주 정식 스태프 | 훈련(`sim::training`)·계약(`sim::market`)·평가(`sim::eval`) 3개 모듈에 동시 배선 필요 — 감독(효과 1곳)과 달리 스코프가 훨씬 큼 | 별도 세션 하나를 통째로 이 3개 배선에만 씀 | 대형 신규 시스템 |
