...
> 상태: **I0·I1·I2·I3·I4 완료, I5 진행중(1~5차분 완료)** — 2026-07-15
...
**다음 세션이 할 일**(5차분 완료로 갱신, 아래 §6-9 참고): I5 잔여 중 급성형(경기 중 우발) 부상 — `sim::match_sim`을 건드려야 하는 별도 스코프라 5차분에서 이월. 또는 `sim/eval`(평가 시스템)·`sim/market`(시장 계약)·`sim/npc`(세대교체). 아니면 I6(주인공 플로우)로 넘어가 `sign_core_state`·`find_protagonist_game_today`가 실제 주인공으로 동작하는 걸 볼 수도 있음 — 순서는 다음 세션 시작 시 확인.
