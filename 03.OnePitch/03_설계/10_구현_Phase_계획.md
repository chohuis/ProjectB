...
> 상태: **I0~I6 완료(I5는 8차분까지, `eval`·`market`은 I6로 재배치, I6는 8차분까지 — `sim/market` 전체 완료), I7 착수(3차분 완료 — 엔진 api 레이어+뉴게임/진행/매치 최소 화면+내 선수 허브+리그 허브)** — 2026-07-16
...
| **I7** | Flutter UI | `04_UI기획/` 00~08 화면 실제 구현(4허브+진행버튼+매치 CustomPainter) | 최소 플레이 가능한 루프가 실제 화면에서 끝까지 동작(뉴게임→진행→경기→시즌종료) | `04_UI기획/` 전체 | 🟨 **착수**(3차분 완료, 2026-07-16) — 1차분(엔진 `api::game` 레이어 + Riverpod/go_router + 뉴게임·진행/매치 최소 화면으로 "뉴게임→진행→경기→시즌 경계" 브리지 동작 확인)+2차분(4허브 첫 번째 — 내 선수 허브 상태/훈련 탭 실동작, 재정 탭은 엔진 미구현이라 placeholder)+3차분(4허브 두 번째 — 리그 허브 로스터/일정/순위/라이벌 4탭, 172팀 전 팀 스카우팅 실동작). 남은 4허브 2개(기록·메시지함)·전용화면(협상/트레이드/진로/드래프트/치료/은퇴)·매치 CustomPainter 비주얼·세이브 슬롯 영속성·반응형 레이아웃은 계속 이월. 상세는 아래 §6-21~§6-23 |
...
**다음 세션이 할 일**(I7 3차분 완료로 갱신, 아래 §6-23 참고): 사용자가 확정한 추천 순서대로 — ①남은 4허브 2개(기록·메시지함) 중 다음 것, ②매치 CustomPainter 비주얼(다이아몬드·존그리드, 05_매치.md), ③`injuryTreatment` 전용 화면(전용화면 5종 중 지금 유일하게 실제 발동 가능), ④진로 갈림길 엔진 작업(고교→대학/독립/프로, 01_커리어_구조.md §5 — `contractNego`/`tradeDecision`/`draft`/`careerChoice`가 실전에서 의미를 가지려면 선행), ⑤그 4종 전용 화면, ⑥세이브 슬롯 영속성(`createSlot`/`loadSlot`), ⑦반응형 레이아웃, ⑧은퇴 화면. I6 계속 이월분(감독 개입·콜드게임 인터랙티브 경로·개인 통산 스탯·월/시즌 단위 평가)도 여전히 남아있음 — 이 순서와 독립적이라 아무 지점에 끼워 넣어도 무방. `build_runner`/Dart 3.10.3 호환 이슈(§6-22)와 `testWidgets`+frb 비동기 조합 시 `runAsync` 안에 `pumpWidget`까지 넣어야 하는 패턴(§6-23)은 다음 위젯 테스트 작성 시 재확인.
