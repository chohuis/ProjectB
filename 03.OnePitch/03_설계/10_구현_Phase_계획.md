...
| 타순 최적화(성적 기반 라인업 재배치) | 타자 season_stats는 §6-76에서 완전히 저장되지만, 로테이션처럼 이미 있는 서열화 알고리즘이 타순 쪽엔 없음(현재 타순은 로스터 고정 순서) | 타순 최적화 알고리즘 신설 | 엔진 확장 |
