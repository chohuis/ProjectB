...
- **I2 완료 시점 데이터 갭 총정리**(전부 기존 기획 문서에 이미 있던 미확정 항목, 이번에 새로 발견하지도 지어내지도 않음):
  1. 독립리그 9개 시민구단 budget — 전력★ 미정의라 NULL ([07_데이터관리](07_데이터관리.md) §8 참고).
  2. 대학 경기·인천(14)·대구·경북(5) 소속 19팀 stadium_id — A/B, D/E조 배정 미정이라 NULL.
  3. 07_구장_파크팩터.md §1 요약("독립 5구장")과 §6 실제 표(4구장)가 어긋남 — 실제 표를 따라 독립 4구장만 시드(총 27구장, §1 요약의 "28"과 불일치). 사소한 문서 오기로 보이나 확인 필요.
