...
> 상태: **I0~I5 완료(I5는 8차분까지, `eval`·`market`은 I6로 재배치), I6 진행중(1차분 완료)** — 2026-07-15
...
| **I6** | 주인공 플로우 | 캐릭터 생성([07_주인공_생성](../02_기획/07_주인공_생성.md)), 주인공 등판 매치 세션(`startMatch`/`pitch`) | 캐릭터 생성 후 첫 경기를 실제로 뛸 수 있음(반자동 모드 최소) | [04_UI기획/06_캐릭터생성](../04_UI기획/06_캐릭터생성.md) · [07_매치_엔진](../02_기획/육성코어/07_매치_엔진.md) | 🔶 진행중 — 1차분(`create_protagonist`, 캐릭터 생성 데이터 계층) 완료(2026-07-15). 주인공 등판 매치 세션(`startMatch`/`pitch`, 축2 진행모드)은 이월. 상세는 아래 §6-13 |
...
**다음 세션이 할 일**(I6 1차분 완료로 갱신, 아래 §6-13 참고): I6 2차분 — 주인공 등판 매치 세션(`startMatch`/`pitch`, 07_매치_엔진.md 축2 진행모드). 그 다음 후보: `sim/eval`(S~D 평가)·`sim/market`(계약/FA/트레이드)·`sim/injury`의 치료 선택(`apply_injury`/`treat`)·`sim/training`(06_훈련_시스템 슬롯 배분) — 전부 "주인공이 선택하는" UI 중심 시스템이라 매치 세션과 자연스럽게 이어짐.
