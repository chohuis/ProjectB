...
**I0~I6 완료(I6는 10차분까지, §6-29), I7은 13차분까지 완료(팀명 버그·에러UI·로딩·전환애니메이션·디자인 시스템 전부 §6-34·§6-37), I8은 1차분 착수(밸런스 하네스, §6-36).** 사용자 실플레이 피드백 4가지(①팀명 ②디자인 ③성능 ④콘텐츠 공백) 중 ①②는 완료, ③은 근본 원인(뉴게임 생성 시 일정 INSERT가 트랜잭션 없이 개별 autocommit — §6-38)을 찾아 수정 완료, ④가 I8로 진행 중.

**다음 세션이 이어서 할 것(I8)**:
1. **밸런스 하네스 대규모 실행** — §6-36의 N=1 예비 관측(부상으로 2시즌 만에 조기 은퇴)이 실제 경향인지 확인하려면 N을 수십~수백으로 늘려야 함. 시즌당 ~40~50초라 오래 걸림 — 백그라운드로 돌리고 다른 작업 병행 권장. 결과 보고 `sim::retirement::INJURY_RETIREMENT_WEIGHT_THRESHOLD`(§6-29)나 부상 발생확률(D그룹) 조정 검토.
2. **콘텐츠 스키마+import 파이프라인** — `seed_content.rs`는 아직 events/characters/achievements/narrative_templates/messages/strings 테이블을 다루지 않음(I2 시드 테이블만 지원). 이 6개 테이블 지원 추가.
3. **실제 콘텐츠 저작** — 이벤트·업적·서술템플릿 등, AI(=이 세션) 대량생성으로 채움. 의존순서: 01_캐릭터(생성규칙만, 저작 거의 없음)→02_이벤트(척추)→03_메시지+04_업적(병행)→05_뉴스→06_서술템플릿.

이월 중인 것들: I6 잔여(감독 개입·콜드게임 인터랙티브 경로·월/시즌 단위 평가·갈림길 B 전용 UX 다듬기·대학 4년 후 재드래프트, §6-20·§6-26), I7 자체 이월분("Continue 상시노출" 세부·허브 화면 중첩 Scaffold 시각 다듬기, §6-33), I9(실기기 검증 — Android+Steam/PC 우선, Mac/Linux는 이월).
