...
> 상태: **I0~I5 완료(I5는 8차분까지, `eval`·`market`은 I6로 재배치), I6 착수 대기** — 2026-07-15
...
**다음 세션이 할 일**(8차분 완료로 I5 전체 완료, 아래 §6-12 참고): **I6(주인공 플로우)** — 캐릭터 생성([07_주인공_생성](../02_기획/07_주인공_생성.md)), 주인공 등판 매치 세션. `sim/eval`(S~D 평가)·`sim/market`(계약/FA/트레이드)·`sim/injury`의 치료 선택(`apply_injury`/`treat`)·`sim/training`(06_훈련_시스템 슬롯 배분)이 전부 이때 자연스럽게 같이 설계됨 — 전부 "주인공이 선택하는" UI 중심 시스템이라 I5(배경 시뮬)와는 성격이 다름.
