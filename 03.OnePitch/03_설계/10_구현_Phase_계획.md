...
| 나머지 이벤트 10개(38개 중 28개 저작 완료 — §6-120·§6-121·§6-133, 2026-07-26, 문장 뱅크 엔진+고교 1~3차 배치 17개)·서사형 업적·뉴스·미디어 | 저작 시간 문제 — 파이프라인·문장 뱅크는 갖춰짐. 남은 10개 중 대부분(FA·트레이드·콜업강등·스토브리그·드래프트데이·시상식 등)은 프로/대학 전용이라 고교엔 해당 없음, 데뷔전·대회우승 확정은 콜백 훅 신설 필요. 대학·독립리그·프로2군은 stage 전용 콘텐츠가 아예 0개(공통 10개만 적용) | 반복 저작 세션 — 대학/프로 전용 소재부터 채우거나(공백이 가장 큼), 데뷔전·대회우승용 콜백 훅부터 엔진에 배선 | 콘텐츠 저작(반복) |
...
### 6-92. 아마추어 경기수 확대 + 프로 포스트시즌 정비 — Part A(프로 5강 게이팅 버그 수정 + 프로2군 PO 신설) (2026-07-26, 완료)

**Context**: 리그별 연간 평균 경기수를 계산하는 대화 도중(리그 총경기수×2÷팀수로 정확 계산) 두 가지가 드러났다 — (1) 고교 SP 연 3.6경기·대학 SP 연 2.5경기로 선발 등판 기회가 지나치게 적음, (2) `run_pro_postseason`이 5강(`01_프로.md` §4)이 아니라 `standings`의 팀 전부(실제 10팀)를 게이지에 넣어서 6~10위까지 포스트시즌에 끌려들어가고 `best_ofs=[1,5,5,7]`(5팀=4라운드용) 밖 라운드는 전부 단판으로 폴백되던 진짜 버그. 이번 서브분은 4갈래 계획(경기수 확대+청백전+주인공 로테이션 경쟁 포함, 계획서 `happy-wibbling-bunny.md` 참고) 중 Part A만.

**구현**:
- `run_pro_postseason`: 승률 정렬 후 `seeded.truncate(5)` 추가 — 5강만 게이지에 들어가게 수정. 회귀 테스트(`run_pro_postseason_excludes_sixth_through_tenth_place_from_a_ten_team_league`) 신규 — 10팀 시나리오에서 6~10위가 `tournaments.participants`에 없는지 검증(기존 테스트는 5팀만 넣고 테스트해서 이 버그를 못 잡았음).
- `run_pro_farm_postseason`(신설) — `01_프로.md` §5 "축약 포스트시즌: 상위4팀, 단판 사다리" 그대로. `season_rollover`에 `run_pro_postseason` 바로 다음 줄로 배선. `tournament_display_name`(`api/game.rs`)에 `"pro_farm_postseason"` → "퓨처스 포스트시즌" 추가.
...
### 6-104. 매치엔진 리얼리즘 강화 Phase 3 — 주루 현실화 (2026-07-26, 완료)

**Context**: Phase 2까지도 주자 진루는 "무조건 hit_bases만큼"으로 고정이라(2루 주자가 단타에 무조건 3루까지만, 아무리 발 빠른 주자도 절대 홈까지 못 옴), 07_매치_엔진.md §9 "스피드 vs 경기운영+수비"로 판정하는 도루가 아예 없었다. `BatterStats`에 `speed`(스피드) 필드 추가, 단타·2루타의 "무리한 추가진루"와 1루 주자의 도루 시도를 확률화.

**구현**(`engine/src/sim/match_.rs`):
- `average_speed(lineup)` 신규(`average_defense`와 동일 패턴) — 개인이 아니라 "타석에 선 팀의 평균 주력"으로 근사.
- `advance_runners_realistic(rng, bases, hit_bases, outs, team_speed)` 신규 — 단타·2루타에서만 쓰며(3루타·홈런은 이미 전원 홈이라 판단 불필요), 이미 베이스에 있던 주자 각자가 `extra_base_prob`(평균 주력 높을수록·2아웃일수록↑)로 한 베이스씩 더 뛸지 개별 판정. 타자 본인은 항상 `hit_bases`까지만.
- `attempt_steal(rng, team_speed, pitcher_game_management, team_defense)` 신규 — 1루 주자만 대상(2·3루 동시 도루는 스코프 밖). 시도 자체도 스피드에 비례, 성공률은 스피드 vs 투수 경기운영(§10 "견제 흡수")·수비 중인 팀의 평균 수비력(포수 대리)로 판정.
- `simulate_half_inning`이 "1루 주자가 지금 누구인지"만 곁다리로 추적(`runner_on_first_id`, `bases` 자체는 여전히 점유 여부만 담는 단순 모델) — 매 타석 전에 도루 시도를 판정하고, 도루 성공/실패는 그 자리에서 아웃카운트·베이스 상태에 바로 반영.
- `BatterGameStats`에 `stolen_bases`·`caught_stealing` 필드 추가.

**구현**(`engine/src/data/match_session.rs`, `engine/src/data/repository.rs`):
- `apply_pa_outcome`(주인공이 던지는 인터랙티브 하프이닝)도 단타·2루타에 `advance_runners_realistic`을 쓰도록 갱신 — 배경 시뮬과 판정이 갈라지지 않게. **도루 이벤트 자체는 이 인터랙티브 경로에선 스코프 밖**으로 명시적으로 이월(1구 단위 루프에 끼워 넣으면 매 구마다 중복 판정될 위험이 있어 이번엔 배경 하프이닝에만 적용 — 주인공 팀 타석은 항상 배경 경로라 실질적으로는 대부분의 도루가 이미 커버됨, 주인공 자신이 던지는 동안의 상대팀 도루만 이월).
- `repository.rs`의 `BatterStats` 생성 모든 자리에 `speed`("스피드" JSON 키) 파싱 추가, `batter_stats_fields`에 SB·CS 추가해 season_stats에 영구 반영.

**실측 진단으로 발견한 실제 버그 2건 수정**(Phase 3 변경으로 배경 득점 효율이 낮아지면서 게임이 길어져 처음으로 노출됨, migration v15):
1. **수동 모드 감독 개입 핑퐁 무한 루프** — `submit_pitcher_change_decision`으로 "유지"/"맡기기(불풀)"를 확정해도 그 결정이 세션에 안 남아, 바로 다음 `submit_pitch` 호출에서 투구수가 아직 그대로라 강판 소프트캡 "고려 구간" 판정을 처음부터 다시 타서 또 `PitcherChangeDecision`을 돌려주는 무한 왕복. `pull_decision_settled_at_pitch_count` 필드를 세션에 추가해 "이 투구수에서 이미 안 뽑기로 했다"를 기억.
2. **강판하고 싶어도 불펜이 없으면 같은 자리에서 영원히 재시도** — AI가 강판을 결정해도(`should_pull_pitcher`=true) 로스터에 구원투수가 아예 없으면 `load_relief_pitcher`가 `None`을 반환하는데, 이 경우도 "확정됨"으로 안 남아 매 호출마다 다시 강판을 시도하다 실패했다. 위 필드로 동일하게 해결.

**테스트**: `cargo test --lib` 478개 전부 통과(신규 8개 — 빈 라인업 폴백 2개, 스피드에 따른 추가진루 1개, 도루 시도/성공률 3개, 도루 통합 시뮬레이션 1개, 위 핑퐁 버그 회귀 테스트 2개[교체 없음/불펜 없음 시나리오]). `cargo clippy` 클린. frb 재생성 불필요. `cargo build --release` 갱신 후 `flutter test` 27개 전부 통과. `balance_harness` 스모크 확인.
...
### 6-114. 엔진 확장 Phase 5 — NPC 시즌/통산 기록 아카이브 + 리더보드 화면 (2026-07-24, 완료)

**Context**: 5-Phase 계획의 마지막 항목, 확장 범위(사용자 명시적 선택 — 이번 시즌 스탯만 보여주는 최소 버전 대신 NPC 통산 기록 아카이브 테이블까지 신설). 이월 레지스트리 두 항목을 함께 해소: "타율·OPS 등 계산 함수는 있는데 소비 화면 없음"과, 리서치 중 발견한 "NPC 개인 시즌 기록이 시즌 경계마다 `DELETE FROM season_stats`로 통째로 사라짐"(주인공의 `career_history`에 해당하는 게 NPC에는 없었음).

**구현**(`engine/src/data/slot.rs`): migration v24 — `npc_season_history(player_id TEXT, season INTEGER, line TEXT, PRIMARY KEY(player_id, season))`(`career_history`와 같은 모양 + player_id 복합키).

**구현**(`engine/src/data/repository.rs`): `aggregate_stats_line(conn, table, player_id)` — `pitcher_stats_score`/`batter_stats_score`가 이미 하던 JSON 라인 순회·합산 로직을 "점수"가 아니라 원시 합산 라인 자체로 재사용(투수·타자 필드 전부, 필드명은 season_stats JSON과 동일) — `season_stats`(진행 중)와 `npc_season_history`(확정된 과거 시즌들)에 같은 모양으로 적용 가능. `NpcBattingLine`/`NpcPitchingLine` — 그 원시 라인에서 타율/출루율/장타율/OPS, ERA/WHIP/K9을 계산(`match_sim::BatterGameStats`/`CareerLine`과 동일한 공식이지만, season_stats JSON이 DB 경계를 넘나드는 순수 값이라 `pitcher_stats_score`와 같은 관례로 `serde_json::Value`에서 직접 뽑음). `archive_npc_season_stats(conn, season)` — `season_rollover`가 `DELETE FROM season_stats` 직전에 호출, 그 시즌 `season_stats`에 등장한 모든 player_id를 순회해 합산 라인을 `npc_season_history`에 upsert.

**구현**(`engine/src/api/game.rs`): `PlayerBattingStats`/`PlayerPitchingStats`(신규 struct) + 4개 API — `get_team_season_batting_stats(team_id)`/`get_team_season_pitching_stats(team_id)`(진행 중 이번 시즌, `season_stats` 기반)·`get_player_career_batting_stats(player_id)`/`get_player_career_pitching_stats(player_id)`(확정된 과거 시즌 전체 합산, `npc_season_history` 기반 — 이번 시즌 진행분은 시즌 종료 시점에만 아카이브되므로 포함 안 됨, 그 몫은 위 season 함수가 담당). `flutter_rust_bridge_codegen generate`로 Dart 바인딩 재생성(`app/lib/src/rust/api/game.dart` 등 5개 파일) — 이 환경의 build_runner 서브스텝 실패는 기존 전례대로 무시 가능했음. **재생성 후 `cargo build --release`를 다시 안 돌리면 `flutter test`가 "Content hash on Dart side ... different from Rust side" 에러로 즉시 실패**(frb_generated.rs가 바뀌었는데 그걸 컴파일한 DLL이 옛 버전이라 발생) — 이번에 실측으로 확인, 향후 API 표면을 바꾸는 Phase는 "frb 재생성 → release 재빌드" 순서를 반드시 지켜야 함.

**구현**(`app/lib/features/league/league_screen.dart`): `_RosterTab`이 로스터·스태프와 함께 팀 시즌 투수/타자 스탯을 같이 fetch해 `player_id`로 맵핑, `_PlayerListCard`가 각 선수 행에 한 줄 요약(투수: 이닝/ERA/탈삼진/세이브/홀드, 타자: 타율/OPS/홈런/타점)을 추가 표시(표본 0이면 생략). **부수적으로 발견해 같이 고친 버그**: 기존 포지션 필터가 `p.position == '선발투수' || p.position == '구원투수'`였는데, 이 엔진의 실제 포지션 값은 "선발투수/중계투수/마무리투수"(§6-98 Part F 3분류, "구원투수"라는 값 자체가 존재한 적이 없음) — 중계·마무리투수가 전부 "타자" 목록에 잘못 섞여 있었다. `_pitcherPositions = {'선발투수','중계투수','마무리투수'}` 상수로 교체. 통산 기록 상세(탭하면 펼쳐지는 다이얼로그)는 이번엔 스코프 아웃 — 계획 문서의 "카드 한 줄 요약" 최소선은 충족했고, 탭 상호작용까지는 다음 손볼 사람 몫으로 남김(급하지 않음, 현재도 화면이 깨지지 않음).
...
### 6-118. 스케줄 분산 Phase 4 — 재측정 (2026-07-24, 완료, 5-Phase 계획 전체 완료)

**Context**: Phase 1~3(§6-115~117) 적용 후 실제로 워스트케이스가 줄었는지 `perf_probe.rs`로 재측정 — 이 세션의 성능 조사 전체를 매듭짓는 마지막 단계.

**측정**(`perf_probe.exe 1`, 동일 시드 424242·시즌 상한 1, 이 세션 초반 측정과 동일 조건이라 직접 비교 가능):

| 지표 | 원래 baseline(N+1 배치 전) | StatScoreCache만(§ 이전 대화) | Phase 1~3 적용 후 |
|---|---|---|---|
| 1시즌 총 소요 | 73.03s | 69.76s | **61.88s** (-15.3% vs 원래) |
| 워스트케이스 단일 호출 | ~46s | (미측정) | **24.02s** (-48% vs 원래) |
| 진행 일수당 평균 | (미측정) | (미측정) | 166.8ms/일 |
...
### 6-120. 이벤트 콘텐츠 저작 Phase 1~2 — 문장 뱅크 엔진 + 고교 1차 배치 (2026-07-25, 완료)

**Context**: 이월 레지스트리 "나머지 이벤트 27개(38개 중 11개만 저작)"를 채워나가기 시작. 사용자 지정: 고등학교부터 우선, 문구·멘트는 FM(Football Manager)/OOTP 식으로(같은 이벤트가 여러 번 떠도 매번 다른 대사). 조사 결과 `02_기획/콘텐츠/06_서술_템플릿.md`에 이미 "변수 슬롯 템플릿 + 조건별 문장 뱅크(랜덤 선택, 최근 사용 회피)"가 설계돼 있었지만 "구조 확정(초안)" 상태로 실제 저작·엔진 연동은 아무도 안 한 상태였다 — 이번이 그 최초 실체화. `content.db`의 `narrative_templates` 테이블(매치 커멘터리·뉴스 기사 등 더 큰 범위)은 소비 코드가 전혀 없는 걸 확인해, 이번 스코프는 `events` 테이블 자체에 문장 뱅크를 붙이는 좁은 범위로 한정(새 트리거 변수·새 콜백 지점도 안 만듦).

**구현 — 문장 뱅크 엔진**(`engine/src/data/content.rs`): migration v4 — `ALTER TABLE events RENAME COLUMN body TO bodies`(컬럼 타입은 그대로 TEXT, `trigger`/`choices`처럼 JSON 문자열 배열을 담는 관례로 전환). (`engine/src/sim/event.rs`): `pick_body_variant(bodies, recent, rng)` 신설 — `recent`와 같은 문장은 후보에서 제외 후 결정론적 랜덤 선택(배열 길이 1이면 예외적으로 재사용). (`engine/src/data/repository.rs`): `fire_event`에 `world_seed` 파라미터 추가(호출부 2곳), `bodies` JSON 파싱, `last_event_body`(신규 — `career_events.detail`에서 직전 발동 문장을 읽음, 새 테이블 없이 기존 로그 재사용) + `league_sub_seed(world_seed, "event_body:{id}:{day}")`로 시드한 RNG로 `pick_body_variant` 호출, `log_career_event`의 `detail`에 이번에 뽑힌 문장을 `{"body": ...}`로 저장(다음 발동의 "직전 문장" 조회 대상). (`engine/src/bin/seed_content.rs`): `EventSeed.body: String` → `bodies: Vec<String>`, **`bodies.len() < 3`이면 seed 실패**(관리 측면 — 문장 1~2개짜리가 실수로 섞여 들어가는 걸 저작 시점에 차단). (`tool/lib/src/content_seed.dart`): Dart 쪽 TOML→JSON 조립 필드도 `body`→`bodies`로 동기화(빠뜨렸으면 Rust 쪽만 고치고 파이프라인 앞단이 여전히 옛 필드를 읽어 조용히 깨졌을 지점).

**구현 — 콘텐츠**(`data/seed/events.toml`): 기존 11개 전부 `bodies` 배열(문장 3개씩, 기존 문장 포함)로 전환. **고교 1차 배치 8개** 신규 저작(`stage = "league:hs"`, 기존 5개 트리거 유형만 사용) — `event:hs_flower_season_opens`/`hs_flower_season_finale`(캘린더, 개나리기/패왕기 시즌 — `02_아마_고교.md` 5대 전국대회 근거), `hs_crossroads_looms`(확률, 카탈로그 "진로 갈림길" 재구성), `hs_region_losing_skid`(팀 연패, 카탈로그 재구성), `hs_senior_mentorship`·`hs_scout_visit`·`hs_study_pressure`(확률, 신규 고교 전용 소재), `hs_teammate_counsel`(확률, 카탈로그 "동료 상담 요청" 재구성). 총 19개(11+8), 38개 카탈로그 중 19개 남음.

**구현 중 발견 — 파이프라인 2단계 구조**: `seed_content.rs`(Rust)는 JSON만 받는다 — `data/seed/*.toml`을 실제로 파싱해 JSON으로 조립하는 건 `tool/`(Dart) 쪽(`dart run bin/tool.dart content seed`, `Platform.script` 경로 해석 문제로 `dart run tool ...`(패키지명 호출)이 아니라 `dart run bin/tool.dart ...`(직접 파일 경로)로 실행해야 함 — 전자는 `.dart_tool/pub/` 심볼릭 경로 때문에 리포 루트 계산이 깨짐). `content seed`는 `engine/content.db`만 갱신 — `app/assets/content.db`는 그 수동 복사본이라(`07_데이터관리.md`에 이미 문서화된 관례) 이번에 별도로 `cp` 필요.

**테스트**: `sim::event::tests`에 `pick_body_variant` 3개(직전 문장 회피, 풀 1개일 때 재사용, 결정론). `content.rs`에 `v4_renames_body_to_bodies_on_events`, `fresh_db_migrates_to_v4` 갱신. `repository.rs`에 `fire_event_avoids_repeating_the_previously_shown_body_variant`(문장 2개짜리 이벤트를 쿨다운 없이 연속 발동시켜 직전과 다른 문장이 나오는지 — 후보 1개만 남아 결정론적으로 검증됨). `tool/test/content_seed_test.dart`의 기존 이벤트 픽스처를 `bodies` 배열로 갱신 + `bodies` 필드 조립 확인 단언 추가. `cargo test --lib` 529개 전부 통과. `cargo clippy --lib --tests --bins` 클린. `dart test`(tool) 6개 전부 통과, `dart analyze`(tool) 클린. `dart run bin/tool.dart content seed` — `events=19` 커밋 성공, `content validate` FK 위반 0. `cargo build --release` 갱신 후 `flutter test -j 1` — **첫 시도에서 2개 실패**(`game_loop_test.dart`·`records_test.dart`, 둘 다 "정지점(주인공 경기)까지 비-게임 PendingAction을 최대 20번 넘기고 진행"하는 guard 루프가 있는데, 확률형 이벤트가 6개 늘면서 그 20번 안에 실제 게임까지 못 도달 — `pending.first.kind`가 여전히 `'event'`인 채로 assert 실패). guard를 20→60으로 올려 재실행, 27개 전부 통과. `balance_harness -- 5 3` — 평균 이벤트 발동 수 29.40→**58.40건/커리어**(예상대로, probability형 6개 추가로 거의 배증), 등급 분포·부상 이력은 이전과 동일(순수 콘텐츠 확장이라 핵심 시뮬 지표엔 영향 없음), 크래시 없음.

**§5 이월 레지스트리 갱신**: "나머지 이벤트 27개(38개 중 11개만 저작)" → 19개 저작 완료, **19개 남음**(대학·프로·독립리그 및 고교 미저작분) — 완전 해소 아님, 계속 반복 저작 트랙(다음은 사용자 지정에 따라 대학 또는 나머지 고교 소재).

### 6-121. 이벤트 콘텐츠 저작 — 고교 2차 배치 (2026-07-25, 완료)

**Context**: §6-120에서 "다음에 채울 수 있는 고교 소재"로 짚어둔 항목 중, 카탈로그(`02_이벤트.md` §1)에 있고 기존 5개 트리거만으로 되는 것들 — 역할 변경(후보→주전→에이스 뉘앙스)·개인기록근접·신입생 합류·라이벌 매치업·팀 회식·가족 연락 6개. **"데뷔전"·"팀 우승/PO 진출 확정"은 이번에도 스코프 아웃** — 둘 다 실제 사건(첫 등판, 대회 결과)에 새 콜백 훅을 걸어야 해서 순수 콘텐츠 저작 범위를 벗어남, 다음 배치 후보로 남김.

**구현**(`data/seed/events.toml`): `event:hs_role_shift`·`event:hs_record_approaching`·`event:hs_freshman_joins`·`event:hs_rival_matchup`·`event:hs_team_dinner`·`event:hs_family_call` 6개 신규 저작, 전부 `type = "probability"`(정확한 상태 트리거가 없는 항목들이라 플레이버로 근사 — 이 코드베이스에 이미 있던 "동료상태를 감독관계도로 근사" 같은 관례와 같은 결), `stage = "league:hs"`, 문장 3개씩. 엔진 코드 변경 없음(§6-120에서 이미 완성된 문장 뱅크 파이프라인 그대로 재사용).
...
**사용자 피드백으로 후속 수정(4차) — 실사 사진 색조 보정 + 좌표 기반 선수 오버레이**: 3차 커밋 직후 사용자가 "도트 아트 말고 아까 02 버전(=`02.SvelteElectron` 레트로 GBC 모드의 실사 픽셀아트) 디자인으로"를 요청. 진짜 새 포토리얼 아트를 종류별로 생성할 수 있냐는 질문엔 정직하게 답함 — 이 환경엔 이미지 생성 모델 툴 자체가 없어(코드 실행·파일 입출력만 가능) 절차적 벡터 드로잉이나 기존 사진 편집(자르기·색보정·리사이즈)만 가능하고, 요청받은 "종류별로 완전히 새로운 포토리얼 아트"는 재현 불가능함을 먼저 밝힘. `AskUserQuestion`으로 대안 확인 → "원본 사진 1장을 구장별로 색조/밝기 보정"으로 확정. `probaseball.gif` 원본에서 4프레임(480×443, 16프레임 중 4칸 간격 샘플링)을 뽑아 PIL `HSV` 채널 조작(`point()` 룩업, `zlib.crc32` 해시로 색조 ±12도·채도 0.92~1.18·밝기 0.92~1.14·대비 0.96~1.10 결정적 변주 — 처음엔 색조 ±30도로 시도했다가 흙이 빨갛게·잔디가 갈색으로 뒤집혀 보여 폭을 좁힘)로 27+1개 `.gif`(구조 100% 동일, 색만 다름, 총 21MB)를 재생성, 도트아트 PNG는 삭제. 이어서 사용자가 "베이스·타석 좌표를 잡아서 그 위에 바둑알로 선수를 그려달라"고 요청 — 원본 사진에 5%/1% 격자를 겹친 뒤 육안으로 홈·1·2·3루·마운드·양타석 좌표를 픽셀 단위로 캘리브레이션(마커를 그려서 실제 흰색 베이스 위에 정확히 겹치는지 왕복 확인). 외야수·내야수 표시 가능 여부를 물어봐 "엔진이 개별 수비수 위치를 시뮬레이션하지 않는다(`average_defense()`로 팀 전체를 스칼라 하나로 뭉침)"는 제약을 먼저 설명하고, 사용자 확인 후 9자리 전부 장식용 고정 좌표로 추가. 타자는 `npc.handedness`(엔진에 이미 있던 실제 데이터, 플래툰 매치업 계산에 씀)를 `get_batter_profile`에 신규 노출해 좌타자는 1루측, 우타자는 3루측 타석에 정확히 배치(양타는 우타석 고정).

**구현(4차) — 엔진**: `api::game::BatterProfileInfo`에 `handedness: String`(원문 "좌타"/"우타"/"양타") 필드 추가, `get_batter_profile`이 `npc.handedness` 컬럼을 같이 조회.

**구현(4차) — Flutter**: `match_stadium.dart`에 `FieldCoords`(구장 사진 위 0~1 비율 좌표 15개 — 베이스 4+마운드+좌우타석 2+수비 9)와 `_FieldCoord`/마커 렌더 헬퍼 신규. `StadiumFieldView`가 `LayoutBuilder`로 실제 렌더 크기를 얻어 비율 좌표를 픽셀로 환산, `Positioned` 배지로 수비 9자리(장식, `fielderColor`=수비팀 학교색)·타자(손잡이별 타석, `batterHandedness`)·주자(점유 베이스만, `runnerColor`=타격팀 학교색)를 이미지 위에 직접 그린다. `match_screen.dart`는 수비팀 id(`pitchingTeamId` = 타격팀의 반대)를 계산해 `fielderColor`로 넘기고, `getBatterProfile` 결과(이미 `_BatterInfoCard`가 쓰던 것과 별개로 한 번 더 조회, 이 저장소의 기존 관례대로 재빌드마다 재조회)의 `handedness`를 `StadiumFieldView`에 전달. 우측상단 다이아몬드+B-S-O 오버레이는 사용자 확인대로 그대로 유지(정보 중복이어도 한눈에 보기 편함).

**테스트(4차)**: 엔진 `get_batter_profile_returns_name_and_ability_stats_for_a_roster_player`에 `handedness`가 좌타/우타/양타 중 하나인지 단언 추가, `cargo test --lib` 548개 전부 통과, `cargo clippy --lib --tests --bins` 클린. Flutter `match_boxscore_stadium_widget_test.dart`에 수비 9자리 라벨(P/C/1B/2B/SS/3B/LF/CF/RF)이 전부 렌더되는지 단언 추가 + 나머지 두 `StadiumFieldView` 테스트에 신규 필수 파라미터(`fielderColor`/`batterHandedness`) 반영, `flutter analyze` 클린, `flutter test -j 1` 전체(35개 파일) 전부 통과. `balance_harness -- 5 3` 스모크 — 크래시 없음, 등급 분포·S등급 비율(6.7%) 완전 동일(이번 변경도 전부 조회/표시 전용이라 예상대로 시뮬레이션에 영향 없음).

**스크린샷 검증(4차)**: 실제 매치 세션까지 진행시켜 1100×950 캡처 — 수비 9자리 배지가 각자 위치(투수 마운드 위, 포수 홈 뒤, 내야 4자리, 외야 3자리)에 정확히 표시되고, 타자 마커가 실제 타석 박스 안에 들어가며, 박스스코어·구장 폭 정렬·다이아몬드 오버레이 모두 3차와 동일하게 유지됨을 확인.

**영향 없음**: 세이브 마이그레이션은 v26 하나(컬럼 추가만, 기존 세션 로드 경로는 `load_session`이 항상 5개 신규 컬럼을 같이 읽어와 하위 호환 이슈 없음). NPC 시즌 기록 파이프라인(§6-114)·박스스코어 계산 자체는 이미 있던 `HalfInningStats`/`BatterGameStats` 재사용이라 밸런스에 영향 없음(위 스모크 결과로 확인). `handedness` 필드 추가는 기존 `platoon_edge_for_pitcher` 계산 로직을 안 건드리고 읽기만 늘린 것이라 시뮬레이션 영향 없음. 구장 이미지 생성 스크립트(Python/Pillow) 자체는 이 저장소의 Rust/Dart 툴체인과 무관한 일회성 생성기라 저장소엔 안 넣고, 결과물(27+1개 GIF)만 커밋 — 구장 목록이 바뀌면(예: I8 이후 구장 추가) 이 기록을 참고해 같은 스크립트를 재작성하면 됨. 필드 좌표(`FieldCoords`)는 27개 GIF가 전부 같은 원본 사진의 색조 보정판(구조 동일)이라 좌표 하나로 전 구장 공용.

### 6-129. 포지션별 개인 수비 스탯을 실제 판정에 반영 + 매치 화면 수비 배지 실시간 하이라이트 (2026-07-25, 완료)

**Context**: §6-128의 수비 9자리 배지가 순수 장식(엔진에 포지션별 데이터가 없어 표준 대형만 표시)이라는 걸 사용자에게 미리 밝혔는데, 사용자가 "그럼 엔진에 수비 9자리나 선수들 수비 스텟이 적용되나?"라고 되물어 확인해보니 — 선수 개인 "수비" 스탯 자체는 `npc.stats`에 있지만 `average_defense()`가 라인업 전체를 평균 하나로 뭉개서 팀 평균 실책 확률에만 쓰고 있었다(포지션 구분 전혀 없음, `src/sim/match_.rs` 주석에도 "포지션별 가중치는 스코프 밖"이라 명시돼 있었음). 사용자가 "지금 할래, 계획안 올려줘"로 즉시 착수 요청 — 4-Phase 계획(①라인업에 포지션 배선 ②타구 유형→포지션 판정+개인 수비 스탯 적용 ③두 투구 경로(배경/인터랙티브) 배선+`AwaitingPitch` 노출 ④Flutter 배지 하이라이트) 제시 후 "한 번에 다" 진행 확인, 투수·포수는 실제로 타구를 처리하는 일이 드물어 후보에서 제외(사용자 확인) — 내야 4자리·외야 3자리 7종만.

**구현 — Phase 1(라인업 포지션 배선)**: `sim::match_sim::BatterStats`에 `position: String` 필드 추가. `repository::load_batting_lineup`(프로덕션 라인업 로더)와 `run_intrasquad_scrimmage`(청백전)가 `npc.position` 컬럼을 같이 읽어 채움. 기존 `BatterStats` 리터럴 36곳 중 대부분이 이미 `avg_batter()`/`..avg_batter()` 구조체 갱신 문법을 쓰고 있어(2026-07-25 이전 관례) 그 헬퍼 2곳(`match_.rs`·`pitch.rs`)과 나머지 예외적 전체 리터럴 몇 곳만 손보면 됐다 — 처음에 Python 정규식으로 일괄 치환하다 raw string 이스케이프 실수(`\"유격수\"`가 그대로 파일에 박혀 파싱 에러)를 냈고 바로 잡음.

**구현 — Phase 2(타구 유형→포지션 판정+개인 수비 적용)**: 이미 있던 `roll_batted_ball_type`(땅볼 45%/뜬공 35%/직선타 20%)을 확장 — `roll_fielder_position`이 땅볼→내야 4자리 가중치(유격수 30%·2루수 27%·3루수 24%·1루수 19%), 뜬공→외야 3자리 가중치(중견수 40%·좌우익수 각 30%), 직선타→내야/외야 반반 중에서 포지션 하나를 뽑는다. `fielder_defense_at(lineup, position)`이 그 포지션 라인업 선수를 찾아 개인 `defense` 스탯을 반환(못 찾으면 팀 평균 폴백 — 포지션 결원 등 방어적 처리). `resolve_in_play_result`의 반환 타입을 `PaOutcome` 단독에서 `InPlayResolution { outcome, fielder_position }`로 변경 — 안타든 아웃이든 "어느 포지션으로 갔는지"는 항상 의미가 있어(안타여도 그 방향 야수가 처리/추격) 결과 종류와 무관하게 항상 포지션을 반환. `simulate_plate_appearance`(배경 엔진)는 시그니처만 `team_defense: f64` → `fielding_lineup: &[BatterStats]`로 바꾸고 반환은 여전히 `PaOutcome`(`.outcome`만 취함, 배경 시뮬은 포지션 정보 자체가 필요 없음).

**구현 — Phase 3(두 경로 배선+API 노출)**: `simulate_half_inning`·`simulate_game`(배경 전체 시뮬)과 `sim::pitch::simulate_at_bat_automatically`(분포 회귀 테스트 전용, 프로덕션 미사용)까지 전부 `team_defense: f64` 매개변수를 `fielding_lineup: &[BatterStats]`로 교체 — `simulate_game`은 이제 라인업을 그대로 넘기기만 해서 오히려 사전 계산(`average_defense`)이 사라져 더 단순해짐. `data::match_session.rs`의 인터랙티브 1구 루프(`run_until_decision_point`)에 `last_fielder_position: Option<(String, bool)>` 로컬 변수를 `loop` 밖에 둬서 "InPlay 처리하는 반복"과 "그다음 AwaitingPitch를 반환하는 반복"이 같은 함수 호출 안의 다른 반복이어도 값이 살아있게 하고, 하프이닝이 넘어가는 두 지점(자기 팀 전환·배경 하프이닝 완료→outs=3 세팅)에서 지워 몇 이닝 전 플레이가 계속 표시되는 걸 막는다. `MatchStepResult`/`MatchStepInfo`(Dart 노출용) 양쪽의 `AwaitingPitch`에 `last_fielder_position: Option<String>`·`last_play_was_error: bool` 필드 추가.

**구현 — Phase 4(Flutter 배지 하이라이트)**: `match_stadium.dart`에 엔진 한글 포지션명 → 배지 라벨 매핑(`_koreanPositionToBadge`) 신규. `StadiumFieldView`가 `lastFielderPosition`/`lastPlayWasError`를 받아 해당 배지에 성공(파랑, `AppColors.accent`)/실책(빨강, `AppColors.danger`) 테두리+글로우를 얹는다 — 장식이던 9자리 배지가 실제 판정과 연결됨. `match_screen.dart`가 `awaiting.lastFielderPosition`/`.lastPlayWasError`를 그대로 threading.

**테스트**: 엔진 신규 5건 — `roll_fielder_position_never_picks_pitcher_or_catcher`, `roll_fielder_position_stays_within_the_infield_for_ground_balls_and_outfield_for_fly_balls`, `fielder_defense_at_uses_the_individual_players_stat_when_present`, `fielder_defense_at_falls_back_to_team_average_when_the_position_is_missing`, `a_shortstop_with_worse_defense_than_the_rest_of_the_lineup_produces_more_errors_on_ground_balls`(핵심 회귀 — 유격수만 수비 낮은/높은 라인업 5000시행씩 비교, 팀 평균이 아니라 그 자리 선수 개인 스탯이 실제로 쓰이는지 검증). 기존 `resolve_in_play_result`/`simulate_plate_appearance`/`simulate_half_inning`/`simulate_at_bat_automatically` 직접 호출 테스트 약 15곳을 새 시그니처(`&[BatterStats]` 인자, `.outcome` 필드 접근)에 맞춰 갱신, `lower_team_defense_produces_more_reach_on_error_outcomes_on_average`는 7자리 전부 같은 수비력으로 채운 라인업을 만들어 기존 "팀 평균" 테스트 취지를 그대로 유지. `cargo test --lib` 553개 전부 통과, `cargo clippy --lib --tests --bins` 클린. Flutter `match_boxscore_stadium_widget_test.dart`에 하이라이트 색상 검증 신규 1건(유격수 지정 시 SS 배지 테두리가 성공/실책/없음에 따라 정확히 바뀌는지), `flutter analyze` 클린, `flutter test -j 1` 전체(37개 파일) 전부 통과. `balance_harness -- 5 3` — RNG 스트림에 새 굴림(포지션 뽑기)이 매 인플레이마다 추가돼 결과가 달라지는 게 정상(§4 목표 등급 분포는 유지되는지만 확인): 크래시 없음, S등급 비율 6.5%(§6-128의 6.7%와 노이즈 수준 차이, §4 목표 "남발 안 되되 0은 아님" 그대로 충족), 등급 분포·이벤트/부상/업적 평균 전부 같은 범위 — 예상대로 실책 확률의 "실효 스케일"(공식 자체는 안 바꾸고 입력만 팀 평균→개인 스탯으로 교체) 덕에 큰 요동 없이 안정적임을 확인.
...
### 6-131. 포지션별 개인 수비 기록(기회·실책·수비율) 누적 + 로스터 탭 표시 (2026-07-25, 완료)

**Context**: §6-130 완료 후 남은 두 항목(포지션별 개인 수비 기록 누적, 볼카운트 판정 공식 수치 스케일 확정) 중 전자를 진행. 설계 도중 발견 — §6-129가 포지션별 개인 수비 스탯을 실책 "확률 계산"엔 이미 쓰고 있었지만, 그 결과를 선수 개인 기록으로 "적립"하는 건 인터랙티브 경로(`data::match_session`)와 배경 경로(`sim::match_.rs`의 `simulate_plate_appearance`/`simulate_half_inning`) 둘 다 통째로 안 하고 있었다 — 특히 배경 NPC 대 NPC 경기(`process_day`)는 `fielder_position` 자체를 계산만 하고 버리는 구조라, 애초 계획서의 "두 경로에 배선"보다 실제로는 `HalfInningStats`에 새 누산기를 얹는 구조 변경이 필요했다(당초 예상보다 스코프가 컸던 지점).

**구현 — 엔진 데이터 구조**: `sim::match_.rs`에 `FieldingGameStats { chances, errors }`(+`fielding_percentage()`, 기회 0이면 1.0 폴백) 신규. `simulate_plate_appearance`의 반환 타입을 `PaOutcome` 단독에서 `PlateAppearanceResult { outcome, fielder_position: Option<&'static str> }`로 변경 — 삼진·볼넷·사구는 `None`, 인플레이면 `resolve_in_play_result`가 뽑은 포지션을 그대로 실어 호출부가 개인 수비 기록을 쌓을 수 있게 한다. `HalfInningStats`에 `fielders: HashMap<String, FieldingGameStats>` 필드 추가, `simulate_half_inning`이 `par.fielder_position`으로 `fielding_lineup`에서 실제 담당 선수를 찾아 기회(+실책이면 실책)를 적립. `GameResult`에 `home_fielding_stats`/`away_fielding_stats` 추가 — 강판으로 갈라진 3단계 누산기(선발/중계/마무리 이닝)의 수비 기록은 투수와 무관하게 같은 팀 수비수 것이므로 `merge_fielding_stats`로 합친다(주의: `top_half_stats`는 원정 타자 vs 홈 투수라 `.fielders`는 "홈" 수비수 — 배터 쪽과 팀이 반대로 뒤집힘).

**구현 — 영속화**: `repository.rs`에 `upsert_fielder_season_stats`/`upsert_fielder_practice_stats`(기존 `upsert_stats_fields` 델타머지 재사용, `season_stats.line`이 스키마리스 JSON이라 **마이그레이션 불필요**) 추가, `process_day`(배경 NPC전)와 `run_intrasquad_scrimmage`(청백전)에 각각 배선. `data::match_session.rs`는 두 지점 — ①주인공 팀의 배경 하프이닝(`simulate_half_inning` 호출부)에서 `half_inning_stats.fielders`를 그대로 순회 upsert, ②인터랙티브 1구 루프의 InPlay 분기(`resolve_in_play_result` 직접 호출부, `last_fielder_position` 계산과 같은 자리)에서 `fielding_lineup.iter().find(|b| b.position == resolution.fielder_position)`로 실제 선수를 찾아 1건씩 upsert. `season_stats`가 스키마리스 JSON이라 `npc_season_history`로의 시즌 아카이브(`archive_npc_season_stats`)도 필드 목록을 하드코딩하지 않고 전체 키를 합산하므로, 통산(커리어) 기록도 별도 작업 없이 자동으로 이어졌다.

**구현 — 조회 API + UI**: `repository::NpcFieldingLine`(수비율 계산), `api::game::PlayerFieldingStats`+`get_team_season_fielding_stats(team_id)` 신규 — `get_team_season_batting_stats`와 동일한 대상(투수 3종 제외)·소스(진행 중 `season_stats` 합산). 리그 화면 로스터 탭(`league_screen.dart`의 `_RosterTab`, 팀 batting/pitching 요약이 이미 있던 자리)에 `_fieldingSummary`로 한 줄 추가 — "수비율 .833 (기회 6 실책 1)" 형식, 기회가 0(투수·포수는 §6-129부터 수비 후보 제외라 항상 0)이면 표시 자체를 생략. 원래 계획엔 "RecordsScreen에 탭 추가"라고 적었지만, 실제 코드를 보니 `RecordsScreen`의 7탭은 전부 주인공 개인 기록(경기 로그·계약·부상 등)이고 팀 로스터 타격/투구 성적은 리그 화면 로스터 탭이 담당하는 구조라 — 그 기존 패턴을 따라 리그 화면에 배치(주인공은 투수 전용 캐릭터라 본인 수비 기록 자체가 존재하지 않으므로 RecordsScreen엔 애초에 넣을 게 없음).
...
### 6-133. 이벤트 콘텐츠 저작 — 고교 3차 배치 (2026-07-26, 완료)

**Context**: "이벤트를 좀 더 확대하자"로 착수 전, 현재 저작 현황(38개 카탈로그 중 25개 완료, 대학·독립리그·프로2군은 stage 제한 이벤트가 0개라는 공백)을 정리해 제시 — 사용자가 "고교 3차 배치"를 선택. §6-121에서 두 번째로 스코프아웃됐던 "동료 상태 기반" 카탈로그 항목 중 새 콜백 없이 되는 나머지 3개(동료 부상·은퇴·이적/작별, 동료 마일스톤 축하, 새 스태프 부임)를 저작.

**구현**(`data/seed/events.toml`): `event:hs_teammate_setback`·`event:hs_teammate_milestone`·`event:hs_new_staff_arrival` 3개 신규 저작. 전부 `type = "probability"`(chance=0.02, 기존 고교 확률형 이벤트와 동일 — "그 동료가 정확히 누구인지·무슨 사건인지" 판별할 실제 트리거 변수가 없어서, §6-120/121이 이미 확립한 "동료상태를 확률형으로 근사" 관례를 그대로 재사용), `stage = "league:hs"`, 문장 3개씩. 엔진 코드 변경 없음(기존 문장 뱅크 파이프라인·5개 트리거 변수 그대로).

**테스트**: 신규 Rust/Dart 테스트 없음(순수 seed 데이터 추가). `dart run bin/tool.dart content seed` — `events=28`(25+3) 커밋 성공, `dart run bin/tool.dart content validate` FK 위반 0. `app/assets/content.db` 재동기화. `cargo test --lib` 556개(변경 없음이라 그대로 통과), `flutter test -j 1` 38개 파일 전부 통과. `balance_harness -- 5 3` — 평균 이벤트 발동 수 80.80→**87.60건/커리어**(확률형 3개 추가로 예상대로 소폭 증가), S등급 비율 6.5%(§6-132와 동일, 변화 없음), 크래시 없음.
...
### 6-135. 이벤트·메시지·뉴스 전체 확장 로드맵 확정 + Phase 1(독립리그·프로2군 이벤트 배치) (2026-07-26, Phase 1 완료)

**Context**: "이벤트를 좀 더 확대"·"선택지없는 정보 메세지도 늘려보자"에 이어, 사용자가 지역·전국 순위 조회처를 물어보며 "저런거나 이런 게임에 재미가 되는 소소하거나 큰 정보들이 이벤트오는것처럼 메세지로 왔으면"이라고 요청 — 조사해보니 정확히 `02_기획/콘텐츠/05_뉴스_미디어.md`("구조 확정"까지 끝났지만 한 번도 구현 안 된 카테고리)와 일치함을 확인. 세 갈래(이벤트 콘텐츠·선택지없는 메시지·뉴스·미디어)를 통합해 8-Phase 로드맵으로 정리하고 사용자 확인을 받음: 순서는 제안대로(콘텐츠→작은 엔진 확장→뉴스 기초→나머지 콘텐츠→뉴스 확장), Phase 6(주간 다이제스트)엔 전용 지역·전국 순위 조회 화면도 같이 만들기로, Phase 7(NPC 시장 소식)은 배경 NPC간 트레이드·FA 시뮬레이션 자체가 아직 없어(확인됨) 이번 확장에서 제외하고 별도 논의로 이월.

**확정 로드맵**(요약 — 상세는 위 대화 및 각 Phase 완료 시 이 문서에 개별 기록):
1. 이벤트: 독립리그·프로2군 배치 ← **이번 항목**
2. 메시지: 마일스톤 종류 확장(탈삼진·이닝) + 첫 승/첫 세이브 알림(콜백형)
3. 뉴스 1부: 소속팀·라이벌 경기결과(콜백형, 새 시뮬 불필요 — `schedule`/`standings`/`rivals_json` 재사용)
4. 뉴스 2부: 동료 개인기록 소식 실측화(§6-134 마일스톤 로직을 팀 동료에게도 적용, 기존 확률형 근사 대체)
5. 이벤트: 나머지 캘린더형 카탈로그(스토브리그 개시·시상식·스프링캠프 시작·포스트시즌 개막·드래프트 데이·트레이드데드라인)
6. 뉴스 3부: 인접권역·먼 리그 주간 다이제스트 + 지역·전국 순위 조회 화면(신규 Flutter UI)
7. (별도 이월) NPC 시장 소식 — 배경 NPC간 트레이드·FA 시뮬레이션 신설 필요
8. (별도 이월) 콜백 훅 필요한 이벤트(데뷔전·대회우승 확정) — 이벤트 시스템에 새 콜백 지점 신설 필요

**구현 — Phase 1(`data/seed/events.toml`)**: `stage = "league:independent"`·`stage = "league:pro_farm"` 착수(그 전까지 둘 다 0개, 대학 다음으로 컸던 공백). `04_독립.md`의 "재도전 무대" 정체성(미지명·늦깎이·방출 후 재도전)·4단계 생존리그(매 스테이지 컷오프 긴장)·상무 피닉스 로스터 변동성(매 시즌 40~50% 교체)·7월말 트레이드데드라인 중도 지명 소재로 독립리그 5개(`indie_second_chance`·`indie_stage_cutoff_looms`·`indie_sangmu_rumor`·`indie_veteran_return`·`indie_trade_deadline_looms`), `01_프로.md` §5 "육성·대타 평가" 정체성(1군 콜업 기대·재활파견 선수·축약 포스트시즌)으로 프로2군 4개(`farm_callup_hope`·`farm_rehab_teammate`·`farm_development_focus`·`farm_ladder_playoff`) 저작. 기존 5개 트리거만 사용, 새 엔진 작업 없음. 총 이벤트 37→**46개**.
...
**§6-135 로드맵 진행 상황(당시)**: Phase 1~4 완료, Phase 5·6 남음.

### 6-139. 이벤트 Phase 5 — 프로 캘린더형 카탈로그 잔여분 (2026-07-26, 완료 — 로드맵 Phase 1~5 완료)

**Context**: §6-135 로드맵의 Phase 5. 38개 카탈로그 "캘린더 기반" 중 남아있던 6개(스토브리그 개시·시상식·스프링캠프 시작·포스트시즌 개막·드래프트 데이·트레이드데드라인) — `01_프로.md` §7 시즌 캘린더(2월말 스프링캠프→3월 개막→7월중순 올스타→7월말 트레이드데드라인→10월말~11월초 포스트시즌→11월 시상식·드래프트→12월 스토브리그)를 그대로 따라 `stage="league:pro"`로 6개 저작.

**구현(`data/seed/events.toml`)**: `pro_trade_deadline`(week=15)·`pro_postseason_opens`(week=30)·`pro_awards_ceremony`(week=32)·`pro_draft_day`(week=33)·`pro_stove_league_opens`(week=36)·`pro_spring_camp_begins`(week=50) 6개. week 값은 기존 배치와 같은 정밀도의 근사치 — 스프링캠프만 시즌 경계를 넘는 소재라 이번 시즌 마지막 주차로 근사(다음 시즌 준비를 이번 시즌 끝자락에서 예고). 기존 5개 트리거만 사용, 새 엔진 작업 없음. 이벤트 총 46→**52개**. 38개 카탈로그 기준 28+6=**34개 완료**(University·독립·프로2군 확장 9개는 카탈로그 밖이라 분모엔 안 잡힘).

**테스트**: 신규 Rust/Dart 테스트 없음(순수 seed 데이터 추가). `dart run bin/tool.dart content seed` — `events=52` 커밋 성공, `content validate` FK 위반 0. `app/assets/content.db` 재동기화. `flutter test -j 1` 38개 파일 전부 통과. `balance_harness -- 5 3` — S등급 6.5%·평균 이벤트 발동 수 87.60건 전부 변화 없음(이 하네스 표본이 고교 3시즌 상한 안에서 끝나 프로 이벤트는 발동 기회 자체가 없었음, 정상), 크래시 없음.

**영향 없음**: 신규 엔진 코드 없음(콘텐츠 데이터만 추가), 새 테이블·마이그레이션 없음.

**§6-135 로드맵 진행 상황(당시)**: Phase 1~5 완료, Phase 6 남음.

### 6-140. 뉴스 Phase 6-1 — 내 팀·지역 순위 정기 요약 (2026-07-26, 완료)

**Context**: §6-135 로드맵 Phase 6을 착수하려는데, 원안(주간 다이제스트 + 전용 조회 화면 신설)을 사용자에게 다시 보여주니 "그냥 메세지로 주간 월간 지역 팀별 순위... 텍스트로만 해도 될 것 같다"는 B안 제시 — 전용 Flutter 화면 없이 메시지만으로 가는 쪽으로 확정. Phase 6을 6-1(내 팀 순위 정기 요약)·6-2(인접권역·먼 리그 다이제스트) 둘로 나눠 6-1부터 진행.

**구현(`data::repository.rs`)**: §6-122의 `process_protagonist_rank_updates`가 쓰던 "소속팀·리그 찾기 → `team_rank_context` 계산" 앞부분을 `load_protagonist_team_rank_context`로 뽑아 공유 헬퍼화(리팩터, 새 계산 추가 없음). `process_protagonist_periodic_rank_summary` 신규 — `check_and_notify_rank`처럼 "직전 값과 비교해서 바뀔 때만"이 아니라, 화면이 없으니 이게 유일한 확인 수단이라 **매달 무조건** "이번 달 팀 성적 — 지역 순위 N위 · 전국 순위 M위" 스냅샷 발송(`kind='rank_summary'`). 월간 훅(`process_protagonist_rank_updates` 바로 다음)에 배선.

**테스트**: 신규 1건 — `process_protagonist_periodic_rank_summary_sends_a_snapshot_every_time_even_without_a_change`(순위 변동이 전혀 없어도 매달 새 스냅샷이 뜨는지, `check_and_notify_rank`류와의 차이를 명시적으로 검증). 기존 순위 관련 테스트 13개 전부 리팩터 후에도 그대로 통과(공유 헬퍼 추출이 동작을 안 바꿨는지 확인). `cargo test --lib` 566개 전부 통과, `cargo clippy --lib --tests --bins` 클린.

**영향 없음**: 새 테이블·마이그레이션 없음(`inbox`/`season_meta` 재사용 안 함 — 매번 무조건이라 직전값 저장 자체가 불필요). 새 RNG 굴림 없음.

### 6-141. 뉴스 Phase 6-2 — 인접권역·먼 리그 주간 다이제스트 (2026-07-26, 완료 — §6-135 로드맵 전체 완료)

**Context**: §6-135 로드맵의 마지막 Phase. 05_뉴스_미디어.md §1의 "인접 권역 소식"(요약 수준)·"먼 리그 소식"(헤드라인만) 두 상세도를 그대로 실체화.

**구현(`data::repository.rs`)**: `process_weekly_league_digest` — "인접"을 실제 지리적 인접 그래프로 계산하는 건 스코프 과함(D그룹 placeholder로 남김) — 같은 리그의 "내 권역이 아닌 다른 권역"으로 근사(단일 그룹 리그는 권역 개념 자체가 없어 스킵). 팀 이름은 여전히 노출 안 함(§6-137 관례 유지) — 승률 숫자만으로 "저기 강한 팀이 있다" 정도의 정보량. 리그 5종의 한국어 표시명(`league_display_name`, `tournament_display_name`과 같은 관례)을 "먼 리그" 헤드라인에만 사용 — 리그는 5개 고정 세트라 팀 이름과 달리 작은 매핑으로 충분. §6-140과 달리 매주(`today % 7 == 0`) 무작위로 인접권역 하나·다른 리그 하나씩 골라 발송(다이제스트는 "새 소식이 있을 때만"이 아니라 "정기적으로 훑어주는" 성격이 맞다고 판단).

**테스트**: 신규 1건 — `process_weekly_league_digest_reports_an_adjacent_region_and_a_distant_league`(다른 권역이 있으면 인접권역 소식이 뜨는지, 여러 `day`를 훑어 "프로 리그" 헤드라인이 실제로 나오고 정확한 승률 숫자를 담는지 — 리그 선택이 무작위라 특정 시행 하나에 의존하지 않게 시드 반복). `cargo test --lib` 567개 전부 통과, `cargo clippy --lib --tests --bins` 클린. `flutter test -j 1`(엔진 전용 변경, Phase 6-1+6-2 누적) 38개 파일 전부 통과. `balance_harness -- 5 3`(Phase 6-1+6-2 누적 검증) — S등급 6.5%·평균 이벤트 발동 수 87.60건 전부 변화 없음, 크래시 없음.

**영향 없음**: 새 테이블·마이그레이션 없음(`inbox` 재사용). 다이제스트용 RNG는 `league_sub_seed(world_seed, "digest:{day}")`로 다른 시드 스트림과 분리 — 실제 경기 판정 RNG엔 안 섞임.

**§6-135 로드맵 진행 상황**: Phase 1(이벤트: 독립·프로2군, §6-135)·Phase 2(메시지: 마일스톤 확장+첫 승, §6-136)·Phase 3(뉴스 1부: 소속팀·라이벌, §6-137)·Phase 4(뉴스 2부: 동료 마일스톤, §6-138)·Phase 5(이벤트: 프로 캘린더 잔여분, §6-139)·Phase 6-1(내 팀 순위 정기 요약, §6-140)·Phase 6-2(인접권역·먼 리그 다이제스트, 이 항목) **전부 완료 — 로드맵 종료**. 남은 건 원래 스코프 밖으로 뒀던 것들: Phase 7(NPC 시장 소식, 배경 트레이드·FA 시뮬레이션 신설 필요)·Phase 8(데뷔전·대회우승 확정처럼 새 콜백 지점이 필요한 이벤트)·국대 발탁(병역 면제 연계 대형 시스템) — 전부 이번 스코프보다 훨씬 큰 별도 작업으로 다음 세션 후보.
