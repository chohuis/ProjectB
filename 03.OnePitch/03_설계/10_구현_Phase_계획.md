...
> 상태: **I0~I6 완료(I5는 8차분까지, `eval`·`market`은 I6로 재배치, I6는 8차분까지 — `sim/market` 전체 완료), I7 착수(2차분 완료 — 엔진 api 레이어+뉴게임/진행/매치 최소 화면+내 선수 허브)** — 2026-07-16
...
| **I7** | Flutter UI | `04_UI기획/` 00~08 화면 실제 구현(4허브+진행버튼+매치 CustomPainter) | 최소 플레이 가능한 루프가 실제 화면에서 끝까지 동작(뉴게임→진행→경기→시즌종료) | `04_UI기획/` 전체 | 🟨 **착수**(2차분 완료, 2026-07-16) — 1차분(엔진 `api::game` 레이어 + Riverpod/go_router + 뉴게임·진행/매치 최소 화면으로 "뉴게임→진행→경기→시즌 경계" 브리지 동작 확인)+2차분(4허브 첫 번째 — 내 선수 허브 상태/훈련 탭 실동작, 재정 탭은 엔진 미구현이라 placeholder). 남은 4허브 3개·전용화면(협상/트레이드/진로/드래프트/치료/은퇴)·매치 CustomPainter 비주얼·세이브 슬롯 영속성·반응형 레이아웃은 계속 이월. 상세는 아래 §6-21~§6-22 |
...
**다음 세션이 할 일**(I7 2차분 완료로 갱신, 아래 §6-22 참고): 사용자가 확정한 추천 순서대로 — ①남은 4허브 3개(리그·기록·메시지함) 중 다음 것(리그부터 추천), ②매치 CustomPainter 비주얼(다이아몬드·존그리드, 05_매치.md), ③`injuryTreatment` 전용 화면(전용화면 5종 중 지금 유일하게 실제 발동 가능), ④진로 갈림길 엔진 작업(고교→대학/독립/프로, 01_커리어_구조.md §5 — `contractNego`/`tradeDecision`/`draft`/`careerChoice`가 실전에서 의미를 가지려면 선행), ⑤그 4종 전용 화면, ⑥세이브 슬롯 영속성(`createSlot`/`loadSlot`), ⑦반응형 레이아웃, ⑧은퇴 화면. I6 계속 이월분(감독 개입·콜드게임 인터랙티브 경로·개인 통산 스탯·월/시즌 단위 평가)도 여전히 남아있음 — 이 순서와 독립적이라 아무 지점에 끼워 넣어도 무방. `build_runner`/Dart 3.10.3 호환 이슈(§6-22 "구현 중 발견한 도구 이슈")는 다음에 새 frb enum을 추가할 때 실제로 막힐 수 있으니 미리 인지.
