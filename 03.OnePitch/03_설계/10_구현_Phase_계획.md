...
> 상태: **I0~I6 완료(I5는 8차분까지, `eval`·`market`은 I6로 재배치, I6는 8차분까지 — `sim/market` 전체 완료), I7 착수(4차분 완료 — 엔진 api 레이어+뉴게임/진행/매치 최소 화면+내 선수/리그/기록 허브 3개)** — 2026-07-16
...
| **I7** | Flutter UI | `04_UI기획/` 00~08 화면 실제 구현(4허브+진행버튼+매치 CustomPainter) | 최소 플레이 가능한 루프가 실제 화면에서 끝까지 동작(뉴게임→진행→경기→시즌종료) | `04_UI기획/` 전체 | 🟨 **착수**(4차분 완료, 2026-07-16) — 1차분(엔진 `api::game` 레이어 + Riverpod/go_router + 뉴게임·진행/매치 최소 화면)+2차분(내 선수 허브)+3차분(리그 허브)+4차분(4허브 세 번째 — 기록 허브 7탭, 실데이터 3개(경기로그·계약이력·부상재활)+미구현 placeholder 4개(관계·수상기록·커리어·업적)). `메시지함`은 이벤트/알림 콘텐츠 시스템 자체가 엔진에 없어(I8 콘텐츠 저작 전제) 보류 — 남은 건 매치 CustomPainter 비주얼·전용화면(협상/트레이드/진로/드래프트/치료/은퇴)·세이브 슬롯 영속성·반응형 레이아웃·메시지함. 상세는 아래 §6-21~§6-24 |
...
**다음 세션이 할 일**(I7 4차분 완료로 갱신, 아래 §6-24 참고): 4허브는 사실상 마무리(메시지함은 I8 콘텐츠 저작 전제라 지금은 스킵) — 다음은 ①매치 CustomPainter 비주얼(다이아몬드·존그리드, 05_매치.md), ②`injuryTreatment` 전용 화면(전용화면 5종 중 지금 유일하게 실제 발동 가능), ③진로 갈림길 엔진 작업(고교→대학/독립/프로, 01_커리어_구조.md §5 — `contractNego`/`tradeDecision`/`draft`/`careerChoice`가 실전에서 의미를 가지려면 선행), ④그 4종 전용 화면, ⑤세이브 슬롯 영속성(`createSlot`/`loadSlot`), ⑥반응형 레이아웃, ⑦은퇴 화면, ⑧메시지함(I8 이후 재검토). I6 계속 이월분(감독 개입·콜드게임 인터랙티브 경로·개인 통산 스탯·월/시즌 단위 평가)도 여전히 남아있음 — 이 순서와 독립적이라 아무 지점에 끼워 넣어도 무방. `build_runner`/Dart 3.10.3 호환 이슈(§6-22)와 `testWidgets`+frb 비동기 조합 패턴(§6-23)은 다음 위젯 테스트 작성 시 재확인.
