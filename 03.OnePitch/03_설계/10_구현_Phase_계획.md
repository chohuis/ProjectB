...
> 상태: **I0·I1·I2·I3·I4 완료, I5 진행중(1~7차분 완료)** — 2026-07-15
...
| **I5** | 나머지 sim 모듈 | `sim/growth`·`sim/injury`·`sim/eval`·`sim/match`(배경)·`sim/market`·`sim/npc`·`sim/schedule` — [05_밸런스](05_밸런스.md) §3 순서(A→B→C→{D,E,F}→G,H→I)로 구현+가밸런스 적용 | 배경 시뮬만으로 시즌 1개 완주 가능 | [03_구조](03_구조.md) §5-1 · 육성코어 01~09 각 문서 | 🔶 진행중 — 1차분(`sim/schedule`+`sim/match`, 프로/2군/대학/고교 정규시즌)+2차분(독립리그 4단계 전체+독립/프로 포스트시즌)+3차분(대학 3개 대회·고교 5개 전국대회 포스트시즌)+4차분(`sim/growth` 상승기, 주간 XP→스탯)+5차분(`sim/injury` 누적형 부상+`sim/growth` 하락기/노쇠)+6차분(`sim/injury` 급성형 부상, 매치 엔진 확장)+7차분(`sim/npc` 세대교체 — retire+generate_freshmen) 완료(2026-07-15). **`sim/eval`·`sim/market`은 리서치 결과 대부분 주인공 전용으로 판명돼 I6로 이월**(§6-11 참고), `sim/npc` 병역(enlist/discharge)은 계속 진행 예정. 상세는 아래 §6-11 |
...
**다음 세션이 할 일**(7차분 완료로 갱신, 아래 §6-11 참고): `sim/npc` 병역(enlist/discharge) — 05_밸런스.md §3 "I 기타"로 분류된 항목, npc 생애주기 묶음이라 세대교체 다음으로 배치. 그 다음은 I6(주인공 플로우) — `sim/eval`·`sim/market`(대부분 주인공 전용으로 확인됨)이 자연스럽게 그때 같이 설계됨.
