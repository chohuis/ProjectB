...
fn generate_league_roster_data(content_conn: &Connection, world_seed: i64, league_id: &str) -> anyhow::Result<Vec<GeneratedTeamRoster>> {
    ...
    let coach_role_ctx = content::load_personality_rule(content_conn, "role:코치")?;
    let owner_role_ctx = content::load_personality_rule(content_conn, "role:구단주")?;
...
pub fn generate_schedule(
    ...
    // 프로/프로2군은 실제 KBO처럼 3연전+주간 휴식일 구조(대화 2026-07-24,
    // §6-N) — 팀이 한 그룹뿐이라 `groups`의 첫(유일한) 그룹을 그대로 쓴다.
    let entries = if league_id == "league:pro" || league_id == "league:pro_farm" {
        let teams = groups.into_iter().next().unwrap_or_default();
        schedule::generate_regular_season_series_based(league_slug, &teams, target_games, start_day, &mut rng)
    } else {
        schedule::generate_regular_season_targeted(league_slug, &groups, target_games, start_day, &mut rng)
    };
...
pub fn generate_initial_world(slot_conn: &mut Connection, content_conn: &Connection, canonical_seed: i64) -> anyhow::Result<()> {
    ...
    // 랭킹 배치 캐시(대화 2026-07-24) — 새 게임 시작 시점엔 season_stats·
    // practice_stats가 비어있어 사실상 공짜지만, 아래 세 루프가 이 캐시
    // 하나를 공유하는 패턴 자체는 월간/시즌 재편성과 동일하게 맞춘다.
    let scores = StatScoreCache::load(&tx)?;
    for_each_team_in_all_leagues(&tx, content_conn, |conn, team_id| assign_rotation(conn, content_conn, team_id, &scores))?;
    for_each_team_in_all_leagues(&tx, content_conn, |conn, team_id| assign_batting_order(conn, team_id, &scores))?;
    // 마무리 랭킹(10_구현_Phase_계획.md §6-N Part G) — 로테이션과 같은
    // 트랜잭션에 묶는 이유도 동일(개별 autocommit 방지).
    for_each_team_in_all_leagues(&tx, content_conn, |conn, team_id| assign_closer(conn, team_id, &scores))?;
    tx.commit()?;
    Ok(())
}
...
/// 보유 구종 상한(05_구종_시스템.md §3, 대화 2026-07-24) — 소수 정예
/// 레퍼토리를 깊게 마스터하는 방향 유도. 5개를 채우면 "신규 습득"은
/// 막히고 "기존 마스터리업"만 가능.
pub const MAX_KNOWN_PITCHES: usize = 5;

/// 구종 마스터리 항목(05_구종_시스템.md §2, 대화 2026-07-23) — 새로 습득한
/// 구종은 항상 1단계("습작")·진행도 0에서 시작. `protagonist.pitches`
/// 전용(NPC의 `npc.pitches`는 여전히 이름 문자열 배열 — NPC는 주간 훈련
/// 슬롯 개념이 없어 마스터리 추적 대상이 아님).
fn new_pitch_mastery_entry(name: &str) -> serde_json::Value {
    serde_json::json!({"name": name, "stage": 1, "weeks": 0})
}
...
pub fn create_protagonist(
    ...
    let mut pitch_names = vec!["포심 패스트볼".to_string()];
    if let Some(second) = second_pitch {
        ...
        pitch_names.push(second.to_string());
        ...
        if let Some(auto) = crate::sim::protagonist::second_pitch_candidates(&mut rng, archetype)?.into_iter().next() {
            pitch_names.push(auto);
        }
    }
    // 구종 마스터리(05_구종_시스템.md §2, 대화 2026-07-23) — 시작 구종은
    // 전부 1단계("습작")에서 출발.
    let pitches: Vec<serde_json::Value> = pitch_names.iter().map(|n| new_pitch_mastery_entry(n)).collect();
...
pub fn set_protagonist_training(
    ...
    if let Some(pitch) = new_pitch {
        if known_names.iter().any(|p| p == pitch) {
    ...
    if let Some(pitch) = mastery_pitch {
        let entry = pitches.iter().find(|p| p.get("name").and_then(|n| n.as_str()) == Some(pitch));
        match entry {
            None => anyhow::bail!("{pitch} is not known yet — mastery_pitch is for refining a pitch already in the repertoire"),
            Some(e) if e.get("stage").and_then(|s| s.as_u64()) == Some(5) => {
                anyhow::bail!("{pitch} is already at max mastery (필살기) — nothing left to refine")
            }
            Some(_) => {}
        }
    }

    let active_target = new_pitch.or(mastery_pitch);
    ...
    let pitch_weeks = existing_training
        ...
        .filter(|v| {
            let existing_target =
                v.get("new_pitch").and_then(|p| p.as_str()).or_else(|| v.get("mastery_pitch").and_then(|p| p.as_str()));
            existing_target == active_target
        })
...
fn process_protagonist_week(slot_conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64) -> anyhow::Result<()> {
    ...
    // 코치 보너스(이월 부채 정리, 대화 2026-07-22) — 무소속(입대 등)이거나
    ...
    match crate::sim::injury::check_overuse_injury(&mut injury_rng, effective_fatigue, &philosophy, already_warned) {
    ...
    crate::sim::training::apply_weekly_training(&mut rng, &crate::sim::growth::PITCHER_EXPOSED, &mut stats, &mut xp, effective_genius, &config);
    ...
    if let Some(pitch) = &new_pitch {
        ...
        if weeks >= required {
            ...
            if !pitches.iter().any(|p| p.get("name").and_then(|n| n.as_str()) == Some(pitch.as_str())) {
                pitches.push(new_pitch_mastery_entry(pitch));
            }
    ...
    } else if let Some(pitch) = &mastery_pitch {
        // 기존 구종 마스터리업(05_구종_시스템.md §2, 대화 2026-07-23) —
        // "신규 습득"과 같은 진행도 필드(`pitch_weeks`)를 공유하되, 완료
        ...
        if weeks >= required {
            let mut pitches: Vec<serde_json::Value> = serde_json::from_str(&pitches_raw)?;
            if let Some(entry) = pitches.iter_mut().find(|p| p.get("name").and_then(|n| n.as_str()) == Some(pitch.as_str())) {
                let stage = entry.get("stage").and_then(|s| s.as_u64()).unwrap_or(1);
                entry["stage"] = serde_json::json!((stage + 1).min(5));
                entry["weeks"] = serde_json::json!(0);
            }
            slot_conn.execute("UPDATE protagonist SET pitches = ?1 WHERE id = 'proto:1'", params![serde_json::json!(pitches).to_string()])?;
            training["mastery_pitch"] = serde_json::Value::Null;
            training["pitch_weeks"] = serde_json::json!(0);
        } else {
            training["pitch_weeks"] = serde_json::json!(weeks);
        }
...
pub fn generate_freshmen(conn: &Connection, content_conn: &Connection, world_seed: i64, season: i64) -> anyhow::Result<()> {
    ...
    for league_id in LEAGUE_IDS {
        ...
        for team in &teams {
            ...
            // 스카우팅안목 코치 보너스(이월 부채 정리, 대화 2026-07-22) —
...
/// 시즌 시작 시 팀별 선발 로테이션 확정(04_프로_커리어.md §22, 감독 AI
/// 능력치 서열화, 이월 부채 정리 대화 2026-07-22) — `season_meta`에
/// `rotation:{team_id}` 키로 순서(JSON 배열)를 저장한다. 새 테이블 없이
...
/// `table`은 "season_stats"/"practice_stats" 중 하나(하드코딩 호출부만,
/// 외부 입력 아님) — `upsert_stats_fields`와 같은 이유로 테이블만 다르게
/// 받는다. `era_like`는 Phase 2부터 `runs_allowed - unearned_runs`(자책점)
/// 만 본다(§12 "ERA 등 투수 기록의 정확성에 필수") — 로테이션·마무리
/// 랭킹이 실책으로 내준 비자책점 때문에 억울하게 깎이지 않게.
fn pitcher_stats_score(conn: &Connection, table: &str, player_id: &str) -> anyhow::Result<Option<f64>> {
    ...
    Ok(pitcher_stats_score_from_lines(&lines))
}

/// `pitcher_stats_score`의 순수 계산 부분(쿼리 없이 이미 불러온 라인만
/// 처리) — 단일 선수 조회(`pitcher_stats_score`)와 172팀 일괄 배치
/// (`StatScoreCache::load`) 양쪽이 공유해, 월간 재편성처럼 팀마다
/// 반복 조회하면 N+1이 되는 자리에서 한 번의 전체 스캔으로 대체할 수
/// 있게 한다(대화 2026-07-24, 실측 진단으로 발견 — 로테이션/타순/마무리
/// 랭킹이 매달 172팀 전체를 순회하며 선수 개개인마다 별도 쿼리를 날려
/// 월간 재편성 구간이 예상보다 훨씬 비쌌음).
fn pitcher_stats_score_from_lines(lines: &[String]) -> Option<f64> {
    let (mut outs, mut runs, mut unearned_runs, mut ks) = (0i64, 0i64, 0i64, 0i64);
    for line in lines {
        ...
        unearned_runs += v.get("unearned_runs").and_then(|x| x.as_i64()).unwrap_or(0);
        ks += v.get("strikeouts").and_then(|x| x.as_i64()).unwrap_or(0);
    }
    if outs < 9 {
        return None;
    }
    let earned_runs = (runs - unearned_runs).max(0);
    let innings = outs as f64 / 3.0;
    let era_like = earned_runs as f64 * 9.0 / innings;
    let k_per_9 = ks as f64 * 9.0 / innings;
    Some((50.0 - (era_like - 4.5) * 4.0 + (k_per_9 - 7.0)).clamp(0.0, 100.0))
}

fn season_stats_score(conn: &Connection, player_id: &str) -> anyhow::Result<Option<f64>> {
    pitcher_stats_score(conn, "season_stats", player_id)
}

/// 청백전 누적으로 뽑은 같은 점수 — `season_stats_score`와 계산식은
/// 동일, 표본만 `practice_stats`(비공식)에서 가져온다.
fn practice_stats_score(conn: &Connection, player_id: &str) -> anyhow::Result<Option<f64>> {
...
/// 월간/시즌 재편성 배치 최적화(대화 2026-07-24, 실측 진단으로 발견) —
/// `ranked_rotation_candidates_for_team`/`ranked_batting_order_for_team`/
/// `ranked_closer_candidates_for_team`는 원래 후보 선수마다 `season_stats`·
/// `practice_stats`를 개별 쿼리했는데, 이 세 함수가 매달 172팀 전체를
/// 순회하며 호출되다 보니(`for_each_team_in_all_leagues`) N+1 쿼리가
/// 쌓여 월간 재편성 구간이 예상보다 훨씬 비쌌다(전체 `advance()` 비용의
/// ~22%). 두 테이블을 통째로 한 번씩만 읽어(`load_all_lines_by_player`)
/// 투수·타자 점수를 전부 미리 계산해두고, 팀별 루프는 이 캐시에서
/// `HashMap::get`만 하도록 바꿨다 — 쿼리 수가 (팀 수×선수 수×2)에서
/// 고정 2회(season_stats·practice_stats 테이블 각 1번 풀스캔)로 준다.
fn load_all_lines_by_player(conn: &Connection, table: &str) -> anyhow::Result<HashMap<String, Vec<String>>> {
    let mut stmt = conn.prepare(&format!("SELECT player_id, line FROM {table}"))?;
    let rows: Vec<(String, String)> = stmt.query_map([], |r| Ok((r.get(0)?, r.get(1)?)))?.collect::<Result<_, _>>()?;
    let mut map: HashMap<String, Vec<String>> = HashMap::new();
    for (player_id, line) in rows {
        map.entry(player_id).or_default().push(line);
    }
    Ok(map)
}

struct StatScoreCache {
    pitcher_season: HashMap<String, f64>,
    pitcher_practice: HashMap<String, f64>,
    batter_season: HashMap<String, f64>,
    batter_practice: HashMap<String, f64>,
}

impl StatScoreCache {
    /// 호출 시점의 `season_stats`/`practice_stats`를 통째로 한 번씩만
    /// 읽어 투수·타자 점수를 전부 미리 계산 — 월간 재편성 훅(`advance()`)·
    /// 시즌 경계(`season_rollover`) 진입 시 딱 한 번만 만들어 그 안의
    /// `for_each_team_in_all_leagues` 루프 3개(로테이션·타순·마무리)가
    /// 공유한다.
    fn load(conn: &Connection) -> anyhow::Result<Self> {
        let season_lines = load_all_lines_by_player(conn, "season_stats")?;
        let practice_lines = load_all_lines_by_player(conn, "practice_stats")?;
        let score_map = |lines_by_player: &HashMap<String, Vec<String>>, f: fn(&[String]) -> Option<f64>| -> HashMap<String, f64> {
            lines_by_player.iter().filter_map(|(id, lines)| f(lines).map(|s| (id.clone(), s))).collect()
        };
        Ok(Self {
            pitcher_season: score_map(&season_lines, pitcher_stats_score_from_lines),
            pitcher_practice: score_map(&practice_lines, pitcher_stats_score_from_lines),
            batter_season: score_map(&season_lines, batter_stats_score_from_lines),
            batter_practice: score_map(&practice_lines, batter_stats_score_from_lines),
        })
    }
}
...
fn ranked_rotation_candidates_for_team(
    conn: &Connection,
    content_conn: &Connection,
    team_id: &str,
    scores: &StatScoreCache,
    ...
    for (id, stats_raw) in rows {
        ...
        let season_score = scores.pitcher_season.get(&id).copied();
        let practice_score = scores.pitcher_practice.get(&id).copied();
    ...
    if matches!(league_id.as_deref(), Some("league:hs") | Some("league:univ")) {
        ...
        if let Some(contract_raw) = contract_raw {
            ...
            if contract.get("team_id").and_then(|v| v.as_str()) == Some(team_id) {
                ...
                let practice_score = scores.pitcher_practice.get("proto:1").copied();
...
fn assign_rotation(conn: &Connection, content_conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<()> {
    let rotation = ranked_rotation_candidates_for_team(conn, content_conn, team_id, scores)?;
    if rotation.is_empty() {
        return Ok(());
    }
    conn.execute(
...
fn reassign_rotation_preserving_next_turn(conn: &Connection, content_conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<()> {
    let new_ranked = ranked_rotation_candidates_for_team(conn, content_conn, team_id, scores)?;
...
fn batter_stats_score(conn: &Connection, table: &str, player_id: &str) -> anyhow::Result<Option<f64>> {
    let mut stmt = conn.prepare(&format!("SELECT line FROM {table} WHERE player_id = ?1"))?;
    let lines: Vec<String> = stmt.query_map([player_id], |r| r.get(0))?.collect::<Result<Vec<_>, _>>()?;
    Ok(batter_stats_score_from_lines(&lines))
}

/// `batter_stats_score`의 순수 계산 부분 — `pitcher_stats_score_from_lines`
/// 와 같은 이유로 분리(배치 스캔 재사용).
fn batter_stats_score_from_lines(lines: &[String]) -> Option<f64> {
    ...
    Some((50.0 + (obp_like - 33.0) + (slg_like - 40.0) * 0.5).clamp(0.0, 100.0))
}

fn season_stats_batter_score(conn: &Connection, player_id: &str) -> anyhow::Result<Option<f64>> {
    batter_stats_score(conn, "season_stats", player_id)
}
...
/// 실제 타순 후보와 일치시킨다. 타순은 로테이션과 달리 정원 제한이 없어
/// (로스터 타자 전원 배치) `manager::rank_all_candidates`를 쓴다.
fn ranked_batting_order_for_team(conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<Vec<String>> {
    let mut stmt = conn.prepare(
    ...
    for (id, stats_raw) in rows {
        ...
        let season_score = scores.batter_season.get(&id).copied();
        let practice_score = scores.batter_practice.get(&id).copied();
        candidates.push((id, manager::blend_rotation_score_with_practice(skill_score, season_score, practice_score)));
    }
...
/// 이 함수 하나만 쓴다.
fn assign_batting_order(conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<()> {
    let order = ranked_batting_order_for_team(conn, team_id, scores)?;
    if order.is_empty() {
        return Ok(());
    }
    conn.execute(
...
pub(crate) fn load_starting_pitcher(slot_conn: &Connection, team_id: &str) -> anyhow::Result<match_sim::PitcherStats> {
    ...
    let rotation_pick: Option<String> = rotation_raw.and_then(|raw| {
        let rotation: Vec<String> = serde_json::from_str(&raw).unwrap_or_default();
        if rotation.is_empty() {
            return None;
        }
        let played: i64 = slot_conn
            .query_row("SELECT count(*) FROM schedule WHERE (home = ?1 OR away = ?1) AND result IS NOT NULL", [team_id], |r| r.get(0))
            .unwrap_or(0);
...
fn ranked_closer_candidates_for_team(conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<Vec<String>> {
    let mut stmt = conn.prepare(
...
fn assign_closer(conn: &Connection, team_id: &str, scores: &StatScoreCache) -> anyhow::Result<()> {
    let ranked = ranked_closer_candidates_for_team(conn, team_id, scores)?;
...
/// 코치 스탯 7종(`sim::staff` 보정 함수들에 그대로 넣는 용도) — 감독과
/// 달리 해시 폴백이 없다: 코치 없는 세이브(구버전·합성 테스트)는 보너스
/// 0(중립)이 자연스러워 `None`을 그대로 반환, 호출부가 "코치 없으면
/// 효과 없음"으로 처리한다.
pub(crate) struct CoachStats {
    pub pitching: f64,
    pub batting: f64,
    pub running: f64,
...
/// 구단주 스탯 3종 — `load_coach_stats`와 같은 관례(없으면 `None`).
...
pub(crate) fn load_owner_stats(conn: &Connection, team_id: &str) -> anyhow::Result<Option<OwnerStats>> {
    let stats_raw: Option<String> =
        conn.query_row("SELECT stats FROM npc WHERE id = ?1", [format!("owner:{team_id}")], |r| r.get(0)).optional()?;
    let Some(stats_raw) = stats_raw else {
        return Ok(None);
    };
    let v: serde_json::Value = serde_json::from_str(&stats_raw)?;
    let get = |key: &str| v.get(key).and_then(|x| x.as_f64()).unwrap_or(50.0);
    Ok(Some(OwnerStats { wealth: get("재력"), aggressiveness: get("투자성향"), patience: get("인내심") }))
}
...
/// `table`은 하드코딩된 두 값("season_stats"/"practice_stats") 중 하나만
/// 호출부가 넘긴다(외부 입력 아님, SQL 인젝션 우려 없음) — 공식 기록과
/// 청백전(비공식) 기록이 완전히 같은 누적·머지 로직을 쓰되 서로 안
/// 섞이게 테이블만 다르게 하려고 이렇게 뺐다(대화 2026-07-26).
fn upsert_stats_fields(conn: &Connection, table: &str, player_id: &str, week: i64, fields: &[(&str, u32)]) -> anyhow::Result<()> {
    let existing: Option<String> = conn
        .query_row(&format!("SELECT line FROM {table} WHERE player_id = ?1 AND week = ?2"), params![player_id, week], |r| r.get(0))
        .optional()?;
    ...
    conn.execute(
        &format!("INSERT INTO {table} (player_id, week, line) VALUES (?1, ?2, ?3) ON CONFLICT(player_id, week) DO UPDATE SET line = excluded.line"),
...
pub(crate) fn upsert_pitcher_season_stats(conn: &Connection, player_id: &str, week: i64, s: &match_sim::PitcherGameStats) -> anyhow::Result<()> {
    upsert_stats_fields(conn, "season_stats", player_id, week, &pitcher_stats_fields(s))
}
...
fn upsert_pitcher_practice_stats(conn: &Connection, player_id: &str, week: i64, s: &match_sim::PitcherGameStats) -> anyhow::Result<()> {
    upsert_stats_fields(conn, "practice_stats", player_id, week, &pitcher_stats_fields(s))
}
...
/// 정렬된 목록을 번갈아 두 편으로 나눈다(0,2,4...→A / 1,3,5...→B) — 이미
/// 능력치 내림차순으로 정렬해뒀다는 전제하에 강한 선수가 양쪽에 고르게
/// 갈린다. 새 RNG 불필요(대화 2026-07-26, 청백전은 "그때그때 다른 팀
/// 구성"보다 "균형"이 우선).
fn split_alternating<T>(items: Vec<T>) -> (Vec<T>, Vec<T>) {
    let mut a = Vec::new();
    let mut b = Vec::new();
...
/// 청백전(연습경기) — `02_고교.md` §4-5 "팀 내부 청군vs백군 스크리미지...
/// 비공식 기록(개인 통산 스탯 미반영), 단 포지션 뎁스차트·코치 평가에는
/// 반영"을 그대로: 팀 로스터를 능력치 기준으로 균형 있게 청/백 두 진영에
/// 나눠 `match_sim::simulate_game`(배경 경기와 같은 엔진)으로 시뮬레이션
/// 하고, 결과를 `season_stats`/`standings`가 아니라 `practice_stats`에만
...
fn run_intrasquad_scrimmage(slot_conn: &Connection, content_conn: &Connection, world_seed: i64, team_id: &str, day: i64) -> anyhow::Result<()> {
    ...
    let contract_raw: Option<String> = slot_conn.query_row("SELECT contract FROM protagonist WHERE id = 'proto:1'", [], |r| r.get(0)).optional()?;
    if let Some(contract_raw) = contract_raw {
        let contract: serde_json::Value = serde_json::from_str(&contract_raw).unwrap_or_default();
        if contract.get("team_id").and_then(|v| v.as_str()) == Some(team_id) {
    ...
    // 양 진영 다 투수·타자가 최소 1명씩은 있어야 경기가 성립 — 로스터가
    // 아주 작은 합성 테스트 등에서는 조용히 건너뛴다.
    if pitchers.len() < 2 || batters.len() < 2 {
        return Ok(());
    }

    pitchers.sort_by(|a, b| (b.control + b.stuff).partial_cmp(&(a.control + a.stuff)).unwrap_or(std::cmp::Ordering::Equal));
    batters.sort_by(|a, b| (b.contact + b.eye + b.power).partial_cmp(&(a.contact + a.eye + a.power)).unwrap_or(std::cmp::Ordering::Equal));
    let (blue_pitchers, white_pitchers) = split_alternating(pitchers);
    let (blue_batters, white_batters) = split_alternating(batters);
    if blue_pitchers.is_empty() || white_pitchers.is_empty() || blue_batters.is_empty() || white_batters.is_empty() {
        return Ok(());
    }

    let league_id: String = content_conn.query_row("SELECT league_id FROM teams WHERE id = ?1", [team_id], |r| r.get(0))?;
    let manager = load_manager_stats(slot_conn, team_id)?;
    ...
    upsert_pitcher_practice_stats(slot_conn, &white_starter.id, week, &result.away_pitcher_stats)?;
    for (batter_id, s) in &result.home_batter_stats {
...
/// 월간 훅) — `for_each_team_in_all_leagues`는 5개 리그 전부 돌고
/// 클로저가 `(conn, team_id)` 2개 인자만 받아 `world_seed`/`day`를 못
/// 실어서 이 얇은 전용 루프를 따로 둔다.
fn run_monthly_scrimmages(slot_conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64) -> anyhow::Result<()> {
    for league_id in ["league:hs", "league:univ"] {
        for team_id in content::load_team_ids_for_league(content_conn, league_id)? {
            run_intrasquad_scrimmage(slot_conn, content_conn, world_seed, &team_id, day)?;
        }
    }
    Ok(())
}
...
/// 통산 기록 마일스톤 근접 알림(대화 2026-07-26, §6-N) — 03_메시지_알림.md
/// §1 "개인기록근접"(원래 카탈로그, 여태 미구현이던 마지막 항목 — 부상
/// 전조경고는 `injury_warning`, 방출·FA·재계약은 `contractNego`로 이미
/// 커버됨). 처음엔 통산 승수만 구현했다가(§6-134) 이번에 탈삼진·이닝으로
/// 확장 — `CareerStat`이 세 종류를 한 함수로 묶는다. `ach:career_100_wins`
/// (§6-36)가 쓰던 "career_history 합산 + 이번 시즌 진행분"과 같은 계산을
/// 재사용하되, 그건 "달성" 시점(시즌 종료)에만 확인하는 반면 이건
/// "근접"이라 `process_protagonist_rank_updates`와 같은 월간 훅에서 매달
/// 확인 — 그래야 시즌 도중에도 "이제 곧이다"라는 긴장감이 생긴다. 한
/// 마일스톤당 딱 한 번만 알리도록 `season_meta`에 마지막으로 알린
/// 마일스톤 값을 저장(순위 알림의 "직전 값" 패턴과 동일 재사용, 새
/// 테이블 없음). 세 종류 다 `kind='stat_milestone'`로 공유 — 순위 알림
/// (§6-122)이 4가지 순위를 `kind='rank_update'` 하나로 묶은 것과 같은 관례.
...
impl CareerStat {
    const ALL: [CareerStat; 3] = [CareerStat::Wins, CareerStat::Strikeouts, CareerStat::InningsPitched];

    fn json_key(self) -> &'static str {
        match self {
            CareerStat::Wins => "wins",
            CareerStat::Strikeouts => "strikeouts",
            CareerStat::InningsPitched => "innings_pitched",
        }
    }

    fn meta_key(self) -> &'static str {
        match self {
            CareerStat::Wins => "stat_milestone_notified:wins",
            CareerStat::Strikeouts => "stat_milestone_notified:strikeouts",
            CareerStat::InningsPitched => "stat_milestone_notified:innings_pitched",
        }
    }
    ...
    fn label(self) -> &'static str {
        match self {
            CareerStat::Wins => "승",
            CareerStat::Strikeouts => "탈삼진",
            CareerStat::InningsPitched => "이닝",
        }
    }

    /// 단위(step)·근접 판정폭(lookahead) — 승수는 §6-134에서 확정한
    /// 10/3. 탈삼진·이닝은 시즌당 누적 속도가 승수보다 훨씬 빨라(한
    /// 경기에 여러 개씩 쌓임) 같은 비율(약 30%)로 확대한 50/10.
    fn step(self) -> i64 {
        match self {
            CareerStat::Wins => 10,
            CareerStat::Strikeouts | CareerStat::InningsPitched => 50,
        }
    }
    ...
    fn lookahead(self) -> i64 {
        match self {
            CareerStat::Wins => 3,
            CareerStat::Strikeouts | CareerStat::InningsPitched => 10,
        }
    }

    fn value_in(self, line: &CareerLine) -> i64 {
        match self {
            CareerStat::Wins => line.wins,
            CareerStat::Strikeouts => line.strikeouts,
            CareerStat::InningsPitched => line.innings_pitched,
        }
    }
}

fn total_protagonist_stat_to_date(conn: &Connection, current_season: i64, stat: CareerStat) -> anyhow::Result<i64> {
    ...
    let past: i64 = lines
        ...
        .filter_map(|v| v.get(stat.json_key()).and_then(|w| w.as_i64()))
        .sum();
    let current = stat.value_in(&aggregate_game_log(conn, Some(current_season))?);
    Ok(past + current)
}

fn process_protagonist_stat_milestone(slot_conn: &Connection, day: i64, current_season: i64, stat: CareerStat) -> anyhow::Result<()> {
    let total = total_protagonist_stat_to_date(slot_conn, current_season, stat)?;
    let step = stat.step();
    let next_milestone = ((total / step) + 1) * step;
    let remaining = next_milestone - total;
    if remaining > stat.lookahead() {
        return Ok(());
    }

    let already_notified: Option<i64> = slot_conn
        .query_row("SELECT value FROM season_meta WHERE key = ?1", [stat.meta_key()], |r| r.get::<_, String>(0))
        .optional()?
        .and_then(|s| s.parse().ok());
    ...
    let body = format!("통산 {remaining}{label}만 더 채우면 {next_milestone}{label} 고지다.");
    slot_conn.execute(
        "INSERT INTO inbox (id, kind, urgency, read, day, body) VALUES (?1, 'stat_milestone', 'normal', 0, ?2, ?3)",
        params![format!("inbox:stat_milestone:{}:{next_milestone}", stat.json_key()), day, body],
    )?;
    slot_conn.execute(
        "INSERT INTO season_meta (key, value) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value = excluded.value",
        params![stat.meta_key(), next_milestone.to_string()],
    )?;
    Ok(())
}

fn process_protagonist_career_milestones(slot_conn: &Connection, day: i64) -> anyhow::Result<()> {
    ...
    for stat in CareerStat::ALL {
        process_protagonist_stat_milestone(slot_conn, day, current_season, stat)?;
    }
    Ok(())
}
...
fn process_week(conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64) -> anyhow::Result<()> {
    ...
    let coach_by_team = load_coach_stats_by_team(conn)?;

    let mut stmt = conn
        .prepare("SELECT id, team_id, position, age, stats, xp, live_state, injury, military_return_day FROM npc WHERE retired = 0 AND position NOT IN ('감독', '코치', '구단주')")?;
    ...
    for WeeklyNpcRow { id, team_id, position, age, stats_raw, xp_raw, live_state_raw, injury_raw, mut military_return_day } in rows {
        ...
        if military_return_day.is_some() {
            ...
            // 코치 보너스(이월 부채 정리, 대화 2026-07-22) — 투수면 투수지도력,
            // 타자면 타격지도력을 전문 능력치로, "스피드" 스탯만 주루지도력을
            ...
            crate::sim::growth::apply_weekly_growth_with_focus(&mut growth_rng, exposed, &mut stats, &mut xp, genius, |stat| {
                crate::sim::staff::coach_growth_focus(stat, specialized, general, running)
            });
        ...
        if let crate::sim::injury::OveruseOutcome::Injury(part, severity) =
            crate::sim::injury::check_overuse_injury(&mut injury_rng, effective_fatigue, &philosophy, true)
        {
            record_injury(&mut injury, part, severity, day);
        }
...
fn process_month(conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64) -> anyhow::Result<()> {
    ...
    let mut live_state: serde_json::Map<String, serde_json::Value> = serde_json::from_str(&live_state_raw)?;
    let morale = live_state.get("사기").and_then(|v| v.as_f64()).unwrap_or(50.0);
    let mut adjustment = crate::sim::eval::monthly_morale_adjustment(avg_grade_score);
    // 멘탈코칭 코치 보너스 — 경기별 보정(match_session.rs)과 같은 원칙,
    // 나쁜 페이스로 인한 하락폭만 완화.
    if adjustment < 0.0 {
        let contract_raw: Option<String> = conn.query_row("SELECT contract FROM protagonist WHERE id = 'proto:1'", [], |r| r.get(0)).optional()?;
        let team_id = contract_raw
            .and_then(|raw| serde_json::from_str::<serde_json::Value>(&raw).ok())
    ...
    process_protagonist_promotion(conn, content_conn, world_seed, day, avg_grade_score)?;
    Ok(())
}

/// 콜업(2군→1군)·강등(1군→2군) — §6-65, `04_프로_커리어.md` Phase3
/// "승강(1군↔2군)". "1군 로스터 공백 시점" 같은 정교한 트리거 대신 이미
/// 있는 이달의 페이스(`monthly_avg_grade_score`, 위 `process_month`가
/// 계산해둔 값 재사용)로 단순화 — NPC 로테이션 경쟁·역할배정은 감독 AI가
/// 없어 이번 스코프 밖(주인공 본인 경로만). 1군-2군 팀 페어링은 새 스키마
/// 없이 `data/seed/teams.csv`의 `{1군팀id}_farm` 명명 규칙을 그대로
/// 이용한다.
const PROMOTION_GRADE_THRESHOLD: f64 = 3.5; // B 이상 페이스
const DEMOTION_GRADE_THRESHOLD: f64 = 1.5; // D 이하 페이스
const PROMOTION_BASE_PROB: f64 = 0.3;
const DEMOTION_BASE_PROB: f64 = 0.3;

fn process_protagonist_promotion(conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64, avg_grade_score: f64) -> anyhow::Result<()> {
    let contract_raw: Option<String> = conn.query_row("SELECT contract FROM protagonist WHERE id = 'proto:1'", [], |r| r.get(0)).optional()?;
    let Some(contract_raw) = contract_raw else {
        return Ok(());
    };
    ...
    let (new_team_id, kind, body): (String, &str, &str) = match league_id.as_deref() {
        Some("league:pro_farm") if avg_grade_score >= PROMOTION_GRADE_THRESHOLD => {
            // 감독 신뢰도(관계도, §6-59)가 높을수록 콜업이 살짝 더 잘 된다 —
            // "복수 후보 겹치면 감독신뢰도 우선"이라는 설계를 단순화 반영.
            let relationship = relationship_value(conn, &format!("manager:{team_id}"))? as f64;
            let prob = (PROMOTION_BASE_PROB + relationship * 0.002).clamp(0.05, 0.9);
            let mut rng = ChaCha8Rng::seed_from_u64(league_sub_seed(world_seed, &format!("promotion:{day}")));
            if !rng.gen_bool(prob) {
                return Ok(());
            }
            let Some(pro_team_id) = team_id.strip_suffix("_farm") else {
                return Ok(()); // 명명 규칙을 벗어난 팀(합성 테스트 등) — 방어적 무시
            };
            (pro_team_id.to_string(), "callup", "좋은 페이스를 인정받아 1군으로 콜업됐다!")
        }
        Some("league:pro") if avg_grade_score <= DEMOTION_GRADE_THRESHOLD => {
            let mut rng = ChaCha8Rng::seed_from_u64(league_sub_seed(world_seed, &format!("demotion:{day}")));
            if !rng.gen_bool(DEMOTION_BASE_PROB) {
                return Ok(());
            }
            (format!("{team_id}_farm"), "demotion", "최근 부진으로 2군으로 조정 배치됐다.")
        }
        _ => return Ok(()),
    };

    contract["team_id"] = serde_json::json!(new_team_id);
    ...
    conn.execute(
        ...
        params![format!("inbox:{kind}:{day}"), kind, day, body],
    )?;
    let season = current_season_value(conn)?;
    log_career_event(conn, day, season, kind, serde_json::json!({"team_id": new_team_id}))?;
    Ok(())
}
...
fn process_protagonist_contract(conn: &Connection, content_conn: &Connection, world_seed: i64, season: i64, day: i64) -> anyhow::Result<()> {
    ...
    // 구단주 재력·인내심(이월 부채 정리, 대화 2026-07-22) — 코치와 같은
    // 관례로 없으면(구버전 세이브 등) 무보정(1.0).
    let owner = load_owner_stats(conn, &team_id)?;
    let patience_mult = owner.as_ref().map(|o| crate::sim::staff::owner_patience_factor(o.patience)).unwrap_or(1.0);

    if crate::sim::market::is_released(&mut rng, avg_grade_score, salary, &resource, patience_mult) {
    ...
    if years_remaining <= 0 {
        let offer_mult = owner.as_ref().map(|o| crate::sim::staff::owner_offer_multiplier(o.wealth, o.aggressiveness)).unwrap_or(1.0);
        let offer_salary = crate::sim::market::initial_offer(salary, avg_grade_score, attention, &resource, offer_mult);
...
fn process_protagonist_trade(conn: &Connection, content_conn: &Connection, world_seed: i64, day: i64) -> anyhow::Result<()> {
    ...
    let counterpart = if is_cash {
        ...
        let mut stmt = conn.prepare("SELECT id FROM npc WHERE team_id = ?1 AND retired = 0 AND position NOT IN ('감독', '코치', '구단주')")?;
...
fn process_protagonist_career_path(conn: &Connection, content_conn: &Connection, world_seed: i64, season: i64, day: i64) -> anyhow::Result<()> {
    ...
    if crate::sim::career::is_drafted(&mut rng, avg_grade_score, attention) {
        // 2군 배치가 기본(§6-65, 04_프로_커리어.md Phase2 "지명 → 2군 배치")
        // — "특급 신인"만 예외적으로 1군 직행.
        let target_league = if crate::sim::career::is_elite_prospect(avg_grade_score, attention) { "league:pro" } else { "league:pro_farm" };
        let Some(drafted_team) = pick_random_team_in_league(content_conn, &mut rng, target_league)? else {
            return Ok(());
        };
...
pub fn season_rollover(conn: &Connection, content_conn: &Connection, day: i64) -> anyhow::Result<()> {
    ...
    // 감독·코치·구단주(§6-59, 이월 부채 정리)는 나이·은퇴·병역 생애주기
    // 대상이 아니라 이 세대교체 파이프라인 전체에서 제외한다 —
    // position NOT IN ('감독', '코치', '구단주') 가드.
    conn.execute("UPDATE npc SET age = age + 1 WHERE retired = 0 AND position NOT IN ('감독', '코치', '구단주')", [])?;
    // 복무 중(military_return_day가 NULL이 아님)인 선수는 은퇴·입대 판정
    // 둘 다 대상에서 뺀다 — 이미 다른 생애주기 이벤트가 진행 중.
    let mut stmt =
        conn.prepare("SELECT id, team_id, age, military_served FROM npc WHERE retired = 0 AND military_return_day IS NULL AND position NOT IN ('감독', '코치', '구단주')")?;
    ...
    // 랭킹 배치 캐시(대화 2026-07-24, 실측 진단으로 발견) — 아래 세 루프가
    // 172팀을 순회하며 선수마다 season_stats/practice_stats를 개별 쿼리하던
    // N+1 패턴을, 이 시점의 두 테이블을 한 번씩만 통째로 읽어 만드는
    // 캐시로 교체 — 세 루프가 이 캐시 하나를 공유한다.
    let scores = StatScoreCache::load(conn)?;
    for_each_team_in_all_leagues(conn, content_conn, |c, team_id| assign_rotation(c, content_conn, team_id, &scores))?;
    for_each_team_in_all_leagues(conn, content_conn, |c, team_id| assign_batting_order(c, team_id, &scores))?;
    // 마무리 랭킹도 시즌 경계마다 재계산(10_구현_Phase_계획.md §6-N Part G) —
    // 방금 끝난 시즌의 season_stats가 아직 남아있는 지금이라야 반영된다.
    for_each_team_in_all_leagues(conn, content_conn, |c, team_id| assign_closer(c, team_id, &scores))?;
    ...
    conn.execute("DELETE FROM practice_stats", [])?;
...
pub fn advance(slot_conn: &mut Connection, content_conn: &Connection) -> anyhow::Result<Vec<PendingActionRow>> {
    ...
    for _ in 0..MAX_DAYS_PER_CALL {
        ...
        if today % 28 == 0 {
            ...
            // 청백전(연습경기, 대화 2026-07-26) — 고교·대학 팀 전체가 이번
            // 달 한 번씩 자체 스크리미지를 치른다. 바로 다음 줄의 로테이션
            // 재배정이 이 결과(practice_stats)까지 반영하므로 반드시 그
            // 전에 실행해야 한다("이번 달 청백전 결과가 이번 달 로테이션에
            // 곧바로 반영"이라는 자연스러운 순서).
            run_monthly_scrimmages(&tx, content_conn, world_seed, today)?;
            // 랭킹 배치 캐시(대화 2026-07-24, 실측 진단으로 발견) — 아래
            // 세 루프가 172팀을 순회하며 선수마다 season_stats/practice_stats
            // 를 개별 쿼리하던 N+1 패턴을, 이 시점(청백전 반영 후)의 두
            // 테이블을 한 번씩만 통째로 읽어 세 루프가 공유하는 캐시로
            // 교체(실측 결과 개선 폭은 기대보다 작았음 — `assign_batting_order`
            // 는 눈에 띄게 빨라졌지만 `reassign_rotation_preserving_next_turn`
            // 은 거의 그대로였다. 후자는 `schedule`/`season_meta` 개별
            // 조회가 이미 있어 그쪽이 더 큰 비중이었을 가능성 — 다음
            // 진단 대상은 `schedule`에 인덱스가 아예 없다는 점, §6-115 참고).
            let scores = StatScoreCache::load(&tx)?;
            // NPC 로테이션 시즌 중 실시간 재편(10_구현_Phase_계획.md §6-N) —
            // season_stats가 그동안 쌓은 성적을 이 시점에 로테이션 순번에
            // 반영. "다음 등판" 자리는 `preserve_next_turn`으로 보존해
            // 휴식 없는 재등판을 막는다.
            for_each_team_in_all_leagues(&tx, content_conn, |conn, team_id| reassign_rotation_preserving_next_turn(conn, content_conn, team_id, &scores))?;
...
mod tests {
    ...
    fn create_protagonist_inserts_expected_row() {
        ...
        assert_eq!(pitch_names_from_mastery(&pitches), vec!["포심 패스트볼".to_string(), "커터".to_string()]);
        for p in &pitches {
            assert_eq!(p.get("stage").and_then(|s| s.as_u64()), Some(1), "starting pitches begin at mastery stage 1");
        }
    ...
    fn create_protagonist_without_second_pitch_starts_fastball_only_for_non_control_archetypes() {
        ...
        for archetype in ["강속구형", "체력형", "돌부처형"] {
            ...
            let pitches: Vec<serde_json::Value> = serde_json::from_str(&pitches_raw).unwrap();
            assert_eq!(pitch_names_from_mastery(&pitches), vec!["포심 패스트볼".to_string()], "{archetype} should start with fastball only");
        }
    }
    ...
    fn create_protagonist_control_archetype_auto_gets_a_second_pitch_without_explicit_choice() {
        ...
        assert_eq!(names[0], "포심 패스트볼");
        assert!(crate::sim::protagonist::is_valid_second_pitch("제구형", &names[1]).unwrap());
    }
    ...
    #[test]
    fn set_protagonist_training_rejects_both_new_and_mastery_pitch_at_once() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "훈련테스트", "우완", "team:hanseong_hs", "강속구형", None).unwrap();
    ...
    #[test]
    fn set_protagonist_training_rejects_mastery_pitch_not_yet_known() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "훈련테스트", "우완", "team:hanseong_hs", "강속구형", None).unwrap();
    ...
    fn set_protagonist_training_rejects_new_pitch_once_the_five_pitch_cap_is_reached() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "만렙레퍼토리", "우완", "team:hanseong_hs", "제구형", None).unwrap();
        slot_conn
            .execute(
                "UPDATE protagonist SET pitches = ?1 WHERE id = 'proto:1'",
                params![
                    serde_json::json!([
    ...
    fn find_protagonist_game_today_benches_the_protagonist_when_it_is_not_their_rotation_turn() {
        ...
        assign_rotation(&slot_conn, &content_conn, "team:hanseong_hs", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    #[test]
    fn process_protagonist_week_a_strong_pitching_coach_shortens_pitch_learning() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        ...
        insert_test_player(
            ...
            "team:hanseong_hs",
            "코치",
            serde_json::json!({"투수지도력": 80.0, "타격지도력": 50.0, "주루지도력": 50.0, "컨디셔닝": 50.0, "멘탈코칭": 50.0, "스카우팅안목": 50.0, "종합지도력": 50.0}),
        );

        // "강" 강도 기본 8주 - coach_pitch_learning_bonus(80.0)=2주 = 6주.
        for week in 0..6i64 {
            process_protagonist_week(&slot_conn, &content_conn, 1, week * 7).unwrap();
        }
    ...
    #[test]
    fn process_protagonist_week_advances_pitch_mastery_stage_after_enough_weeks() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        ...
        let pitches_raw: String = slot_conn.query_row("SELECT pitches FROM protagonist WHERE id = 'proto:1'", [], |r| r.get(0)).unwrap();
        let pitches: Vec<serde_json::Value> = serde_json::from_str(&pitches_raw).unwrap();
        let fastball = pitches.iter().find(|p| p.get("name").and_then(|n| n.as_str()) == Some("포심 패스트볼")).unwrap();
        assert_eq!(fastball.get("stage").and_then(|s| s.as_u64()), Some(2), "6 weeks of mastery training should advance stage 1 -> 2");

        let training_raw: String = slot_conn.query_row("SELECT training FROM protagonist WHERE id = 'proto:1'", [], |r| r.get(0)).unwrap();
        let training: serde_json::Value = serde_json::from_str(&training_raw).unwrap();
        assert!(training.get("mastery_pitch").unwrap().is_null(), "mastery_pitch should reset to null once advanced");
    }

    #[test]
    fn set_protagonist_training_rejects_mastery_pitch_already_at_max_stage() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "만렙테스트", "우완", "team:hanseong_hs", "강속구형", None).unwrap();
        slot_conn
            .execute(
                "UPDATE protagonist SET pitches = ?1 WHERE id = 'proto:1'",
                params![serde_json::json!([{"name": "포심 패스트볼", "stage": 5, "weeks": 0}]).to_string()],
            )
            .unwrap();
    ...
    #[test]
    fn process_protagonist_week_pitch_mastery_never_exceeds_stage_five() {
        // set_protagonist_training이 5단계 구종을 막아주지만, 방어적으로
        // process_week 자체도 상한을 건다(대화 2026-07-23) — training 컬럼을
        // 직접 조작해 그 상한을 확인.
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "상한테스트", "우완", "team:hanseong_hs", "강속구형", None).unwrap();
        slot_conn
            .execute(
                "UPDATE protagonist SET pitches = ?1 WHERE id = 'proto:1'",
                params![serde_json::json!([{"name": "포심 패스트볼", "stage": 5, "weeks": 0}]).to_string()],
        ...
        let pitches: Vec<serde_json::Value> = serde_json::from_str(&pitches_raw).unwrap();
        let fastball = pitches.iter().find(|p| p.get("name").and_then(|n| n.as_str()) == Some("포심 패스트볼")).unwrap();
        assert_eq!(fastball.get("stage").and_then(|s| s.as_u64()), Some(5), "mastery stage must never exceed 5");
    }
        ...
        slot_conn
            .execute(
                "INSERT INTO protagonist (id, name, handedness, archetype, stats, xp, live_state, finance, pitches, contract, injury)
                 VALUES ('proto:1', '시장테스트', '우완', '강속구형', '{}', '{}', ?1, '{}', '[{\"name\":\"포심 패스트볼\",\"stage\":1,\"weeks\":0}]', ?2, '{\"current\":null,\"history\":[]}')",
    ...
    fn insert_market_protagonist_with_age(slot_conn: &Connection, contract: &serde_json::Value, attention: f64, age: i64) {
        slot_conn
            .execute(
                "INSERT INTO protagonist (id, name, handedness, archetype, stats, xp, live_state, finance, pitches, contract, injury, age)
                 VALUES ('proto:1', '시장테스트', '우완', '강속구형', '{}', '{}', ?1, '{}', '[{\"name\":\"포심 패스트볼\",\"stage\":1,\"weeks\":0}]', ?2, '{\"current\":null,\"history\":[]}', ?3)",
    ...
    fn process_month_raises_morale_after_a_good_month() {
        ...
        process_month(&slot_conn, &content_conn, 1, 28).unwrap();
    ...
    #[test]
    fn process_protagonist_promotion_calls_up_a_hot_farm_pitcher() {
        let content_conn = build_career_content_db();
        let mut triggered = false;
        for seed in 0..30i64 {
            let slot_conn = slot::open_in_memory().unwrap();
            insert_market_protagonist(&slot_conn, &serde_json::json!({"team_id": "team:pro_a_farm"}), 0.0);
            insert_game_log_grade_on_day(&slot_conn, "g1", 10, "S");
            insert_game_log_grade_on_day(&slot_conn, "g2", 15, "S");

            process_month(&slot_conn, &content_conn, seed, 28).unwrap();

            if read_contract(&slot_conn)["team_id"] == "team:pro_a" {
                let inbox_count: i64 = slot_conn.query_row("SELECT count(*) FROM inbox WHERE kind = 'callup'", [], |r| r.get(0)).unwrap();
                assert_eq!(inbox_count, 1);
                assert_eq!(career_event_kinds(&slot_conn), vec!["callup"]);
                triggered = true;
                break;
            }
        }
        assert!(triggered, "expected at least one seed to call up a hot farm pitcher");
    }

    #[test]
    fn process_protagonist_promotion_demotes_a_struggling_pro_pitcher() {
        let content_conn = build_career_content_db();
        let mut triggered = false;
        for seed in 0..30i64 {
            let slot_conn = slot::open_in_memory().unwrap();
            insert_market_protagonist(&slot_conn, &serde_json::json!({"team_id": "team:pro_a"}), 0.0);
            insert_game_log_grade_on_day(&slot_conn, "g1", 10, "F");
            insert_game_log_grade_on_day(&slot_conn, "g2", 15, "F");

            process_month(&slot_conn, &content_conn, seed, 28).unwrap();

            if read_contract(&slot_conn)["team_id"] == "team:pro_a_farm" {
                let inbox_count: i64 = slot_conn.query_row("SELECT count(*) FROM inbox WHERE kind = 'demotion'", [], |r| r.get(0)).unwrap();
                assert_eq!(inbox_count, 1);
                assert_eq!(career_event_kinds(&slot_conn), vec!["demotion"]);
                triggered = true;
                break;
            }
        }
        assert!(triggered, "expected at least one seed to demote a struggling pro pitcher");
    }

    #[test]
    fn process_protagonist_promotion_is_a_no_op_outside_pro_leagues() {
        let content_conn = build_career_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
    ...
    #[test]
    fn process_protagonist_contract_a_patient_owner_releases_less_often() {
        let content_conn = build_market_content_db();

        let release_count = |team_id: &str, with_patient_owner: bool| -> i64 {
            let mut count = 0;
            for seed in 0..40i64 {
                let slot_conn = slot::open_in_memory().unwrap();
                insert_market_protagonist(&slot_conn, &serde_json::json!({"team_id": team_id, "salary": 20000, "years_remaining": 3}), 0.0);
                insert_game_log_grades(&slot_conn, 0, &["F", "F", "F"]);
                if with_patient_owner {
                    insert_test_player(
                        &slot_conn,
                        &format!("owner:{team_id}"),
                        team_id,
                        "구단주",
                        serde_json::json!({"재력": 50.0, "투자성향": 50.0, "인내심": 80.0}),
                    );
                }
                process_protagonist_contract(&slot_conn, &content_conn, seed, 0, 364).unwrap();
                if read_contract(&slot_conn).get("team_id").map(serde_json::Value::is_null).unwrap_or(false) {
                    count += 1;
                }
            }
            count
        ...
        let impatient = release_count("team:poor_a", false);
        let patient = release_count("team:poor_a", true);
    ...
    fn build_career_content_db() -> Connection {
        let conn = content::open_in_memory().unwrap();
        for league_id in ["league:hs", "league:univ", "league:independent", "league:pro", "league:pro_farm"] {
        ...
        for (team_id, league_id) in [
            ...
            ("team:pro_a_farm", "league:pro_farm"),
        ] {
    ...
    fn insert_career_protagonist(slot_conn: &Connection, team_id: &str, age: i64, attention: f64) {
        slot_conn
            .execute(
                "INSERT INTO protagonist (id, name, handedness, archetype, stats, xp, live_state, finance, pitches, contract, injury, age)
                 VALUES ('proto:1', '갈림길테스트', '우완', '강속구형', '{}', '{}', ?1, '{}', '[{\"name\":\"포심 패스트볼\",\"stage\":1,\"weeks\":0}]', ?2, '{\"current\":null,\"history\":[]}', ?3)",
                params![
    ...
    #[test]
    fn process_protagonist_career_path_drafted_non_elite_player_starts_in_the_minors() {
        let content_conn = build_career_content_db();
        let mut triggered = false;
        for seed in 0..30i64 {
            let slot_conn = slot::open_in_memory().unwrap();
            insert_career_protagonist(&slot_conn, "team:hs_a", 19, 300.0); // 특급 기준(주목도 800) 미달
            insert_game_log_grades(&slot_conn, 0, &["B", "B", "B"]); // 특급 기준(4.5) 미달
            ...
            if let Some(action) = pending.iter().find(|p| p.kind == "draft") {
                let payload: serde_json::Value = serde_json::from_str(&action.payload).unwrap();
                assert_eq!(payload["team_id"], "team:pro_a_farm", "특급 신인이 아니면 2군에 배치돼야 한다");
                assert_eq!(read_contract(&slot_conn)["team_id"], "team:pro_a_farm");
                triggered = true;
                break;
            }
        }
        assert!(triggered, "expected at least one seed to draft a merely-decent HS senior into the minors");
    }
    ...
    fn advance_stops_at_protagonist_game_day_and_resolve_unblocks_it() {
        ...
        slot_conn
            .execute(
                "INSERT INTO protagonist (id, name, handedness, archetype, stats, xp, live_state, finance, pitches, contract, injury)
                 VALUES ('proto:1', '주인공', '우투', '강속구형', ?1, '{}', '{\"피로도\":0}', '{}', '[{\"name\":\"포심 패스트볼\",\"stage\":1,\"weeks\":0}]', '{\"team_id\":\"team:x\"}', '{}')",
    ...
    #[test]
    fn process_protagonist_career_milestones_notifies_once_when_within_reach_and_not_again_after() {
        let slot_conn = slot::open_in_memory().unwrap();
        ...
        let inbox_count = || -> i64 { slot_conn.query_row("SELECT count(*) FROM inbox WHERE kind = 'stat_milestone'", [], |r| r.get(0)).unwrap() };

        process_protagonist_career_milestones(&slot_conn, 28).unwrap();
        assert_eq!(inbox_count(), 1, "100승까지 3승 남았으면 알림이 떠야 함");
        let body: String = slot_conn.query_row("SELECT body FROM inbox WHERE kind = 'stat_milestone'", [], |r| r.get(0)).unwrap();
        assert!(body.contains("3승") && body.contains("100승"), "정확한 잔여 승수·목표 승수가 들어가야 함: {body}");

        // 같은 달에 다시 체크해도 같은 마일스톤이면 중복 알림 없음.
        process_protagonist_career_milestones(&slot_conn, 56).unwrap();
        assert_eq!(inbox_count(), 1, "같은 마일스톤은 두 번 알리지 않아야 함");
    }

    #[test]
    fn process_protagonist_career_milestones_stays_silent_when_far_from_the_next_step() {
        let slot_conn = slot::open_in_memory().unwrap();
        insert_market_protagonist(&slot_conn, &serde_json::json!({"team_id": "team:p1"}), 0.0);
    ...
    #[test]
    fn process_protagonist_career_milestones_covers_strikeouts_and_innings_too() {
        let slot_conn = slot::open_in_memory().unwrap();
        insert_market_protagonist(&slot_conn, &serde_json::json!({"team_id": "team:p1"}), 0.0);
        slot_conn
            .execute(
                "INSERT INTO game_log (game_id, season, detail) VALUES ('game:k', 0, ?1)",
                [serde_json::json!({"decision": "패", "strikeouts": 45, "innings_pitched": 48, "runs_allowed": 3}).to_string()],
            )
            .unwrap();

        process_protagonist_career_milestones(&slot_conn, 28).unwrap();
        let bodies: Vec<String> = {
            let mut stmt = slot_conn.prepare("SELECT body FROM inbox WHERE kind = 'stat_milestone' ORDER BY body").unwrap();
            stmt.query_map([], |r| r.get(0)).unwrap().collect::<Result<_, _>>().unwrap()
        };
        assert!(bodies.iter().any(|b| b.contains("탈삼진") && b.contains("50탈삼진")), "탈삼진 45개(50까지 5개 남음)도 알림이 떠야 함: {bodies:?}");
        assert!(bodies.iter().any(|b| b.contains("이닝") && b.contains("50이닝")), "이닝 48(50까지 2 남음)도 알림이 떠야 함: {bodies:?}");
    }
    ...
    /// 청백전 테스트용 — 청/백 양쪽 다 투수·타자가 최소 1명씩은 필요해
    /// `insert_minimal_roster`(투수 1명뿐)보다 투수를 더 넣는다.
    fn insert_scrimmage_ready_roster(conn: &Connection, team_id: &str) {
        for i in 0..4 {
            insert_test_player(
                conn,
                &format!("{team_id}_sp{i}"),
                team_id,
                "선발투수",
                serde_json::json!({"제구": 50.0 + i as f64, "구위": 50.0}),
            );
        }
        for i in 0..8 {
            insert_test_player(
    ...
    #[test]
    fn run_intrasquad_scrimmage_records_results_in_practice_stats_only() {
        let content_conn = content::open_in_memory().unwrap();
        content_conn.execute("INSERT INTO leagues (id, meta) VALUES ('league:hs', NULL)", []).unwrap();
        content_conn.execute("INSERT INTO teams (id, league_id, color, meta) VALUES ('team:x', 'league:hs', NULL, NULL)", []).unwrap();
        ...
        assert!(practice_rows > 0, "practice_stats should have rows after a scrimmage");
        let season_rows: i64 = slot_conn.query_row("SELECT count(*) FROM season_stats", [], |r| r.get(0)).unwrap();
    ...
    #[test]
    fn run_intrasquad_scrimmage_includes_the_protagonist_when_on_that_team() {
        let content_conn = build_hs_school_content_db();
        let slot_conn = slot::open_in_memory().unwrap();
        create_protagonist(&slot_conn, &content_conn, 1, "청백전주인공", "우완", "team:hanseong_hs", "강속구형", None).unwrap();
        insert_scrimmage_ready_roster(&slot_conn, "team:hanseong_hs");

        // 매번 최고 스탯 투수만 등판하면 스탯이 낮은 주인공은 영영 안
        // 뽑힌다 — day 기반 순환 선발이라, 실제로 여러 번(day 값을 바꿔가며)
        // 돌리면 결국 순서가 온다는 걸 확인한다(day를 1씩만 늘려 어느
        // 쪽 진영·길이든 모든 나머지를 다 훑도록).
        for day in 0..10i64 {
            run_intrasquad_scrimmage(&slot_conn, &content_conn, 1, "team:hanseong_hs", day).unwrap();
        }

        let proto_practice_rows: i64 =
            slot_conn.query_row("SELECT count(*) FROM practice_stats WHERE player_id = 'proto:1'", [], |r| r.get(0)).unwrap();
        assert!(proto_practice_rows > 0, "주인공도 순환 선발을 통해 청백전에 참가해 practice_stats에 기록이 남아야 함");
    }

    #[test]
    fn run_intrasquad_scrimmage_is_a_noop_when_the_roster_is_too_small_to_split() {
        let content_conn = content::open_in_memory().unwrap();
        content_conn.execute("INSERT INTO leagues (id, meta) VALUES ('league:hs', NULL)", []).unwrap();
        content_conn.execute("INSERT INTO teams (id, league_id, color, meta) VALUES ('team:x', 'league:hs', NULL, NULL)", []).unwrap();

        let slot_conn = slot::open_in_memory().unwrap();
        insert_minimal_roster(&slot_conn, "team:x"); // 투수 1명뿐 — 양쪽으로 못 나눔
    ...
    fn assign_rotation_stores_ordered_rotation_in_season_meta() {
        ...
        assign_rotation(&slot_conn, &content_conn, "team:a", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
        ...
        assert_eq!(rotation, vec!["team:a_sp2", "team:a_sp3", "team:a_sp1"], "능력치 내림차순이어야 함");
    }
    ...
    /// 청백전(practice_stats)도 season_stats처럼 로테이션 순위에 반영돼야
    /// 함(대화 2026-07-26, §4-5 "청백전 결과... 뎁스차트에 반영") — 시즌
    ...
    fn assign_closer_picks_the_higher_skill_reliever_and_stores_it_in_season_meta() {
        ...
        assign_closer(&slot_conn, "team:a", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn ranked_rotation_candidates_for_team_includes_the_protagonist_when_they_are_a_pitcher_on_that_team() {
        ...
        let ranked = ranked_rotation_candidates_for_team(&slot_conn, &content_conn, "team:hanseong_hs", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn ranked_rotation_candidates_for_team_excludes_the_protagonist_when_on_a_different_team() {
        ...
        let ranked = ranked_rotation_candidates_for_team(&slot_conn, &content_conn, "team:other", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn ranked_rotation_candidates_for_team_excludes_the_protagonist_outside_hs_and_univ() {
        ...
        let ranked = ranked_rotation_candidates_for_team(&slot_conn, &content_conn, "team:pro_x", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn ranked_batting_order_for_team_prefers_a_batter_with_a_better_season_line_over_equal_skill() {
        ...
        let order = ranked_batting_order_for_team(&slot_conn, "team:a", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn reassign_rotation_preserving_next_turn_keeps_the_immediately_due_pitcher() {
        ...
        reassign_rotation_preserving_next_turn(&slot_conn, &content_conn, "team:a", &StatScoreCache::load(&slot_conn).unwrap()).unwrap();
    ...
    fn for_each_team_in_all_leagues_reassigns_every_teams_rotation() {
        ...
        let scores = StatScoreCache::load(&slot_conn).unwrap();
        for_each_team_in_all_leagues(&slot_conn, &content_conn, |conn, team_id| assign_rotation(conn, &content_conn, team_id, &scores)).unwrap();
    ...
    fn load_starting_pitcher_cycles_through_the_rotation_as_games_complete() {
        ...
        // 완료 경기 1개를 기록 — 로테이션 1번째로 넘어가야 함.
        slot_conn
            .execute(
                "INSERT INTO schedule (game_id, day, home, away, result) VALUES ('game:1', 1, 'team:a', 'team:x', ?1)",
                [serde_json::json!({"home": 3, "away": 1}).to_string()],
            )
            .unwrap();
        assert_eq!(load_starting_pitcher(&slot_conn, "team:a").unwrap().id, "team:a_sp2");

        // 완료 경기 2개 — 로테이션 길이(2)만큼 돌았으니 다시 0번째로.
        slot_conn
            .execute(
                "INSERT INTO schedule (game_id, day, home, away, result) VALUES ('game:2', 2, 'team:a', 'team:x', ?1)",
                [serde_json::json!({"home": 2, "away": 5}).to_string()],
            )
            .unwrap();
        assert_eq!(load_starting_pitcher(&slot_conn, "team:a").unwrap().id, "team:a_sp1");
    }
    ...
    #[test]
    fn load_starting_pitcher_falls_back_when_no_rotation_assigned() {
        let slot_conn = slot::open_in_memory().unwrap();
        insert_minimal_roster(&slot_conn, "team:a");
        // assign_rotation을 아예 안 부름(구세이브 시뮬레이션) — 기존 "id
        // 기준 첫 명" 방식으로 그대로 동작해야 함.
        assert_eq!(load_starting_pitcher(&slot_conn, "team:a").unwrap().id, "team:a_sp");
    }
    ...
    fn load_starting_pitcher_falls_back_when_rotation_pitcher_becomes_unavailable() {
        ...
        // 로테이션 0번째(team:a_sp1)가 이탈했어도 크래시 없이 폴백(id 기준
        // 첫 명 — 남은 유일한 후보 team:a_sp2)으로 계속 등판 가능해야 함.
        assert_eq!(load_starting_pitcher(&slot_conn, "team:a").unwrap().id, "team:a_sp2");
    }
    ...
    fn season_rollover_archives_standings_and_resets_season_tables() {
        ...
        .unwrap();
        conn.execute("INSERT INTO practice_stats (player_id, week, line) VALUES ('npc:x', 1, '{}')", []).unwrap();
        ...
        let practice_stats_count: i64 = conn.query_row("SELECT count(*) FROM practice_stats", [], |r| r.get(0)).unwrap();
        assert_eq!(practice_stats_count, 0, "청백전 성적도 시즌 경계에 비워져야 함");
    ...
    #[test]
    fn generate_freshmen_gives_teams_with_a_sharper_scout_a_better_stat_pool() {
        let content_conn = content::open_in_memory().unwrap();
        content_conn.execute("INSERT INTO leagues (id, meta) VALUES ('league:hs', NULL)", []).unwrap();
        let rules = r#"{"roster_size":20,"pitcher_ratio":0.5,"sp_ratio":0.5,"stat_min":20.0,"stat_max":50.0}"#;
        content_conn.execute("INSERT INTO generation_rules (league_id, rules) VALUES ('league:hs', ?1)", [rules]).unwrap();
        for team_id in ["team:scouted", "team:unscouted"] {
            content_conn
                .execute("INSERT INTO teams (id, league_id, color, meta) VALUES (?1, 'league:hs', NULL, NULL)", [team_id])
                .unwrap();
            content_conn
                .execute(
                    "INSERT INTO team_traits (team_id, philosophy, resource, status) VALUES (?1, '전통/정통', '안정', '중견')",
        ...
        insert_test_player(
            ...
            "team:scouted",
            "코치",
            serde_json::json!({"투수지도력": 50.0, "타격지도력": 50.0, "주루지도력": 50.0, "컨디셔닝": 50.0, "멘탈코칭": 50.0, "스카우팅안목": 80.0, "종합지도력": 50.0}),
        );
        // team:unscouted는 코치 npc 자체가 없음.

        generate_freshmen(&slot_conn, &content_conn, 7, 1).unwrap();

        let mean_stat = |team_id: &str| -> f64 {
            let mut stmt = slot_conn.prepare("SELECT stats FROM npc WHERE team_id = ?1 AND position != '코치'").unwrap();
            let rows: Vec<String> = stmt.query_map([team_id], |r| r.get(0)).unwrap().collect::<Result<_, _>>().unwrap();
            let mut total = 0.0;
            let mut count = 0.0;
            for raw in rows {
                let v: serde_json::Value = serde_json::from_str(&raw).unwrap();
        ...
        let scouted_mean = mean_stat("team:scouted");
        let unscouted_mean = mean_stat("team:unscouted");
        assert!(scouted_mean > unscouted_mean, "scouted={scouted_mean} unscouted={unscouted_mean}");
    }
    ...
    #[test]
    fn process_week_grows_batters_faster_on_teams_with_a_stronger_batting_coach() {
        let content_conn = content::open_in_memory().unwrap();
        let slot_conn = slot::open_in_memory().unwrap();
        ...
        insert_test_player(&slot_conn, "npc:coached", "team:coached", "타자", initial.clone());
        insert_test_player(&slot_conn, "npc:uncoached", "team:uncoached", "타자", initial);
        insert_test_player(
            &slot_conn,
            "coach:team:coached",
            "team:coached",
            "코치",
            serde_json::json!({"투수지도력": 50.0, "타격지도력": 80.0, "주루지도력": 50.0, "컨디셔닝": 50.0, "멘탈코칭": 50.0, "스카우팅안목": 50.0, "종합지도력": 50.0}),
        );
        // team:uncoached는 코치 npc 자체가 없음 — coach_growth_focus가
        // 기본값(50.0)만 받아 배율 1.0(무보정)이 되는 케이스.

        for week in 0..40i64 {
            process_week(&slot_conn, &content_conn, 999, week * 7).unwrap();
        }

        let sum = |id: &str| read_stats(&slot_conn, id).as_object().unwrap().values().filter_map(|v| v.as_f64()).sum::<f64>();
        let coached_total = sum("npc:coached");
        let uncoached_total = sum("npc:uncoached");
        assert!(coached_total > uncoached_total, "coached={coached_total} uncoached={uncoached_total}");
    }
