...
## 화면이 멈춘 것처럼 보이지 않는 한계.
##
## ⚠ **2026-08-18에 1.0 → 30.0으로 늘렸다 (사용자 확정).** 늘린 이유가 있다 —
## 1.0에서는 여유가 7%(0.930초)뿐이라 **02에 있는 것을 못 옮기고 있었다**:
##
##   · P-18b 출장 시간 축 — 02 조기 입대 확률의 네 항 중 하나가 04에선
##     축이 없어 중립(1.0)으로 죽어 있다. NPC 9,600명의 주간 출장을 쌓아야 산다
##   · NPC 복무를 주 단위로 — 지금은 무거워서 "한 해에 `WEEKS_PER_SEASON`"으로
##     뭉쳐 세고 있다. 그래서 시즌 도중 전역이 없다
##   · `NpcGrowth`의 좁은 창
##
## ⚠ **여유를 다 쓰라는 뜻이 아니다.** 30.0은 **한계지 목표가 아니다** —
## 하루가 30초면 사실상 못 논다. **3초를 넘기면 게이트 안이라도 보고한다**
const GATE_DAY_SECONDS: float = 30.0

## 프레임을 넘기는 값까지 포함한 한계.
##
## ⚠ **넘기면 그만큼 늘어난다.** 얼어붙는 것보다는 낫지만 기어가면 그것대로
## 못 쓴다 — 실측 0.712 → 1.078초로 0.37초가 붙는다.
## 일하는 시간과 같은 비(2배)를 지킨다
const GATE_DAY_WALL_SECONDS: float = 60.0
