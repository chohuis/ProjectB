...
마지막 갱신: 2026-08-14 · 커밋 8d5d99bde
...
저장·불러오기   0.57MB · 저장 77ms · 로드 92ms
탭 5개         소식 · 나 · 팀 · 리그 · 일정
```
...
새 게임 화면    지금은 코드로만 시작한다
슬롯 화면      머리말 읽기는 되어 있다(0ms)
경기 화면      구장 이미지 27장 재사용 · 투구 선택
결산·드래프트 보드·훈련·재정·업적 …
```

⚠ **"68일 진행"이 16초다.** 최악의 날 게이트(1초)는 지키지만 여러 날을
한 번에 가면 쌓인다. 진행 표시가 뜨므로 멈춘 건 아니지만, 고교 개막 전처럼
긴 구간을 건너뛸 때 길다. 게이트로 감시할지는 정해야 한다.

⚠ **주인공 등판이 팀 경기 전부다.** 로테이션이 아직 안 붙어서 고교 20경기를
다 던진다 — M3의 `Roster.team_rotation`을 `MatchDay`에 끼우면 풀린다.
