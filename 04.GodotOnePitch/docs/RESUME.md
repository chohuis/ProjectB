...
마지막 갱신: 2026-08-14 · 커밋 81ec9c7e8
...
이주 중. P0·P1·**M1·M2·M3-1·M4·M5·M6·D1·D2·D3** 끝났다. **M7이 진행 중**이다 — 진행 화면·프레임 쪼개기까지 끝났고 나머지 화면이 남았다.
...
**검사 784개 통과 · 변이 전부 검출 · 게이트 통과.**
**M1·M2·M3-1·M4·M5·M6·D1·D2·D3 끝났다** — `sim/`에 28개 모듈.

## 성능 게이트 (실측으로 다시 잡음)

```
최악의 날 · 일하는 시간    0.71초  기준 1.0초   ← bench:day (진짜 일정·엔진)
최악의 날 · 기다리는 시간  1.08초  기준 2.0초   ← 프레임 쪼개기 값 포함
...
⚠ **여유가 얇다** — 하루 1.41배 · 시즌 1.12배. P0 때는 3배였다.
...
**끝난 것 — 진행 화면 한 벌과 그 규약:**

```
ui/main_vm.gd              날짜·요일·"다음 등판까지"·탭 알림 (D4)
ui/screens/main_screen.*   껍데기 — 헤더·탭·진행 버튼·진행 중 표시
ui/day_runner.gd           프레임 쪼개기 (M2가 미뤄둔 숙제)
bench/day_bench.gd         최악의 날을 진짜 일정·엔진으로 잰다
```

**남은 것 — 나머지 화면과 그것들을 묶는 앱 루트:**

```
앱 루트          세이브를 들고 `MainScreen.advance_requested`를 받아
                `DayRunner`를 돌리는 쪽. 지금은 아무도 안 받는다
탭 내용 6개      소식·나·팀·리그·인물·일정
경기 화면        구장 이미지 27장 재사용
나머지 화면      결산·드래프트 보드·훈련·재정 …
```

⚠ **화면은 `ViewModel` 사전 하나만 받는다.** 02 결함 상당수가 "화면이
계산을 갖고 있어서" 생겼다 — 수상 집계가 결산 모달에만, 드래프트 보드가
자기 후보 풀을 따로, 경력 기록 조립이 모달 안에만.

검사가 **양쪽 방향을 다 막는다**: 화면 소스에 `Calendar.`·`DayEngine.`·
`sort_custom`이 없어야 하고, ViewModel 소스에 `Control`·`Label`이 없어야
한다. 한쪽만 막으면 두 방향 의존이 생겨 ViewModel을 검사만으로 못 돌린다.

⚠ **`--ignoreHeadlessMode` 전제가 여기서 깨진다.** 씬 조작 검사를 만들면
`InputEvent`가 필요하다. 지금 화면 검사는 메서드를 직접 불러서 아직 괜찮다.

원본: `apps/ui/src/pages/**` (`MainPage.svelte` 859줄 · `StatusPage.svelte` 1,004줄)
...
⚠ **M6는 순서와 판정만 옮겼다.** 실제 오프시즌 처리(FA 계약·트레이드·방출
실행 · NPC 은퇴 · 진급)는 아직 없다 — `SeasonEnd.PHASES`가 그 자리를
이름으로 잡아 두고 있다.
