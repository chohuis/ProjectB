...
마지막 갱신: 2026-08-14 · 커밋 3aacd4d37
...
이주 중. P0·P1·**M1·M2·M3-1·M4·M5·M6·D1·D2·D3** 끝났다. 다음은 **M7(화면 55개)**.
...
**여기가 지금까지 만든 모듈의 첫 소비자다.** 그래서 M2·M6가 미뤄둔 것도
같이 붙는다:

```
D4 날짜 표시     "다음 경기까지 N일" · 요일. `Calendar`가 이미 판다
프레임 쪼개기    최악의 날 0.69초를 멈춤이 아니라 진행 표시로.
                경기 사이에 프레임을 넘긴다 — 이제 부르는 쪽이 생겼다
C 검사 9개       화면이 계산을 갖지 않는지 보는 것들
```
...
`InputEvent`가 필요하다. 그때 다시 본다.

원본: `apps/ui/src/features/**` · `routes/**`
