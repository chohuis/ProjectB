...
마지막 갱신: 2026-08-14 · 커밋 b5a7a01b7
...
이주 중. P0·P1·**M1·M2·M3-1·M4·D1·D2** 끝났다. 다음은 **M5(드래프트·수상)**.
...
**검사 584개 통과 · 변이 전부 검출 · 게이트 통과.**
**M1·M2·M3-1·M4·D1·D2 끝났다** — `sim/`에 23개 모듈.

## 성능 게이트 (실측으로 다시 잡음)

```
최악의 날 (83경기)     0.69초  기준 1.0초   ← 사용자가 실제로 기다리는 시간
합성 벤치 (98경기)     0.80초  기준 1.0초   ← bench:game 이 재는 값
...
6,396경기이고 최악의 날이 83경기다. `bench:game`은 아직 국내 기준 합성
부하(98경기/주)를 재는데, 하루 부하는 `schedule_test`가 따로 본다.
...
실측 최악의 날 **83경기 = 0.69초** (게이트 1초). 압축했을 때(77)보다 오히려
많다 — 성기게 펴면 리그들이 어떤 날엔 몰린다.

### 지금: M5 드래프트·수상 (다음 차례)

원본: `packages/engine-native/src/draft.rs` · `npc_sim.rs`의 수상 판정
검사: `protagonistAward` · `draftRelative` · `pickInRound` · `draftBoardSeat` 등

02에서 실측으로 잡은 값이 그대로 온다 (`RESUME` 아래 "02에서 무엇을 읽나"):
드래프트 앵커 `round = 11 - (score-78) × 0.40` · 문턱 78 ·
수상 자격선 다승·탈삼진 40 · 방어율 45.

M1이 깔아둔 것을 쓴다 — 수상 후보는 `Leaderboard`, 통산 집계는 `CareerSummary`.
...
`neededPositions` — 충원 시 **어느 자리를 채우나**. 포지션 공백만 보면
야수 총원이 빈다. 실측 고교 102팀 전부가 어느 해엔가 포지션 공백,
포수 0명이 31팀. M6(오프시즌)와 짝이다.
...
```
M5  드래프트·수상
M6  시즌 진행·오프시즌 (+ D3 진행기 · 리그 무관 승강·FA·방출)
M7  화면 55개 (C 검사 9개 포함, + D4 날짜 표시)
P6  Steam 빌드·패키징
```
