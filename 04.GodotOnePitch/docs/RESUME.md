...
마지막 갱신: 2026-08-14 · 커밋 91d73192c
...
이주 중. P0·P1·**M1·M2·M3-1·M4·M5·D1·D2** 끝났다. 다음은 **M6(시즌 진행·오프시즌)**.
...
**검사 634개 통과 · 변이 전부 검출 · 게이트 통과.**
**M1·M2·M3-1·M4·M5·D1·D2 끝났다** — `sim/`에 25개 모듈.
...
### 지금: M6 시즌 진행·오프시즌 (다음 차례)

**여기가 D3(일 단위 진행기)의 집이다.** 그리고 미뤄둔 것들이 모인다:

```
D3 진행기       advance_day / advance_until_stop
                ⚠ 주 경계를 건너뛰면 성장이 통째로 빠진다 —
                  "하루씩 30번 = 30일 한 번" 검사가 첫 관문
프레임 쪼개기    최악의 날 0.69초를 진행 표시로. 여기가 부르는 쪽이다
리그 무관 관리   승강·FA·방출이 02에선 `LEAGUE_KBL`에만 걸려 있었다 —
                해외를 매일 풀 시뮬하므로 안 고치면 로스터가 부푼다
                (실측 상한 34인 1군이 41명)
neededPositions M3에서 미룬 것. 충원 시 어느 자리를 채우나
```

원본: `usecases/advanceWeek.ts` · `weekPhases/*` 12개 · `week_engine.rs`
