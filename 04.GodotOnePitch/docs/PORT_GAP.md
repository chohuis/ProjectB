...
⚠ **아직 미필이 계속 줄기만 한다**(12해에 7,336 → 1,975).
**이 계측이 게임 경로를 안 탄다** — `NpcMilitary.run`만 부르고 시즌 롤오버를
안 돌려서 **신인이 안 들어온다.** 실제 게임은 해마다 드래프트로 미필 신인이
채워진다. ⬜ **`SeasonRunner`를 지나는 계측으로 다시 잰다** — 이번 루프에서
같은 형태(계측이 게임 경로를 안 탐)를 **네 번째** 만났다.
