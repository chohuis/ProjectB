...
### 성능 게이트를 1.0 → **3.0초**로 늘렸다 (2026-08-18 · 사용자 확정)
...
❓ **다만 단조 감소지 오르내림이 아니다.** 상위 미필이 빠져나가는 만큼
신인이 못 채운다 — 2033년부터 **풀 최고가 70.0에 눌러앉는다.**
세계 전체가 마르는 쪽(선수 7,587 → 6,744)이라 **P-18이 아니라 성장·신인
쪽 문제로 보인다.** ⬜ D-3(성장 곡선)에서 같이 본다. 여기서 값을 만지면
두 변수를 동시에 움직이는 셈이라 손대지 않았다.

검사 19(`test/sports_unit_quota_test.gd`) · **변이 12/12** ·
성능 **0.902 · 0.960 · 0.915초**(기준 3.0).
