...
| 항목 | 02 | 04 | 할 일 |
|---|---|---|---|
| **테마 (라이트/다크)** | ✅ radiogroup · `styles.css`에 값 둘 다 있다 | ❌ | **사용자가 콕 집었다.** `AppTheme` 한 곳이 색 정본이라 뒤집을 자리는 이미 있다 |
| **언어** | ✅ | ❌ | 04는 한국어 단일 — 문구를 표로 빼는 일이 먼저다 |
| **연출 속도** | ✅ | ❌ | ⚠ 04 주석 "볼 값이 어디에도 없다" — **지금도 맞는지 확인** |
| **모션 줄이기** | ✅ (`prefers-reduced-motion`과 OR) | ❌ | 위와 같다 |
| 창 크기 | ✅ | ✅ | |
