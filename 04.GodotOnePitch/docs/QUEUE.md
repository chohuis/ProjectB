...
| # | 무엇 | 손질 범위 | 검사 |
|---|---|---|---|
| ⬜ **U-1** | **`injury_history`를 아무도 안 채운다.** `status_vm.gd:48`이 읽고 `status_screen.gd:428-440`이 카드를 만드는데 쓰는 곳이 `fixtures.gd`뿐이다 — **진짜 게임에선 부상 이력 카드가 안 뜬다.** 실제 기록은 `body_log`에 있다(`injury_runner.gd:187-190`, 영구 손실까지) | `status_vm.gd` +15줄. **키를 지우고 `body_log`를 읽는 쪽이 낫다** — 두 정본을 만들지 않는다 | 새 검사 1 — "완치 뒤 이력 카드에 그 부상이 뜨는가" |
| ⬜ **U-2** | **병역이 하나도 안 보인다.** `sim/military.gd`가 매주 도는데(`career_runner.gd:42-43`) `status_vm`·`main_vm`에 병역 키가 없다. 02는 네 자리에서 보여줬다 | `status_vm.gd` +10 · `status_screen.gd` 카드 1 | 새 검사 1 |
| ⬜ **U-3** | **OVR을 어디에도 안 보여준다.** 내 OVR은 팀 탭 로스터에서 내 줄을 찾아야 보인다 | `main_vm.gd` +5 · `main_screen.gd` +15 · `.tscn` | `main_vm` 키 검사 |
| ⬜ **U-4** | 설정 화면이 없다 — **창 크기도 못 바꾼다**(PC가 1차 목표) | 새 화면 ~120줄 · `app.gd` 입구 | 새 검사 1 |
| ⬜ **U-5** | 키보드가 0건 — 스페이스로 진행 | `app_root.gd` +10 | ⚠ **`--ignoreHeadlessMode` 전제 재검토.** 키 처리를 함수로 뽑아 함수 단위로 검사한다 |
| ⬜ **U-6** | 탭에 "나 / 세계" 구분선 | `main_screen.gd` +5 | 없다 |
| ⬜ **U-7** | 슬롯 덮어쓰기 — 시작 버튼을 "덮어쓰고 시작"으로 | `new_game_screen.gd` +5 | 없다 |
| ⬜ **U-8** | 관계 7색이 이웃끼리 안 갈린다 (신뢰↔각별 대비 **1.06**) | `theme.gd:45-53` | 없다 |
| ⬜ **U-9** | 빈 상태 문구 — 영영 없을 수 있는 것에만 이유를 붙인다 | `season_end_screen.gd` · `people_vm.gd` | 없다 |

**U-1 · U-2가 먼저다** — 나머지는 편의이고 이 둘은 **죽은 배선**이다.
