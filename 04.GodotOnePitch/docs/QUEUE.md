...
### 동결 해제 후 (F-1~F-6) — `UX_REPORT.md` 맨 아래

`F-1` 구종 습득 시스템(**`world.gd:280`이 직구 하나를 박고 끝. 소비처는 다 있는데
생산처가 없다**) · `F-2` 계약 협상 틀 넓히기(`kind: "negotiate"`) ·
`F-3` `TeamProfile` 초기화 + 팀 평가 화면(**축 열둘 중 아홉이 영원히 50**) ·
`F-4` 선수·팀 상세 · `F-5` 경기 전 브리핑 · `F-6` 투구 방향·폼·생년월일
