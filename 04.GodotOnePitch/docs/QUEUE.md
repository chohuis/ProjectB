...
| ⬜ **F-8b** | 소식 화면에서 **선택지를 고를 자리가 없다.** `NewsVm`이 `decision`을 거르기만 하고 화면에 버튼이 없다 — **지금은 아무도 답할 수 없다**(진행을 안 막으니 자동 진행도 안 답한다). ⚠ **`AutoAdvance.pick_choice`는 아직 실제 게임에서 안 돈다** — 막는 소식이 0건이기 때문이다 | |
