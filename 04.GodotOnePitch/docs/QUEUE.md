...
- [ ] **P-7d 4해를 굴렸는데 등판이 10경기다** (축 9)
      1해 실행과 등판 수가 **같다** — 2년차부터 등판이 안 잡힌다는 뜻이다.
      라이벌이 5명(02는 8명)이고 평균이 −0.8(02는 +3.8)인 것도 여기서
      올 수 있다.
      ⚠ **입력부터 센다.** 진로가 진행되며 소속이 어떻게 바뀌는지,
      일정에 `is_protagonist_game`이 계속 붙는지 본다.
