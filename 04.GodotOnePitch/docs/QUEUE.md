...
| ⬜ **F-3** | `TeamProfile` 초기화 + 팀 평가 화면 — **축 열둘 중 아홉이 영원히 50**이다. 아래 조사 결과를 먼저 읽는다 | |
...
| ⬜ **F-4b** | **팀 상세** — 02 `TeamDetailModal`(979줄). 이번 시즌·팀 평가·구성원·팀 역사·로테이션·타순. **F-3(`TeamProfile` 초기화)이 먼저다** — 축 열둘 중 아홉이 영원히 50이라 평가 절이 지금은 거짓말을 한다 | |
