...
| ⬜ **F-4c** | **국내 구장 이름 데이터가 없다.** 고교·대학·독립·KBL **182팀의 `stadium`이 전부 `STADIUM_SEOUL_ROYALS` 꼴**이고, 해외 56팀만 "엠파이어 스타디움"처럼 이름이 있다. 지금은 id 형태면 안 보여준다(원문이 새는 것보다 낫다) — 이름이 생기면 그 가드를 걷는다 | |
