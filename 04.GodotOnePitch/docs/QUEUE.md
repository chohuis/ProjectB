...
- [ ] ⚠ **D-8 성능 — 진짜 엔진이 게이트를 넘는다** (2026-08-15 실측)
      ```
      최악의 날 85경기      0.907초  기준 1.0   ○ (bench:day, 예전부터 진짜 엔진)
      최악의 주 98경기      1.117초  기준 1.0   ✗
      한 시즌 2,612경기    29.779초  기준 25.0  ✗
      경기당 투구 169.1 · 득점 7.67
      ```
      **지금까지 통과라고 적어 온 "시즌 0.6초"는 P0 뼈대의 값이다.**
      진짜 엔금은 경기당 약 11.4ms다.
      ⚠ **밸런스 동결 중이라 손대지 않았다** — 튜닝이 아니라 최적화라도
      경기 결과가 바뀌면 대조가 무너진다. **사용자 판단이 필요하다.**
      먼저 볼 것: `GameLoop.play` 안의 할당(루프에서 사전 만들기) ·
      `MatchDay.bullpen_of`가 경기마다 도는지 · 투구마다 `Dictionary` 조회
