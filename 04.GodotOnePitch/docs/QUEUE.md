...
| ⬜ **F-6b** | 주인공의 방향 · **투구 폼** · **생년월일** — 새 게임 화면에 입력이 없다. 02 `NewGamePage.svelte:21-40`이 셋을 고르게 한다(폼 넷: overhand · threeQuarter · sidearm · underhand). ⚠ **02에서도 `pitchingForm`은 저장만 되고 아무 데도 안 쓰인다**(`grep pitchingForm` → 타입·화면·픽스처뿐) — 옮기더라도 표시용이라고 적어 둔다. 생년월일은 `"2010-MM-DD"` 주인공 전용 | |
