...
- [ ] **P-6b 2군에 스태프가 없다** — **동결과 무관**
      02는 감독 182명(2군 포함), 04는 172명(1군만).
      `World.teams_of`가 `_FARM` 리그를 1군에서 파생하는데, 그 사전이
      `{id, name, league_id, parent_id}`뿐이라 **`power`와 `resource`를
      안 싣는다**(`world.gd:76-87`). 그래서 2군에 스태프를 만들려 해도
      전력★ 2 · 안정으로 뭉개진다.
      04 `teams.json`엔 KBL `_2` 행이 그대로 있고(20팀) 방금 `resource`도
      붙었다 — 파생할 때 그 행을 쓰면 된다.
      ⚠ 파생을 하나로 통일한 것은 의도다(`world.gd` 주석) — 필드만 싣는다.
