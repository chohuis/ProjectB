...
static func run(state: Dictionary, at_day: int = -1) -> void:
	...
	# NPC 병역 — P-18. **NPC도 상무 영향을 받아야 주인공이 신청하는 해에
	# 경쟁이 빡세기도 하고 아니기도 하다**(사용자 확정).
	# 안 부르면 미필 풀이 영영 안 줄어 당락이 주인공 OVR 하나로 고정된다.
	...
	# 고교 유망주 TOP 10 — 🔴 **04엔 이 소식도 이 효과도 통째로 없었다.**
	# 02는 고교 시절 4주마다 랭킹을 보내고 그 순위가 인기·스카우트 점수·
	# 사기에 실제로 닿는다. 고교 3년 내내 자기 위치를 알 방법이 없었다.
	# 고교일 때 · 랭킹 주에만 실제로 돈다 — 안쪽에서 본다
	ProspectRunner.run(state, at_day)
