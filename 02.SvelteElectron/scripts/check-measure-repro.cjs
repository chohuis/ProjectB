 ...
 *
 *   ⚠ **결정 ⑭(시작 능력치)를 넣은 뒤** (2026-09-09 · 2회):
 *       대학합격3 · 3년차 OVR 68 · 구속 67 · **2회 동일 · 완주**
 *       (프리셋 OVR 70 → 61 · 전 스탯 일괄 −9)
 *       🔴 지명이 대학합격으로 바뀐 것이 **이 변경의 목적**이다 —
 *          상위픽이 보장돼 대학·독립 갈래를 겪을 이유가 없던 것을 고쳤다.
 *
 *   ⚠ **구장 담장을 배선한 뒤** (2026-09-22 · A · 2회 · `track/engine`):
 *       대학합격3 · 3년차 OVR **70** · 구속 **71** · **2회 동일 · 완주**
 *       (앞줄 68·67 에서 올랐다. 결과 갈래는 그대로 대학합격3 이다)
 *       🔴 **의도된 변화다.** 주인공 쪽 여섯 자리가 `parkDims` 를 안 넘겨
 *          주인공은 어디서 던지든 중립 담장이었다. 이제 고교 27개 구장의
 *          치수가 실제로 걸린다 — 같은 씨앗의 고교 경기가 달라졌고,
 *          그러니 3년치 성장·기록도 달라진다.
 *          같은 커밋 묶음에서 구단 기질 셋(철학·자원 파생)도 들어갔다.
 *          이 검사가 지키는 것은 「같은 입력이면 같은 결과」지
 *          「값이 안 변한다」가 아니다 — 위 2026-09-08 주석과 같은 이유다.
 *
 *   ⚠ **구단 성향 정본을 파생 하나로 통일한 뒤** (2026-09-22 · A · 2회 · `track/engine`):
 *       대학합격3 · 3년차 OVR 70 · 구속 **72** · **2회 동일 · 완주**
 *       (앞줄 70·71 에서 구속만 +1. 결과 갈래는 그대로 대학합격3 이다)
 *       🔴 ABL 16팀의 성향 시작값이 손수값 → 예산·성향 파생으로 갈아탔고,
 *          `fromSaveGame` 이 세이브에 없는 팀을 파생으로 채우게 됐다
 *          (예전엔 `?? {}` 라 옛 세이브가 전 팀 50이었다). 배경 리그의
 *          시즌 롤오버가 달라지니 같은 씨앗의 3년치도 달라진다 —
 *          **입력이 바뀌었으니 결과가 바뀌는 것이 맞다.**
 *
 *   ⚠ **프로 정원 압축 사상(γ=0.20)·`rosterMin` 29 를 넣은 뒤** (2026-09-25 · A · 2회 · `track/engine`):
 *       대학합격3 · 3년차 OVR 70 · 구속 **73** · **2회 동일 · 완주**
 *       (앞줄 70·72 에서 구속만 +1. 결과 갈래는 그대로 대학합격3 이다)
 *       🔴 **의도된 변화다.** 프로 1군 인구가 1,268 → 1,153 명으로 줄었다
 *          (KBL 340→303 · ABL 544→488 · JBL 384→362). 배경 리그의 로스터가
 *          달라지니 드래프트 풀·FA·승강이 달라지고, 같은 씨앗의 3년치도
 *          달라진다 — **입력이 바뀌었으니 결과가 바뀌는 것이 맞다.**
