...
log("## 판별");
log("");
log("| # | 성향 | 프리셋 | 씨앗 | 진로 | 고졸직행 | 프로시즌 | 통산승 | 최고OVR | 최고연봉 | 히든 | 예외 | 폴백 | 정지 |");
log("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|");
const rows = runs.map((r, i) => {
  const ys = r.해마다;
  // 고졸 직행 = **고교 무대인 해에 지명**됐다(대학·독립을 안 거쳤다)
  const 고졸직행 = ys.some((y) => y.무대 === "highschool" && y.일.some((x) => x.startsWith("drafted")));
...
for (const x of rows) {
  log(`| ${x.n} | ${x.성향} | ${x.프리셋} | ${x.씨앗} | ${x.진로} | ${x.고졸직행 ? "✅" : "—"} `
    ...
    + `| ${x.예외 ? `🔴 ${x.예외}` : 0} | ${x.폴백 ? `🔴 ${x.폴백}` : 0} | ${x.정지 ? "🔴 정지" : "—"} |`);
}
log("");
...
if (폴백합) {
  ...
  log("| 무대/등급 | 횟수 |");
  log("|---|---|");
  for (const [k, v] of Object.entries(by).sort((a, b) => b[1] - a[1])) log(`| ${k} | ${v} |`);
  log("");
}
