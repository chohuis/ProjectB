...
const rows = runs.map((r, i) => {
  ...
  // 🔴 **마지막 해가 무대 갈림이면 그 줄이 안 접힌다** (2026-09-13 실측 · #7).
  //   `독립 → 지명 → 2군 → 입대` 로 12해를 다 지났는데 마지막 해(입대)가
  //   줄로 안 남아 **11시즌으로 보이고 「정지」로 찍혔다.** 판은 멀쩡했다 —
  //   `커리어이벤트` 에 그 해가 그대로 있다(`2037:military_enlist`).
  //   ⚠ 이 저장소가 아홉 번 밟은 「잣대가 먼저 틀렸다」의 또 한 자리다.
  //   줄과 커리어이벤트 **둘 중 넓은 쪽**을 판의 연도폭으로 본다.
  const evYears = (r.꼬리.커리어이벤트 ?? [])
    .map((x) => Number(String(x).split(":")[0]))
    .filter((y) => Number.isFinite(y) && y > 1900);
  const 첫해 = ys.length ? ys[0].연도 : 0;
  const 끝해 = Math.max(ys.length ? ys[ys.length - 1].연도 : 0, ...(evYears.length ? evYears : [0]));
  const 연도폭 = ys.length ? 끝해 - 첫해 + 1 : 0;
  const 정지 = 연도폭 < SEASONS_ASKED && r.머리.은퇴나이 == null;
  // 🔴 **같은 "연도" 가 두 줄로 찍히는 자리(2026-09-11 확인).** `불완전`
  //   (빈 자리표시자·전 칸 0)과는 **다른 결함**이다 — 실측해 보니 두 줄
  //   다 나이·스탯이 실제로 다른 **진짜 두 해**인데(예: 군 복무 중 한
  //   해·전역한 다음 해) "연도" 라벨만 같은 값을 반복했다(예:
  //   2036·2037·2037 — 나이는 26·27·28로 제대로 늘었다). 원인은
  //   `probe-a-simrun-worker.cjs` 의 연도 표기 쪽으로 보이나 이번엔
  //   워커를 고치지 않는다(재실행 없이 이미 받은 판만 다시 센다).
  //   `해마다.length`(실제 줄 수)가 `연도폭`(라벨상 달력 폭)보다 크면
  //   그 차이만큼 라벨이 겹친 것이다 — 필러(`불완전`)는 갭을 메워
  //   달력 폭 **안에** 들어가므로(그 해가 없었다면 원래 있어야 할
  //   자리를 채울 뿐) 이 식에서 따로 뺄 필요가 없다(실측: after #4 —
  //   13줄·2026~2037 폭12·필러0 → 중복 1. final #7 — 13줄·폭12·필러 2개
  //   (2029·2035, 둘 다 유일한 줄) → 중복 1, 필러를 빼면 오히려 0으로
  //   틀렸을 것이다).
  const 중복연도 = Math.max(0, ys.length - 연도폭);
  return {
...
// 🔴 **불완전과는 다른 결함이다** — 필러(빈 자리표시자)가 아니라 진짜
//   서로 다른 두 해가 "연도" 라벨만 겹친 것이다(위 산식 주석 참고).
//   "판 전체가 깨졌다"는 뜻이 아니다 — 정지 0·예외 0 은 그대로다.
const 중복연도합 = rows.reduce((a, x) => a + x.중복연도, 0);
const 중복연도판 = rows.filter((x) => x.중복연도 > 0).length;
log(`| 같은 해가 두 줄로 찍힘(연도 라벨 겹침 — 판 실패 아님) | 0 | **${중복연도합}건 / ${중복연도판}판** | `
  + `${중복연도합 ? "🔴 워커의 연도 표기 결함 · 값 자체는 두 줄 다 진짜다" : "ok"} |`);
log(`| 폴백 | 0 | **${폴백합}** | ${폴백합 ? "🔴" : "ok"} |`);
log("");
