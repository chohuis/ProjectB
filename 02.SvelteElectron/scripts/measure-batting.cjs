...
// 12타수 7안타(.583)가 타격왕이 됐다.
//
// 🔴 **`pa ≠ ab+bb` 가 0이어야 한다는 말은 낡았다** (2026-09-02).
//   그건 사구·희생타·희생플라이가 엔진에 없던 시절 기준이다. 지금은
//   `PA = AB + BB + HBP + SAC + SF` 라 어긋나는 게 정상이고,
//   그 낡은 잣대 때문에 84% 를 결함으로 적어 목록에 올려 뒀었다.
//   프로브가 보는 값은 `pa파생깨짐`(음수거나 25% 초과)이다.
//
