...
### 4.2 ⚠ 미결 — 배경 NPC 주간 시뮬 범위

두 안 중 미확정. 결정 전까지 관련 코드는 현행 유지.

| | A안: 주인공 리그만 | B안: Named는 전 리그 |
|---|---|---|
| 주간 성장 | 소속 리그 NPC만 주간, 타 리그는 시즌 종료 1회 일괄 | A안 + Named NPC는 리그 무관 주간 처리 |
| 부상 | 소속 리그만, 3단계(결장 주수만) | 동일 |
| 성능 | 최대 (주간 IPC 1~2회) | 중간 |
| 서사 | 타 리그 간 라이벌 추적 약함 | 고교 동기가 ABL 가도 성장 추적 가능 |
| 권장 상황 | 순수 성능 우선 | 라이벌 서사를 게임 축으로 유지할 때 |

> 판단 힌트: 감정 시스템·라이벌 스토리후크를 Lite에서도 살릴 거면 B안, 서사를 이벤트 텍스트로만 처리할 거면 A안.
...
- [ ] 신규 스키마: 파일=슬롯(`slot_<id>.db`), `npc` 단일 테이블(능력치·XP JSON 컬럼), `meta`(worldSeed·서명)
...
- [ ] §4.2 배경 NPC 주간 시뮬 범위 (A안 vs B안) — R3 착수 전 확정
- [ ] npc 테이블 능력치 JSON 컬럼의 Rust 측 직렬화 규약 (serde camelCase 유지 여부)
- [ ] 드리프트 리그의 시즌 종료 "리그 리더" 생성 규칙 상세
