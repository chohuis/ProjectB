...
| **시뮬 원칙** | **전 리그 풀 시뮬** (2026-08-06 해외 개방). 반경 게이트는 껍데기만 남았다 |
...
| **해외** | ABL 32 · JBL 24 | **열림 (2026-08-06)** — 국내와 같이 경기 단위 풀 시뮬.<br>승강·FA·트레이드가 `activeProLeagues()`로 같이 돈다.<br>닫으려면 `shared/config/releaseScope.ts`의 두 Set에 다시 넣는다 |
...
- 미지명 → 대학·**육성선수(프로 2군)**·독립·입대 재경로.

**미지명 진로 순서** (`draft.rs` `Placer::place` — 미지명 졸업생 · 방출된 프로 ·
FA 미계약자가 **같은 규칙**을 탄다):

```
대학(고교 졸업자만) → 프로 2군 육성선수 → 독립(나이 제한) → 야구를 그만둔다
```

**육성선수 = KBO 육성선수 제도** (2015년 이전 명칭 "신고선수"):

| | 드래프트 지명 | 육성선수 |
|---|---|---|
| 정원 | 정식 등록 | **정원 밖** |
| 연봉 | 최저 3000만원 + 계약금 | **2000만원 · 계약금 없음** |
| 기간 | 3년 | **단년** |
| 1군 등록 | 즉시 | **입단 연도 5월 1일 이후** |
| 능력치 | 2군 48~76 | **42~64 — 단 천장은 76 기준** |
| 정원 | 정식 34명 안 | **정원 밖 · 팀당 10명까지** |

정원이 정식(34)과 **따로**인 게 제도의 전제다. 34를 같이 쓰면 로스터가 찬
순간 제도가 없는 것과 같다 — 실측에서 KBL 2군 여유가 5자리라 미지명자
1,373명 중 **0명**이 들어갔다. 반대로 무제한이면 2군이 육성선수로 채워져
드래프트 지명의 가치가 사라지므로 팀당 상한을 둔다.

배정 순서는 **능력치 순**이다. 방출자와 미지명 졸업생이 같은 루프를 타는데
인덱스 순으로 돌면 방출자가 자리를 다 먹는다(실측: 졸업생 0명).

마지막 줄이 이 제도의 핵심이다. **지금은 약하지만 클 수 있다** — 그래서
"장기적으로 프로가 되려면 엄청 노력해야 한다"가 성립한다(사용자 확정
2026-08-07). 시작 능력치를 낮추면서 천장까지 낮추면 그냥 약한 선수가 되고
경로 자체가 없어지므로, 생성 시 `potentialOvrMax`를 따로 넘긴다.

신분의 정본은 `NpcSaveState.developmentSince` 하나다. **소속이 아니다** —
강등된 정식 등록 선수도 2군에 있으므로 리그 ID로 판정하면 안 된다. 1군에
등록되는 순간 신분이 풀린다(정식선수 전환).

검사: `npm run check:devplayer` (실제 엔진 호출 + 배선 제거 대조군).
...
| **스태프 능력치 15종 전면 배선** | 코치5·감독5·구단주5 | ✅ **7-5 완료.** 실측해보니 쓰이는 건 2종이 아니라 **1종**이었다(이름이 세 벌이라 조용히 기본값으로 돌았다). 계수 정본은 `staff_rules.toml [effects]`, 읽는 곳은 `utils/staffEffects.ts` 하나 |
| **부상 전조 경고** | 피로 임계 초과 **첫 주는 경고만** → 다음 주도 초과면 실제 부상 판정. NPC는 경고 없이 기존대로 | ✅ **7-5 완료.** 유예 주에도 고강도 훈련은 다친다(면죄부 방지). 경고에 다음 주 실제 확률을 숫자로 담는다 |
| **개인 재정** ✅ 7-5 | 스폰서·CF(명성 연동 4카테고리) · 누진세(수령 시 자동공제) · 사치품(동료에 쓰면 관계도↑, 자기 소비는 성격에 따라 명성 ±) · 개인 트레이닝 구독(3분야×2단계, **보너스가 팀 자원에 반비례**) · 투자(시즌말 3택) · 최종 자산(은퇴 화면, 등급 없이 숫자만) | ✅ **7-5 완료.** "페이지 이미 존재"는 사실이 아니었다 — 화면이 OVR·사기로 수입을 즉석 계산해 `money`와 무관한 숫자를 보여주고 있었다. 통째로 다시 썼다 |
| **뉴스·미디어 4종** | 소속팀·라이벌 경기결과 · 동료 마일스톤(실측) · 내 팀/지역 순위 월간 요약 · 인접권역·먼 리그 주간 다이제스트 | ✅ **6-137~141 완료.** 7-6에서 국가대표·국제대회·FA시장·쇼케이스·올스타가 추가됐다 |
| **문장 뱅크 엔진** | 이벤트 본문을 배열로 두고 **직전 문장을 제외**하고 랜덤 선택. 저작 시 3개 미만이면 빌드 실패 | ✅ **7-6 완료.** `test:sentencebank`가 "빌드 실패"의 구현. 직전 선택은 세이브에 남는다 |
...
- [x] 대학 이벤트 2종(쇼케이스·올스타전) — **Phase 7-7에서 완료** (고교 스카우트 데이도 같이)
...
- [x] **관계도** — 스태프 전원 + 팀동료, 주인공 기준 1:N (−100~100, 7단계 라벨).
      slot.db `relationship` 테이블(마이그레이션 v4) · Rust `relationship.rs` ·
      **구 감정 시스템 838줄 폐기** · 효과 배선(보직 ±6 OVR · 훈련 ±6%p) · 인물 화면
- [x] (별건) 선수 학적 역행 차단 — 대학 두 번 입학이 실제로 가능했다
- [x] (별건) 팀 역사 화면 v1 데이터 드리프트 — 새 게임 팀 선택이 렌더 크래시였다

**R6b3 — 로스터 디테일** (Phase 6.5, 2026-07-30) — 설계 정본 [docs/design/roster.md](docs/design/roster.md)
- [x] 나이 체계 한국 나이 (고1 17세) — **졸업↔입단 1년 구멍** 제거
- [x] 투수 구종 생성 — 생성물에 구종이 **아예 없었다**
- [x] 연봉 = OVR(지수) × 연차 × 나이 × 팀예산 × 리그배수 — 구 선형식은 연차가 무의미
- [x] 로스터를 **전력★에 연동** — 상관계수 고교 −0.05 → 0.92 (명문교와 약팀이 같았다)
- [x] 경력 이력 생성 (입단·이적·FA·트레이드) → `transactions`
- [x] 군경팀(상무) 로스터 — 병역 "미필"인 민간 선수가 상무에 있었다
- [x] (별건) `TEAM_SPORTS_UNIT` ID 드리프트 — 입대자가 없는 팀으로 가고 있었다

**R6c — 커리어·시장·신규 시스템** (= 로드맵 Phase 7) ✅ **완료 2026-07-31**

> ⚠ **이 절이 쓰일 당시엔 해외가 확장팩이었다** (2026-07-30). **2026-08-06에
> 열었다** — 아래 R6f 참고. 그래서 "FA는 국내 이적만"이라는 전제는 더 이상
> 사실이 아니다: FA 시장이 활성 프로 전 리그를 받는다.

- [x] **7-1** 11월 통합 드래프트 + 미지명자 진로 — 설계 [docs/design/draft.md](docs/design/draft.md)
      · 회귀 `npm run test:draft` · 규모 실측 `npm run measure:draft`
      - [x] 졸업 전(W47) 1회로 통합 — 관전 보드가 **실제 결과를 재생**한다.
            예전엔 보드와 시즌 종료가 각각 돌려 거래기록이 두 벌 쌓였다
      - [x] 후보를 졸업 후보 / 소속 유지 신청 후보로 나눔 (대학 저학년·독립 얼리)
      - [x] 지명 순서 역순위 · 라운드 수·신인 계약을 규칙 파일로
      - [x] 상위 라운드는 1군, 나머지는 2군 시작 + 1군 최소 인원 보충
      - [x] 진로 배정을 **미지명·방출·FA 미계약 한 로직으로**. 대학은 고교 졸업자만
      - [x] 로스터 상한 정본화 — 세 곳에 각각 있던 표를 규칙 파일 하나로
      - [x] (별건) 지명자 400명이 refs에 없는 팀 소속이던 문제, 상무에 미지명자가
            배정되던 문제, 대졸 미지명자가 대학에 재입학하던 문제
- [x] **7-2** 프로 진입 시 2군 배치 + 승강 2경로 — 설계 [docs/design/promotion.md](docs/design/promotion.md)
      · 회귀 `npm run test:promotion` · 실측 `npm run measure:promotion`
      - [x] 월간 정기(콜업+콜다운) + 상시 콜업(부상·부진) 2경로
      - [x] 판정에 시즌 성적 반영 — 표본이 적으면 능력치 쪽으로 당긴다
      - [x] 주인공도 강등되고, 강등되면 2군 리그 경기를 직접 뛴다
      - [x] **특급 신인 1군 직행** — `draftRules.firstTeamRounds` 이하는 1군 시작 (§11 해소)
      - [x] (별건) 강등이 리그를 안 바꾸던 문제, 주인공이 프로가 아니면 승강이
            멈추던 문제, 콜다운이 정원과 무관하게 나가 1군이 30→16명으로 마르던 문제
- [x] **7-3** 국가대표 · 병역 면제 — 설계 [docs/design/military.md](docs/design/military.md)
      · 회귀 `npm run test:military`
      - [x] 국제대회 3종 4년 주기 — **한 해에 하나만** 열린다
      - [x] 결과만 합성 (경기 시뮬 없음) → 순위·메달·면제
      - [x] 발탁 시 소속팀 결장 — 부상과 같은 취급이라 상시 콜업이 메운다
      - [x] 상무 정원 제어 (연간 입대 = 정원/복무연수). 40~50% 교체가 여기서 해결
      - [x] (별건) 상무가 정원의 1.5배로 유지되던 문제, 2군이 후보에서 빠져 있던 문제
- [x] **7-4** 방출 2단계 · FA 등급제 · 구단주 관계 — 설계 [docs/design/fa.md](docs/design/fa.md)
      · 회귀 `npm run test:fa`
      - [x] FA 자격 연수 정본화 (네 곳 → 규칙 파일 + 대조 테스트)
      - [x] KBO식 A/B/C 등급 — **보상선수가 실제로 이동한다**
      - [x] 방출 2단계 (정원 안이어도 성적·연봉·뎁스로 판정)
      - [x] 구단주 관계가 방출 점수를 움직인다 (주인공 전용 — 관계도가 1:N이라)
      - [x] 주인공 재계약 UI 경로 · NPC FA 시장 오프시즌 배선 → **7-5 F-4에서 완료**
- [x] **7-5** 개인 재정 전체 · 스태프 15종 배선 · 부상 전조 경고
      — 설계 [docs/design/finance.md](docs/design/finance.md)
      · 회귀 `test:staff` `test:injury` `test:finance` · 실측 `measure:finance`
      - [x] 스태프 15종 전부 소비처 확보. **실측해보니 쓰이던 건 2종이 아니라 1종**이었다
            (이름이 세 벌이라 `JSON.stringify`가 undefined를 지우고 조용히 기본값으로 돌았다)
      - [x] 계수 정본을 `staff_rules.toml [effects]` 하나로. 읽는 곳은 `utils/staffEffects.ts`
      - [x] 부상 전조 — 유예 1주. **훈련 무리는 유예를 안 탄다**(면죄부 방지).
            경고에 다음 주 실제 확률을 담고 회귀가 실측과 대조한다(오차 0.6%p)
      - [x] 개인 재정 — 누진세·분리과세·스폰서 4카테고리·구독(팀 자원 반비례)·투자·사치품.
            **FinancePage는 통째로 다시 썼다** — 화면이 OVR·사기로 수입을 즉석 계산해
            `money`와 무관한 숫자를 보여주고 있었다
      - [x] 7-4 이월 둘 — NPC FA 시장 오프시즌 배선, 구단주 관계의 재계약 소비처
      - [x] (별건) **시설 `tier` 유령** — 국내 NPC 5,600명이 전부 `facility_factor("독립")`
            으로 성장하던 문제. **치료비 단위** — 원 단위를 만원에서 빼 한 주에 자산이 0
- [x] **7-6** 문장 뱅크 엔진 · 뉴스화 · 감정 문구 재작성 · 회귀 `test:sentencebank`
      - [x] 직전 문장 제외 랜덤. 3개 미만이면 회귀 실패(= "빌드 실패"의 구현)
      - [x] 직전 선택을 **세이브에 남긴다** — 안 남기면 로드마다 같은 문장이 나온다
      - [x] 로그뿐이던 국가대표·국제대회·FA 시장에 뉴스를 붙였다
      - [x] 감정 문구 7건 — 트리거를 9축 임계값에서 **관계 라벨 진입**으로.
            **`DecisionEffect.relationDelta`를 먼저 만들었다** — 구 코드의 진짜 결함은
            문구가 아니라 "trust +5"라 적고 사기만 움직이던 표시·동작 불일치였다
      - [x] 선택지 적용 경로를 `usecases/decisions.ts` 하나로 (화면·자동진행 공용)
- [x] **7-7** 대학 쇼케이스 · 올스타전 + **고교 스카우트 데이** · 회귀 `test:campus`
      - [x] 쇼케이스 세 경로(팀 추천·주목도·구단 지명) — 하나면 "이미 아는 이름"만 모인다
      - [x] 올스타 **대학당 캡 2명** + 포지션 쿼터. 선발 점수에 인기도 30%
      - [x] 고교 스카우트 데이 — §A-9의 "신규 기획 필요"를 해소.
            **전용 규칙을 만들지 않고** showcase 규칙을 축소해 쓴다
      - [x] (별건) 로스터 캡이 강등 유입을 재검사 안 하던 문제 (2군 35명)

**R6f — 해외 진출** ✅ **열림 (2026-08-06)**
- [x] `releaseScope.ts`의 두 Set 해제 (리그 + 커리어 단계)
- [x] 해외 로스터 검증 — `diag:roster` 2시즌 실측, 전 리그 상한 안
- [x] FA 해외 진출 경로 — Rust 오퍼는 있었는데 **TS가 목적지 팀을 KBL만 넘겼다**
- [x] 반경 정책 — 국내와 같이 풀 시뮬
- [x] 외국인 용병을 실제 ABL_FARM에서 이적 · 이름 다국어
- [ ] 진출 오퍼 연출 · `pro_abl`/`pro_jbl` 화면 배선 점검
...
- ~~**특급 신인 1군 직행 조건**~~ → **7-1에서 `draftRules.firstTeamRounds`=4로 확정**
- **협상 카운터오퍼 라운드 상한** (§B-7) — OnePitch 구현은 1회로 단순화
- ~~**개인 재정 전 수치**~~ → **7-5에서 `financeRules`로 확정** (2026-07-31)
- **국가대표 발탁 커트라인 · 대회 성과 확률** (§6.2)
- ~~**스태프 15종 각각의 계수**~~ → **7-5에서 `staff_rules.toml [effects]`로 확정** (강도 "중간")
- ~~**고교 유사 이벤트 구체 기획**~~ → **7-7에서 스카우트 데이로 확정.** 전용 계수를 만들지 않고 대학 쇼케이스 규칙을 축소해 쓴다
- **성능 목표 수치** (§10 R6d)
