...
**R6a — 데이터·구조 재정비** (선결)
- [ ] **데이터 관리 규약 확정** — `docs/DATA_POLICY.md`. 정의/상태/파생 3분류, 수정 주체·시점·금지 목록
- [ ] **slot.db 마이그레이션 러너** — 현재 `schema_version`을 쓰기만 하고 읽지 않아 컬럼 추가가 기존 슬롯에 반영 안 됨 (`docs/AUDIT_2026-07.md` B3)
- [ ] 세이브 **v4 클린 브레이크** — 팀 ID 체계 변경으로 기존 슬롯 폐기
- [ ] `season` 단일 JSON 블롭 → `schedule`/`standings`/`season_stats` 테이블 분리 (172팀 규모에서 블롭은 한계)
- [ ] 시드 16파일 → master 포맷 이관 (172팀·27구장·특성·과거순위·타이틀·라이벌·학교·이름풀)
- [ ] `refs.json` 재정본화 — 국내 172 + 해외 24 = 196팀
- [ ] 전력★ CSV 이관 · 182팀 고유색 저작
- [ ] 고아 자산 정리 — 미등록 스크립트 23개, `seeds/v1/` 9파일, `runtime/`
...
- [ ] Node 하네스(`npm run harness`) — 전체 경로, N시즌 무인 진행, 불변식 체크
- [ ] `docs/AUDIT_2026-07.md` 버그 B1~B11 처리
