...
| ~~**해외 — 미진출**~~ | ~~ABL · JBL~~ | **1차 출시 제외 (2026-07-30)** — 비활성. 드리프트도 안 돈다.<br>확장팩에서 `shared/config/releaseScope.ts`의 Set을 비우면 이 행이 살아난다 |
| ~~**해외 — 진출 시**~~ | ~~진출한 리그~~ | 1차 출시 제외 — 확장팩 |
...
> **1차 출시 범위 확정 (2026-07-30)**: 해외 리그(ABL·JBL)와 해외 진출은 **확장팩으로 미룬다.**
> 코드·데이터는 지우지 않고 `shared/config/releaseScope.ts`가 노출만 막는다.
> 그래서 아래에서 "FA·포스팅 해외 진출"이 빠지고, FA는 국내 이적만 다룬다.
