<script lang="ts">
  ...
  } from "../../../shared/stores/autoAdvance";
  ...
  $: filteredEvents = filterKey === "all" ? events : events.filter((e) => e.type === filterKey);
  ...
  function exportLog() {
    const lines = events.flatMap((e) => {
      ...
      const rows = e.players.map(
        (p) => `  ${p.name} | ${p.fromTeamId ?? "-"}→${p.toTeamId ?? "-"} | ${p.detail}`,
      );
    ...
    a.href = url;
    a.download = `player-events-${Date.now()}.txt`;
    a.click();
      ...
      <div class="filter-row">
        {#each FILTER_OPTS as opt}
          <button
            ...
            on:click={() => (filterKey = opt.key)}>{opt.label}</button
          >
        {/each}
      </div>
...
<style>
  ...
  .ev-type.ev-enlist_sports {
    color: #c080f0;
    background: #180a30;
  }
  .ev-type.ev-enlist_general {
    color: #f08060;
    background: #2a1010;
  }
  .ev-type.ev-discharge {
    color: #40d0d0;
    background: #082020;
  }
  .ev-type.ev-retire {
