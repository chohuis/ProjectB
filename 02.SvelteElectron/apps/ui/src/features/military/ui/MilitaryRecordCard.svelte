<script lang="ts">
  ...
  $: convText =
    conv.statDelta === 0 && conv.velocityDelta === 0
      ? "능력치 그대로"
...
<div class="mil-record">
  ...
  <p class="unit">
    {record.unitName}{record.roleLabel ? ` · ${record.roleLabel}` : ""}{record.arcLabel &&
    record.arcLabel !== record.roleLabel
      ? ` → ${record.arcLabel}`
      : ""}
  </p>
  <dl class="kv">
    <dt>전역 때 야구 감각</dt>
    <dd>{record.finalBallSense} — {convText} · 회복 {conv.recoveryWeeks}주</dd>
    <dt>표창 · 징계</dt>
    <dd>{record.awards.length} · {record.penalties.length}</dd>
    <dt>휴가</dt>
    <dd>{record.leaveDays}일</dd>
    ...
    {#if record.topRelations.length > 0}
      ...
      <dd>
        {record.topRelations
          .map(
            (r) => `${r.name} ${r.value > 0 ? "+" : ""}${r.value} ${relationLabel(r.value).label}`,
          )
          .join(" · ")}
      </dd>
    {/if}
