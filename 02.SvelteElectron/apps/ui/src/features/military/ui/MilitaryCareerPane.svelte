<script lang="ts">
  ...
  $: topRel = Object.entries(all)
    ...
    .map(([id, v]) => {
      ...
      return {
        id,
        name: m?.name || m?.rank || id,
        value: Math.round(v),
        frozen: ml.relations[id] === undefined,
        label: relationLabel(v).label,
      };
    });
  // rules.discharge 를 위에서부터 첫 일치 — dischargeConversion 과 같은 규칙
  $: dischargeIdx = Math.max(
    0,
    rules.discharge.findIndex((d) => ml.ballSense >= d.minSense),
  );
...
<div class="grid">
    ...
    {:else}
      <dl class="kv">
        {#each perfDesc as x (x.id + x.week)}
          <dt>W{x.week}</dt>
          <dd>{x.note} <small>tier {x.tier}</small></dd>
        {/each}
      </dl>
    {/if}
    <dl class="kv sum">
      <dt>표창 · 징계</dt>
      <dd>{awards.length} · {penalties.length}</dd>
      {#each awards as a (a.id + a.week)}<dt class="sub">W{a.week}</dt>
        <dd class="sub ok">표창 — {a.id}</dd>{/each}
      {#each penalties as a (a.id + a.week)}<dt class="sub">W{a.week}</dt>
        <dd class="sub bad">징계 — {a.id}</dd>{/each}
      <dt>휴가</dt>
      <dd>{ml.leaveDays}일 누계</dd>
    </dl>
  </div>
  ...
  <div class="card">
    ...
    <p class="hint">
      매주 −{rules.ballSense.weeklyDecay} · 공 카드 {rules.ballSense.gainByAccess.join("/")} (접근 0~3)
      · 휴가 +{rules.ballSense.leaveGain} · 힘든 주 −{rules.ballSense.hardWeekLoss}
    </p>
  </div>
  ...
  <div class="card">
      ...
      <tbody>
        {#each rules.discharge as d, i (d.minSense)}
          <tr class:on={i === dischargeIdx}>
            ...
            <td>
              {d.statDelta === 0 ? "능력치 그대로" : `능력치 ${d.statDelta}`}{#if d.velocityDelta}
                · 구속 {d.velocityDelta}{/if} · 회복 {d.recoveryWeeks}주
            </td>
          </tr>
        {/each}
      </tbody>
    ...
    <p class="hint">
      전역 때 감각 하나로 능력치를 한 번 환산한다 — 복무 중엔 능력치를 안 건드린다.
    </p>
  </div>
    ...
    {:else}
      <dl class="kv">
        {#each topRel as r (r.id)}
          <dt>{r.name}</dt>
          <dd>{r.value > 0 ? "+" : ""}{r.value} {r.label}{r.frozen ? " (전역 · 얼림)" : ""}</dd>
        {/each}
      </dl>
    {/if}
    <p class="hint">
      전역 때 이 탭이 "군 경력 한 장"으로 접혀 나 › 상태 › 기록에 남는다. 탭은 사라진다.
    </p>
  </div>
</div>
