<script lang="ts">
  import type { MilitaryCalendarEntry, MilitaryLifeRules, MilitaryLifeState, MilitaryUnit } from "../../../shared/types/militaryLife";
  ...
  $: next = calendar
    .filter((c) => c.week > week && (c.role === undefined || c.role === null || c.role === ml.roleId))
...
<header class="head">
  <div class="who">
    ...
    <div class="sub">
      ...
      {#if role}
        <span class="chip">{roleLabel}{arcLabel && arcLabel !== roleLabel ? ` · ${arcLabel}` : ""}</span>
      {/if}
  ...
  <div class="bar" role="progressbar" aria-valuemin="0" aria-valuemax={total} aria-valuenow={week} aria-label="복무 진행 {pct}%">
    <i style="width:{pct}%"></i>
  </div>
</header>
...
<style>
  .head {
    ...
    display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 10px 16px; align-items: center;
    color: var(--ink);
  }
  .who { min-width: 0; }
  h1 { margin: 0; font-size: 16px; font-weight: 800; color: var(--t-dark); letter-spacing: -.01em; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  ...
  .meta { color: var(--ink-mute); font-size: 11.5px; margin-left: 4px; }
  .svc { text-align: right; font-variant-numeric: tabular-nums; }
  .svc .big { font-size: 22px; font-weight: 800; color: var(--t-dark); line-height: 1.1; }
  .svc .big small { font-size: 12px; color: var(--ink-mute); font-weight: 600; }
  .svc .next { color: var(--warn); font-weight: 700; font-size: 12px; margin-top: 3px; }
