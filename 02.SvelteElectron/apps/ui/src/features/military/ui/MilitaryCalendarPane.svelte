...
<div class="card">
  ...
  <div class="tl">
    ...
    {#each entries as c, i (c.event + c.week)}
      <span
        class="mk {kindOf(c)}"
        class:past={c.week <= week}
        class:next={next?.week === c.week}
        style="left:{x(c.week)}%"
        title="W{c.week} {c.label}"
      ></span>
      <span
        class="lb {LANES[i % LANES.length]}"
        class:next={next?.week === c.week}
        style="left:{x(c.week)}%">{c.label}<small>W{c.week}</small></span
      >
    {/each}
        ...
        {#each past as c (c.event + c.week)}
          <li>
            ...
            <span>
              ...
              {c.label}{c.leaveDays ? ` — ${c.leaveDays}일` : ""}{#if !done(c)}
                <em class="miss">(안 떴다)</em>{/if}
      ...
      <ul class="log">
        {#each upcoming as c (c.event + c.week)}
          <li class:next={next?.week === c.week}>
            <span class="w">W{c.week}</span>
            <span>
              {c.label}{c.leaveDays ? ` — 휴가 ${c.leaveDays}일` : ""}{c.noChoice
                ? " · 선택 없음"
                : ""}{c.role ? ` · ${c.role === "signal" ? "통신병" : "박격포병"} 전용` : ""}
...
<style>
  ...
  .tl .mk.next {
    box-shadow: 0 0 0 3px rgba(179, 49, 31, 0.25);
  }
  .tl .lb {
    position: absolute;
    font-size: 10.5px;
