...
<div class="wrapper">
  <div class="viewport retro-viewport">
    <svg class="field retro-field" viewBox="0 0 1000 920" preserveAspectRatio="xMidYMid meet">
      ...
      <!-- 존 미리보기 -->
      <rect
        x={Math.round(strikeZoneTarget.x) - 6}
        y={Math.round(strikeZoneTarget.y) - 6}
        width="12"
        height="12"
        fill="rgba(101,213,255,0.82)"
      ...
      {#if isPitching}
        <rect
          x={Math.round(strikeZoneTarget.x) - 14}
          y={Math.round(strikeZoneTarget.y) - 14}
          width="28"
          height="28"
          fill="none"
      ...
      {#each defenders as player}
        ...
        >
          {#if selectedPos === player.pos}
            <rect
              x={player.x - STONE_R - 4}
              y={player.y - STONE_R - 4}
              width={(STONE_R + 4) * 2}
              height={(STONE_R + 4) * 2}
              fill="none"
              stroke="#e8e800"
          ...
          <circle
            ...
            fill={stone.body}
            stroke={stone.rim}
            stroke-width={stone.rimWidth}
          />
          <!-- 광택 — 테두리 안쪽까지만 덮는다 -->
          <circle
            cx={player.x}
            cy={player.y}
            r={STONE_R - stone.rimWidth / 2}
            fill="url(#stone-body)"
            pointer-events="none"
          />
          ...
          <text
            ...
            font-size={player.pos.length > 1 ? 13 : 16}
            font-weight="800"
          ...
          {#if hoveredPos === player.pos}
            ...
            <text
              x={player.x}
              y={player.y - STONE_R - 11}
              text-anchor="middle"
              font-size="11"
              font-family="'Courier New',monospace"
              fill="#e8e8c8">{posLabel[player.pos] ?? player.pos}</text
            >
          {/if}
        </g>
      {/each}
      ...
      {#if batterAnimPos !== null}
        <g>
          <ellipse
            cx={batterAnimPos.x + 1}
            cy={batterAnimPos.y + STONE_R * 0.86}
            rx={STONE_R * 0.92}
            ry={STONE_R * 0.28}
            fill="rgba(0,0,0,0.35)"
          />
          <circle
            ...
            fill={batStone.body}
            stroke={batStone.rim}
            stroke-width={batStone.rimWidth}
          />
          <circle
            cx={batterAnimPos.x}
            cy={batterAnimPos.y}
            r={STONE_R - batStone.rimWidth / 2}
            fill="url(#stone-body)"
            pointer-events="none"
          />
      ...
      {#each ballTrail as pos, i}
        ...
        <rect
          x={Math.round(pos.x) - sz / 2}
          y={Math.round(pos.y) - sz / 2}
          width={sz}
          height={sz}
          fill="#e8d870"
          opacity={((i + 1) / ballTrail.length) * 0.5}
          shape-rendering="crispEdges"
        />
      {/each}
      <rect
        x={Math.round(ballPos.x) - 4}
        y={Math.round(ballPos.y) - 4}
        width="8"
        height="8"
        fill="#f0ecc8"
        stroke="#c8c8a0"
