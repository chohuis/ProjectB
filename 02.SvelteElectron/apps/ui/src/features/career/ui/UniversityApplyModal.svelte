<script lang="ts">
  ...
  import {
    ...
    pctToGrade,
    requirementOfPower,
  } from "../../../shared/utils/universityUtils";
  ...
  $: avgPct = Object.values(subjectScores).length
    ? Object.values(subjectScores).reduce((a, s) => a + s.percentile, 0) /
  ...
  $: teams = $teamsL10n.filter(
    (t) => t.leagueId === "LEAGUE_UNIVERSITY" && t.id !== "TEAM_SPORTS_UNIT",
  );
  // 등급은 팀의 전력★에서 나온다 — 예전엔 하드코딩 표를 뒤졌고 50팀 중 1팀만 맞았다
  const TIER_ORDER: Record<string, number> = { S: 0, A: 1, B: 2, C: 3, D: 4 };
  $: sortedTeams = [...teams].sort(
    (a, b) =>
      TIER_ORDER[requirementOfPower(a.power).tier] - TIER_ORDER[requirementOfPower(b.power).tier],
  );
          ...
          <div class="rows">
            {#each sortedTeams as team}
              ...
              >
                ...
                <span
                  class="elig-status"
                  class:pass={elig.eligible}
                  class:warn={!elig.eligible && (elig.meetsAcademic || elig.meetsBaseball)}
                  class:fail={!elig.meetsAcademic && !elig.meetsBaseball}
                >
