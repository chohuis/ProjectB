<script lang="ts">
  ...
  $: teams = FARM_LEAGUES.filter((lid) => isLeagueInScope(lid))
    .flatMap((lid) => ALL_TEAMS_BY_LEAGUE[lid] ?? [])
    .map((id) => $teamsL10n.find((t) => t.id === id))
    .filter((t): t is NonNullable<typeof t> => !!t);
  $: sortedTeams = [...teams].sort(
    (a, b) =>
      (farmLeagueOf(a.id) ?? "").localeCompare(farmLeagueOf(b.id) ?? "") ||
...
<div class="overlay">
  <section class="modal">
    <header>
      ...
      <p class="mine">
        내 능력 <strong>{myOvr}</strong> · 개인 기여 <strong>{myScore}</strong> · 문턱은 부모 1군 전력★이
        정한다 (★3 → 78 · ★4 → 81 · ★5 → 84)
      </p>
    </header>
          ...
          <div class="rows">
            {#each sortedTeams as team}
              ...
              >
                <strong
                  >{team.name}
                  <small class="lg">{leagueLabel(farmLeagueOf(team.id))} {stars(team.id)}</small
                  ></strong
                >
        ...
        <section class="panel detail">
          {#if selectedTeam}
            <h4>
              {selectedTeam.name}
              <span class="lg"
                >{leagueLabel(farmLeagueOf(selectedTeam.id))} · 1군 {stars(selectedTeam.id)}</span
              >
            </h4>
            <div class="stats">
