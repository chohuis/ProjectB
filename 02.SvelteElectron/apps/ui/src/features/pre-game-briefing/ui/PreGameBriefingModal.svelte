<script lang="ts">
  ...
  import {
    derivePreGameWeather,
    derivePreGamePark,
  } from "../../../shared/utils/matchLineupBuilder";
  ...
  function playerOf(e: EntityRow) {
    return (e.details as EntityDetails)?.player ?? {};
  }
  ...
  function buildLineup(oppTeamId: string, entities: EntityRow[]): EntityRow[] {
    ...
    const pool =
      inTeam.length >= 9
        ? inTeam
        : entities.filter(
            (e) => e.role === "player" && !PITCHER_POS.has(String(playerOf(e).position ?? "")),
          );
  ...
  function findStarter(oppTeamId: string, entities: EntityRow[]): EntityRow | undefined {
    return entities
      ...
      .sort((a, b) => {
        ...
        return da !== db
          ? da - db
          : (playerOf(b).pitching?.ovr ?? 0) - (playerOf(a).pitching?.ovr ?? 0);
      })[0];
  }
  ...
  $: {
    ...
    const m = $masterStore;
    const entry =
      s.schedule.find((e) => e.id === scheduleId) ??
      Object.values(s.leagueSchedules)
        .flat()
        .find((e) => e.id === scheduleId);
    if (!entry) break $;
...
<div class="briefing-wrap" role="dialog" aria-modal="true" aria-label="경기 전 브리핑">
  <div class="briefing-panel">
      ...
      <section class="lineup-section">
        <h3 class="section-title">
          ...
          <span class="team-summary"
            >팀 OVR {teamAvgOvr}
            {#if teamAvgStr !== "-"}
              · 타율 {teamAvgStr} · OPS {teamOpsStr}{/if}
          </span>
        </h3>
    ...
    <footer class="briefing-footer">
      <button class="confirm-btn" type="button" on:click={onClose}> 닫기 </button>
    </footer>
  </div>
</div>
...
<style>
  ...
  .ovr-dot.threat-dot-0 {
    background: var(--panel-sunk);
    color: var(--ink);
  }
  .ovr-dot.threat-dot-1 {
    background: rgba(154, 101, 16, 0.12);
    color: var(--warn);
  }
  .ovr-dot.threat-dot-2 {
    background: rgba(179, 49, 31, 0.09);
    color: var(--bad);
  }
