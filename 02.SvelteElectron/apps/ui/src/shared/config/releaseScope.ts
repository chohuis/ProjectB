 ...
 *
 *  ⚠ **남은 것: 1군 로스터 캡이 해외에 안 걸린다.** JBL 팀당 최대 47명
 *     (상한 32), ABL 39명(상한 34). `roster_rule`은 `_FARM` 폴백까지 있어
 *     규칙을 찾고 캡 루프도 리그 무관하게 도는데 왜 안 잘리는지 아직 모른다.
 *     `fill_first_teams`는 `min`까지만 채우므로 47이 나올 수 없다 —
 *     캡 **이전**부터 넘쳐 있었다는 뜻이다.
 *
