...
export function startMilitaryLife(p: ProtagonistSave): MilitaryLifeState | null {
  ...
  const state = emptyMilitaryLife(
    m.militaryUnit.id,
    senseStartFor(m.militaryLifeRules, p.militaryHiatusStage),
  );
  for (const mem of presentMembers(m.militaryMembers, 0))
...
export async function runMilitaryLifeWeek(args: {
  nextWeek: number;
  seasonYear: number;
}): Promise<MilitaryLifeWeekOut | null> {
  ...
  const state: MilitaryLifeState = {
    ...p.militaryLife,
    relations: { ...p.militaryLife.relations },
    frozen: { ...p.militaryLife.frozen },
    calendarDone: [...p.militaryLife.calendarDone],
    cooldown: { ...p.militaryLife.cooldown },
    choiceLog: [...p.militaryLife.choiceLog],
    awards: [...p.militaryLife.awards],
    penalties: [...p.militaryLife.penalties],
    perf: [...p.militaryLife.perf],
    senseCurve: [...p.militaryLife.senseCurve],
  };
  ...
  for (const mem of members) {
    ...
    if (
      mem.leaveWeek === week &&
      week < rules.serviceWeeks &&
      state.relations[mem.id] !== undefined
    ) {
  ...
  if (band !== prevBand && band === 1) {
    messages.push(
      msg(
        `msg-mil-unit-rank-${args.seasonYear}-w${args.nextWeek}`,
        "일병 진급",
        "이병 딱지를 뗐다. 이제 주간 선택이 조금 더 넓어진다.",
      ),
    );
  }
  ...
  if (state.roleId === null && week >= 6) {
    state.roleId =
      unit.roleAssign === "random"
        ? roleFromSeed(seedOf(worldSeed, "military-role"))
        : unit.roleAssign;
    logs.push(
      `[군] 보직 확정 — ${unit.roles.find((r) => r.id === state.roleId)?.label ?? state.roleId}`,
    );
  }
  ...
  const policyChoice =
    policy === "ball" || policy === "people" || policy === "rest"
      ? policy
      : policy === "mix"
        ? (["ball", "people", "rest"] as const)[week % 3]
        : null;
  ...
  const ctx = {
    ...
    morale: p.morale,
    month,
    defaultCooldown: rules.event.defaultCooldown,
  };
  ...
  const payload = {
    ...
    fatigue: p.fatigue,
    morale: p.morale,
    ballSense: state.ballSense,
    calendarFatigue: cal?.fatigue ?? 0,
    calendarBall: cal?.ballDelta ?? 0,
    onLeave,
    bootCamp,
    members: present.map((x) => ({
      id: x.id,
      relation: state.relations[x.id] ?? x.relationStart,
      sameSubunit: x.subunit === mySubunit,
    })),
    candidateWeights: candidates.map(weightOf),
    fatigueBaseByIntensity: rules.fatigue.baseByIntensity,
    fatigueChoiceBall: rules.fatigue.choice.ball,
    fatigueChoicePeople: rules.fatigue.choice.people,
    fatigueChoiceRest: rules.fatigue.choice.rest,
    fatigueNatural: rules.fatigue.natural,
    fatigueLeave: rules.fatigue.leave,
    moraleChoicePeople: rules.morale.choice.people ?? 0,
    moraleChoiceRest: rules.morale.choice.rest ?? 0,
    moraleLeave: rules.morale.leave,
    senseWeeklyDecay: rules.ballSense.weeklyDecay,
    senseGainByAccess: rules.ballSense.gainByAccess,
    senseCapPerAccessGap: rules.ballSense.capPerAccessGap,
    senseLeaveGain: rules.ballSense.leaveGain,
    relationWeeklyDecay: rules.relation.weeklyDecay,
    relationPeopleByBand: rules.relation.peopleByBand,
    relationSameSubunitWeight: rules.relation.sameSubunitWeight,
    eventWeeklyChance: rules.event.weeklyChance,
  };
  const res = JSON.parse(
    await window.projectB!.engine("weekCalcMilitaryLifeNative", JSON.stringify(payload)),
  ) as RustResult;
  ...
  if (fired) {
    ...
    if (fired.perf) {
      tier = perfTier(
        ...
        graderRelation,
        p.fatigue,
        seedOf(worldSeed, args.seasonYear, args.nextWeek, "military-perf") % 3,
      );
    ...
    seasonStore.pushPendingAction({
      ...
      choices: fired.choices.map((c) => ({
        ...
        effects: {
          ...
          perfTierDelta: c.perfTierDelta,
        } satisfies DecisionEffect,
      })),
    });
    logs.push(`[군] ${fired.title}`);
    messages.push(
      msg(`msg-mil-ev-${fired.id}-${args.seasonYear}-w${args.nextWeek}`, fired.title, description),
    );
  }
  ...
  if (week % 4 === 0) {
    const next = m.militaryCalendar
      .filter(
        (c) =>
          c.week > week && (c.role === undefined || c.role === null || c.role === state.roleId),
      )
      .sort((a, b) => a.week - b.week)[0];
    const targets = res.peopleTargets.map(
      (id) =>
        members.find((x) => x.id === id)?.name || members.find((x) => x.id === id)?.rank || id,
    );
    const top = Object.entries(state.relations)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(
        ([id, v]) =>
          `${members.find((x) => x.id === id)?.name || members.find((x) => x.id === id)?.rank || id} ${Math.round(v)}`,
      );
    const body = [
      ...
      .join("\n");
    messages.push(
      msg(`msg-mil-digest-${args.seasonYear}-w${args.nextWeek}`, "이번 달 부대 소식", body),
    );
  ...
  if (state.choiceLog.length > rules.serviceWeeks)
    state.choiceLog.splice(0, state.choiceLog.length - rules.serviceWeeks);
  state.nextChoice = null;
  const bestPerf = state.perf.length ? Math.min(...state.perf.map((x) => x.tier)) : null;
  const arc = arcStageFor({
    roleId: state.roleId,
    week,
    current: state.arcStage,
    present,
    graderRelation,
    bestPerfTier: bestPerf,
    mySubunit,
  });
  if (arc !== state.arcStage && state.roleId) {
    ...
    messages.push(
      msg(
        `msg-mil-unit-arc-${args.seasonYear}-w${args.nextWeek}`,
  ...
  logs.unshift(
    `군 복무(현역) — ${week}주차${choice === "none" ? (onLeave ? " · 휴가" : bootCamp ? " · 훈련소" : "") : ` · ${CHOICE_LABEL[choice]}`}`,
  );

  // 정수로 저장한다 — 옛 군 갈래(Rust u32)와 같다. 소수로 두면 이벤트 조건·소식이 "68.628…" 을 본다 (C 회신 09-02)
  return {
    patch: {
      fatigue: Math.round(res.fatigue),
      morale: Math.round(res.morale),
      militaryLife: state,
    },
    logs,
    messages,
...
export function applyMilitaryEventChoice(eventId: string, fx: DecisionEffect): void {
  ...
  gameStore.setMilitaryLife(
    applyChoiceToState(p.militaryLife, fx, {
      week,
      event,
      present,
      mySubunit: role?.subunit ?? "",
      ballCap: cap,
    }),
  );
}
