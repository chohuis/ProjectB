...
export function calendarEntryFor(
  calendar: readonly MilitaryCalendarEntry[],
  week: number,
  roleId: MilitaryRoleId | null,
): MilitaryCalendarEntry | null {
  return (
    calendar.find(
      (c) => c.week === week && (c.role === undefined || c.role === null || c.role === roleId),
    ) ?? null
  );
}
...
export function senseStartFor(rules: MilitaryLifeRules, hiatusStage: CareerStage | null): number {
  if (
    ...
    hiatusStage === "pro_jbl" ||
    hiatusStage === "pro" ||
    hiatusStage === "independent"
  ) {
    return rules.ballSense.startPro;
  }
  if (hiatusStage === "highschool" || hiatusStage === "university")
    return rules.ballSense.startStudent;
...
export function conditionPasses(c: MilitaryCondition, ctx: EligibilityCtx): boolean {
  ...
  switch (c.type) {
    case "week_between":
      return ctx.week >= (c.from ?? 1) && ctx.week <= (c.to ?? 100);
    case "rank":
      return ctx.band === num;
    case "role":
      return ctx.roleId === c.value;
    case "relation_gte": {
      const r = rel(c.member);
      return r !== undefined && r >= num;
    }
    case "relation_lte": {
      const r = rel(c.member);
      return r !== undefined && r <= num;
    ...
    case "ballSense_gte":
      return ctx.state.ballSense >= num;
    case "ballSense_lte":
      return ctx.state.ballSense <= num;
    case "fatigue_gte":
      return ctx.fatigue >= num;
    case "fatigue_lte":
      return ctx.fatigue <= num;
    case "morale_gte":
      return ctx.morale >= num;
    case "morale_lte":
      return ctx.morale <= num;
    case "member_present":
      return c.member !== undefined && ctx.present.has(c.member);
    case "season_month":
      return ctx.month === num;
    case "leave_recent": {
      // 최근 n 주 안에 휴가가 있었는가 — choiceLog 에 null(선택 없음) 로 남은 휴가 주를 쓴다
      const n = num;
      return ctx.state.choiceLog.some(
        (l) => l.choice === null && ctx.week - l.week <= n && l.week < ctx.week,
      );
    }
    default:
      return false;
...
export function isEligible(e: MilitaryLifeEvent, ctx: EligibilityCtx): boolean {
  ...
  if (last !== undefined && ctx.week - last < (e.cooldownWeeks ?? ctx.defaultCooldown))
    return false;
...
export function eligibleEvents(
  events: readonly MilitaryLifeEvent[],
  ctx: EligibilityCtx,
): MilitaryLifeEvent[] {
  ...
  if (fx.perfTierDelta && ctx.event) {
    for (let i = next.perf.length - 1; i >= 0; i--) {
      if (next.perf[i].id === ctx.event.id) {
        next.perf[i] = {
          ...next.perf[i],
          tier: Math.max(1, Math.min(6, next.perf[i].tier + fx.perfTierDelta)),
        };
        break;
      }
    }
...
export function arcStageFor(ctx: ArcCtx): number {
  ...
  if (ctx.roleId === "mortar") {
    ...
    if (
      stage < 1 &&
      ((ctx.week >= 35 && !gunnerPresent && ctx.graderRelation >= 10) || ctx.week >= 45)
    )
      stage = 1;
    if (
      stage === 1 &&
      ((ctx.week >= 61 &&
        ((ctx.bestPerfTier !== null && ctx.bestPerfTier <= 4) || ctx.graderRelation >= 20)) ||
        ctx.week >= 75)
    )
      stage = 2;
    if (
      stage === 2 &&
      ctx.week >= 85 &&
      ctx.present.some((m) => m.role === "junior" && m.subunit === ctx.mySubunit)
    )
      stage = 3;
...
export function dischargeConversion(
  rules: MilitaryLifeRules,
  ballSense: number,
): { statDelta: number; velocityDelta: number; recoveryWeeks: number } {
  const row =
    rules.discharge.find((d) => ballSense >= d.minSense) ??
    rules.discharge[rules.discharge.length - 1];
  return {
    statDelta: row.statDelta,
    velocityDelta: row.velocityDelta ?? 0,
    recoveryWeeks: row.recoveryWeeks,
  };
}
