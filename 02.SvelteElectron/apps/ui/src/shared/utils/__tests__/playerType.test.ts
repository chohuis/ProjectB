...
describe("무대·리그 배열 조건", () => {
  ...
  it("career_stage — stages 배열", () => {
    ...
    expect(
      evaluateCondition(c, at({ careerStage: "highschool" } as Partial<ProtagonistSave>)),
    ).toBe(false);
  });
  ...
  it("league_id — leagueIds 배열", () => {
    ...
    expect(evaluateCondition(c, at({ leagueId: "LEAGUE_KBL" } as Partial<ProtagonistSave>))).toBe(
      false,
    );
  });
  ...
  it("단일 값은 그대로 동작한다 — 기존 데이터가 안 깨진다", () => {
    expect(
      evaluateCondition(
        { type: "career_stage", stage: "pro_kbl" } as unknown as Condition,
        at({ careerStage: "pro_kbl" } as Partial<ProtagonistSave>),
      ),
    ).toBe(true);
    expect(
      evaluateCondition(
        { type: "league_id", leagueId: "LEAGUE_KBL_FARM" } as unknown as Condition,
        at({ leagueId: "LEAGUE_KBL_FARM" } as Partial<ProtagonistSave>),
      ),
    ).toBe(true);
  });
});
