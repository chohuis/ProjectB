...
describe("선택지 단위 조건", () => {
  ...
  it("조건을 통과한 선택지만 보인다", () => {
    ...
    expect(
      run(d, proto({ money: 1000 })).newMessages[0].decision?.options.map((o) => o.id),
    ).toEqual(["buy", "pass"]);
    expect(run(d, proto({ money: 100 })).newMessages[0].decision?.options.map((o) => o.id)).toEqual(
      ["pass"],
    );
  });
  ...
  it("본문도 없고 선택지도 다 닫히면 아예 안 나간다", () => {
    ...
    const r = runEventEngine(
      [RULE],
      [],
      new Map([["MSG_T", noBody]]),
      new Map([
        [
          "DEC_T",
          dec([{ id: "x", label: "가", conditions: [{ type: "money_gte", value: 99999 }] }]),
        ],
      ]),
      ctxOf(proto({ money: 10 })),
      2026,
      0,
      [0.5],
      TIER_RULES,
