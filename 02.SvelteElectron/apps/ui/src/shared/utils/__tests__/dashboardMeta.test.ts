...
import {
  standingsTableMeta,
  pitcherSeasonTableMeta,
  contractTableMeta,
  gameResultsTableMeta,
  myGameTableMeta,
  tradeTableMeta,
  playerListTableMeta,
  lockNoteOf,
...
describe("순위표 (msg-digest-)", () => {
  ...
  it("모르는 팀만 변동이 없다 — 나머지는 그대로 그린다", () => {
    const md = standingsTableMeta({
      rows,
      myTeamId: "B",
      prevRankOf: (t) => (t === "A" ? 2 : undefined),
    });
  ...
  it("무승부가 한 팀도 없으면 무 열을 안 보낸다", () => {
    ...
    const withDraw = standingsTableMeta({
      rows: [team("A", 3, 1, 2), team("B", 2, 2)],
      myTeamId: "A",
    });
...
describe("시즌 성적 (msg-pro-season-end- · msg-indie-season-end-)", () => {
  it("여덟 항목이 항목·값 두 칸으로 갈린다", () => {
    const md = pitcherSeasonTableMeta(
      "seasonEndPro",
      stats({ w: 12, l: 7, sv: 0, hd: 3, ip: 168.6667, era: 3.21, whip: 1.18, k: 141 }),
    );
...
describe("묶음 2 — 경기·트레이드·말소·웨이버", () => {
  ...
  it("내 경기 — 연습경기는 투구수 줄이 없다", () => {
    const off = myGameTableMeta("officialResult", {
      ip: 6.3333,
      h: 5,
      er: 2,
      k: 7,
