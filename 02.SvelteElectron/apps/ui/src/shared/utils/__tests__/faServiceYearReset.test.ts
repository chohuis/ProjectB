...
const MARKET = readFileSync(
  join(__dirname, "../../usecases/weekPhases/market.ts"), "utf8");

describe("FA 신청이 통산 연차를 지우지 않는다", () => {
  it("`proServiceYears: 0`으로 되돌리는 코드가 없다", () => {
    expect(MARKET, "FA 신청에서 연차를 0으로 리셋하는 코드가 살아 있다")
      .not.toMatch(/proServiceYears:\s*0\s*,/);
  });

  it("리셋 대신 재취득 기간을 본다", () => {
    expect(MARKET, "canReacquireFa 판정이 없다 — 리셋을 빼면 매년 FA가 된다")
      .toMatch(/canReacquireFa\(/);
  });

  it("Rust의 FA_REACQUIRE_YEARS와 값이 같다", () => {
    const rust = readFileSync(
      join(__dirname, "../../../../../../packages/engine-native/src/npc_sim.rs"), "utf8");
