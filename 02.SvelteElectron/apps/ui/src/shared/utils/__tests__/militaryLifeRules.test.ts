...
const rules = JSON.parse(
  readFileSync(resolve(ROOT, "resource/data/master/military/rules.json"), "utf8"),
) as MilitaryLifeRules;
const members = JSON.parse(
  readFileSync(resolve(ROOT, "resource/data/master/military/members.json"), "utf8"),
) as MilitaryMember[];
const calendar = JSON.parse(
  readFileSync(resolve(ROOT, "resource/data/master/military/calendar.json"), "utf8"),
);
...
const ctxOf = (week: number, over: Partial<Parameters<typeof isEligible>[1]> = {}) => ({
  week,
  band: week <= 8 ? 0 : week <= 34 ? 1 : week <= 60 ? 2 : 3,
  roleId: "mortar" as const,
  state: emptyMilitaryLife("U", 60),
  present: new Set(presentMembers(members, week).map((m) => m.id)),
  fatigue: 50,
  morale: 60,
  month: 6,
...
describe("이벤트 후보 거르기 (§28)", () => {
  ...
  it("조건 어휘 전부 — 관계·감각·피로·사기·주·달·재적·역할", () => {
    ...
    expect(
      isEligible(ev({ conditions: [{ type: "relation_lte", member: "MEM_1SG", value: -25 }] }), c),
    ).toBe(true);
    expect(
      isEligible(ev({ conditions: [{ type: "relation_gte", member: "MEM_1SG", value: 0 }] }), c),
    ).toBe(false);
    ...
    expect(isEligible(ev({ conditions: [{ type: "week_between", from: 40, to: 48 }] }), c)).toBe(
      true,
    );
    expect(isEligible(ev({ conditions: [{ type: "week_between", from: 41, to: 48 }] }), c)).toBe(
      false,
    );
    expect(isEligible(ev({ conditions: [{ type: "season_month", value: 6 }] }), c)).toBe(true);
    expect(
      isEligible(ev({ conditions: [{ type: "member_present", member: "MEM_JR_1" }] }), c),
    ).toBe(true);
  ...
  it("eligibleEvents 는 순서를 지킨다 (가중 인덱스가 Rust 와 맞물린다)", () => {
    const list = [
      ev({ id: "a", calendar: true }),
      ev({ id: "b" }),
      ev({ id: "c", roleTag: "signal" }),
      ev({ id: "d" }),
    ];
...
describe("선택지 효과 → 상태 (§28)", () => {
  ...
  it("relationDelta 대상 — 이벤트의 member 가 기본 · all · subunit · junior", () => {
    const s = base();
    const a = applyChoiceToState(
      s,
      { memberRelationDelta: 8 },
      { week: 10, event: ev({ member: "MEM_SQ_LDR" }), present, mySubunit: "SQ1", ballCap: 80 },
    );
    expect(a.relations.MEM_SQ_LDR).toBe(8);
    expect(a.relations.MEM_CO).toBe(0);
    const b = applyChoiceToState(
      s,
      { memberRelationDelta: 2, relationTarget: "all" },
      { week: 10, event: null, present, mySubunit: "SQ1", ballCap: 80 },
    );
    expect(Object.values(b.relations).every((v) => v === 2)).toBe(true);
    const c = applyChoiceToState(
      s,
      { memberRelationDelta: 3, relationTarget: "subunit" },
      { week: 10, event: null, present, mySubunit: "SQ1", ballCap: 80 },
    );
  ...
  it("ballDelta 는 상한을 넘지 않고 · 상벌·휴가는 쌓이고 · perfTierDelta 는 그 이벤트의 마지막 성과에만", () => {
    ...
    s.ballSense = 78;
    s.perf = [
      { week: 5, id: "X", tier: 3, note: "" },
      { week: 9, id: "Y", tier: 4, note: "" },
    ];
    const n = applyChoiceToState(
      s,
      { ballDelta: 6, award: "A1", leaveDays: 4, perfTierDelta: -1 },
...
describe("성과 tier · 보직 아크 (§28 · §38)", () => {
  ...
  it("박격포병 아크 — 사수 자리가 비고 관계가 있으면 W35 부사수 · 늦어도 W45 · 사수는 W61/W75 · 대행은 후임 있어야", () => {
    const at = (week: number, current: number, rel: number, best: number | null) =>
      arcStageFor({
        ...
        week,
        current,
        present: presentMembers(members, week),
        graderRelation: rel,
        bestPerfTier: best,
        mySubunit: "SQ1",
      });
  ...
  it("문안 자리표시자", () => {
    expect(
      fillText("{unit.name} — {role.label} {tier}등급", {
        "unit.name": "중대",
        "role.label": "통신병",
        tier: 2,
      }),
...
describe("전역 (§30)", () => {
  it("환산은 감각 구간으로 — 80 이상 손실 0 · 회복 2주 · 20 미만 −4 에 구속 −1 · 회복 10주 (rules 정본)", () => {
    expect(dischargeConversion(rules, 92)).toEqual({
      statDelta: 0,
      velocityDelta: 0,
      recoveryWeeks: 2,
    });
    expect(dischargeConversion(rules, 80)).toEqual({
      statDelta: 0,
      velocityDelta: 0,
      recoveryWeeks: 2,
    });
    expect(dischargeConversion(rules, 79.9).statDelta).toBe(-1);
    expect(dischargeConversion(rules, 47)).toEqual({
      statDelta: -2,
      velocityDelta: 0,
      recoveryWeeks: 6,
    });
    expect(dischargeConversion(rules, 5)).toEqual({
      statDelta: -4,
      velocityDelta: -1,
      recoveryWeeks: 10,
    });
  ...
  it("군 경력 한 장 — 관계 상위 셋은 전역한 부대원(frozen)도 센다 · 원본 불변", () => {
    ...
    const unit = {
      ...
      name: "중대",
      roles: [
        { id: "mortar" as const, label: "박격포병" },
        { id: "signal" as const, label: "통신병" },
      ],
    };
    ...
    expect(rec.topRelations.map((r) => r.memberId)).toEqual([
      "MEM_AMMO_PR",
      "MEM_GUNNER",
      "MEM_SQ_LDR",
    ]);
