...
describe("뽑기 — 직전 것을 뺀다", () => {
   ...
   *
   * ⚠ **이 수열은 균등 난수가 아니다 — 은행 12 에서 한 자리를 굶긴다.**
   *   곱수 37 · 모듈러 100 이 직전 제외(후보 11)와 만나 닫힌 고리를 만든다:
   *   `k=4` 가 나오는 순간의 `last` 가 언제나 0 이고, `k=3` 이 나오는 순간의
   *   `last` 가 언제나 11 이라 **자리 4 에 닿을 조합이 영영 안 온다**
   *   (2026-09-08 실측 · 곱수를 7·17·31·41 로 바꾸면 열둘 다 닿는다).
   *
   *   `pickSentence` 는 멀쩡하다 — `__tests__/sentenceBankCoverage.test.ts` 가
   *   은행 2~24 에서 모든 자리에 닿는 것을 못박는다. 실게임 난수는 Rust 것이라
   *   화면 결함도 아니다. **은행 크기가 12 가 되면 이 줄을 의심해라.**
   */
