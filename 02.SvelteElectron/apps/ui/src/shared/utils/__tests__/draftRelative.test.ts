...
describe("주인공 드래프트 산식", () => {
  ...
  it("애매하면 미지명이다 — 문턱이 실측 지명률에서 온다", () => {
    ...
    // ①). ⑭ 뒤 고졸 직행이 8.3%(목표 30~40%)로 떨어져 문턱을 다시 쟀다 —
    // 같은 12칸에서 78.0→1/12, 68.0→4/12(목표 안). 라운드 직선의 앵커는
    // 78.0 그대로다(아래) — 68~78점 구간은 11R(끝라운드) 턱걸이가 된다.
    expect(rust).toMatch(/const UNDRAFTED_SCORE: f64 = 68\.0;/);

    // 라운드 직선 앵커는 78.0에 그대로 있다 — 문턱(68.0)과는 이제 일부러
    // 다르다. 78점에서 11R이라는 계산 자체는 안 바뀌었다(앵커를 안 건드림)
    const round = (s: number) => 11.0 - (s - 78.0) * 0.4;
