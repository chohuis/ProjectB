...
import { buildForeignSeed, pickOriginLeague, SIGNING_SPREAD, type SeedPlayer } from "../foreignSeed";
...
const pl = (npcId: string, svc = 0): SeedPlayer =>
  ({ npcId, name: `p${npcId}`, teamId: "TEAM_KBL_A_1", proServiceYears: svc });
...
describe("영입 시드", () => {
  it("⚠ 팀을 지어내지 않는다 — 리그만 적는다", () => {
    ...
    const rows = buildForeignSeed({
      players: [pl("N1")], rules: RULES, seasonYear: 2026, rand: seeded(1),
    });
  ...
  it("영입 연도가 흩어진다 — 전원 올해가 아니다", () => {
    const rows = buildForeignSeed({
      ...
      rules: RULES, seasonYear: 2026, rand: seeded(3),
    });
  ...
  it(`연차가 커도 ${SIGNING_SPREAD}년 넘게 안 거슬러 간다 — 용병은 단년 계약이다`, () => {
    const rows = buildForeignSeed({
      players: [pl("A", 30)], rules: RULES, seasonYear: 2026, rand: seeded(4),
    });
  ...
  it("한 사람당 한 줄", () => {
    const rows = buildForeignSeed({
      players: [pl("A"), pl("B"), pl("C")], rules: RULES, seasonYear: 2026, rand: seeded(5),
    });
  ...
  it("규칙이 없으면 한 줄도 안 만든다", () => {
    expect(buildForeignSeed({
      players: [pl("A")], rules: { weights: {}, returnLeague: "" },
      seasonYear: 2026, rand: seeded(6),
    })).toEqual([]);
  });
});
...
describe("떠난 용병", () => {
  ...
  it("⚠ 기록만 만든다 — 작년 일이라 그 사람은 세계에 없는 게 맞다", () => {
    const rows = buildForeignSeed({
      players: [], rules: RULES, seasonYear: 2026, rand: seeded(7),
      departed: 2, departedNames,
    });
  ...
  it("이름이 모자라면 있는 만큼만 — 없는 이름을 지어내지 않는다", () => {
    const rows = buildForeignSeed({
      players: [], rules: RULES, seasonYear: 2026, rand: seeded(8),
      departed: 10, departedNames,
    });
    expect(rows).toHaveLength(2);
  });
  ...
  it("이름이 없으면 안 만든다", () => {
    expect(buildForeignSeed({
      players: [], rules: RULES, seasonYear: 2026, rand: seeded(9), departed: 5,
    })).toEqual([]);
  });

  it("떠난 사람 ID가 실제 선수와 안 겹친다", () => {
    const rows = buildForeignSeed({
      players: [], rules: RULES, seasonYear: 2026, rand: seeded(10),
      departed: 2, departedNames,
    });
...
describe("결정성", () => {
  it("같은 시드면 같은 기록 — 같은 세계를 다시 열면 같아야 한다", () => {
    const make = () => buildForeignSeed({
      players: [pl("A"), pl("B"), pl("C")],
      rules: RULES, seasonYear: 2026, rand: seeded(42),
    });
