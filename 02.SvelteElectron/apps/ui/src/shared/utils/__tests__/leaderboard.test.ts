...
describe("순위 매기기", () => {
  ...
  it("10이닝 0점대가 ERA 1위를 뺏지 못한다", () => {
    const rows = withQual(
      [pit("ace", { ip: 180, era: 2.14 }), pit("cameo", { ip: 10, era: 0.0 })],
      q,
    );
  ...
  it("마무리는 규정이닝을 못 채워도 세이브 1위가 된다", () => {
    ...
    const rows = withQual([pit("closer", { ip: 64, sv: 34 }), pit("ace", { ip: 180, sv: 0 })], q);
  ...
  it("도루왕도 같은 이유로 자격이 없다", () => {
    const rows = withQual([bat("runner", { pa: 120, sb: 47 }), bat("slug", { pa: 600, sb: 2 })], q);
    expect(rankBy(rows, categoryByKey("sb")!)[0].id).toBe("runner");
  });

  it("타율은 규정타석을 채운 선수만", () => {
    const rows = withQual(
      [bat("reg", { pa: 500, avg: 0.312 }), bat("part", { pa: 40, avg: 0.5 })],
      q,
    );
  ...
  it("동률은 ID로 갈라 순서가 흔들리지 않는다", () => {
    const rows = withQual(
      [pit("zz", { ip: 120, w: 15 }), pit("aa", { ip: 120, w: 15 }), pit("mm", { ip: 120, w: 15 })],
      q,
    );
  ...
  it("limit이 0이면 전부 준다 — 전체표가 쓴다", () => {
    const rows = withQual(
      Array.from({ length: 30 }, (_, i) => pit(`p${i}`, { ip: 120, k: i })),
      q,
    );
