...
/**
 * 통지 표시 (C-1 · 2026-09-21 · `PLAN_MESSAGE_LANES_2026-09-08.md` · `TIER_PRESENTATION §5`).
 *
 * 🔴 **등급이 아니다.** 통지는 주사위가 아니라 **세계의 상태**가 부르고, 고르면
 *   세계가 실제로 바뀐다(콜업·강등·우승). 그런데 지금까지 칩이 하나도 없어
 *   목록에서 **노말과 겉보기가 같았다** — 큰 순간이 매주 오는 것 사이에 묻혔다.
 *
 * ⚠ **등급 칩과 같은 꼴을 쓰지 않는다.** 등급 셋은 「테두리 + 제 색(파랑·보라·
 *   금)」이라, 통지에 네 번째 색을 주면 **레어보다 위인가 아래인가**를 묻게 된다.
 *   통지는 그 사다리에 없다. 그래서 두 가지를 다르게 한다:
 *
 *   - **색조가 없다.** 지면색(`--ink`)을 채워 쓴다 — 어느 등급색과도 안 겹치고,
 *     색이 안 보이는 화면에서도 등급 사다리로 안 읽힌다. 값을 여기 hex 로 안
 *     적는 이유가 그것이다(등급 셋은 색이 곳 자체라 적는다).
 *   - **채운 꼴이다.** 등급은 테두리, 분류는 맨 글자, 통지는 채움 — 셋이
 *     **모양으로** 갈린다. 흑백에서도 선다.
 *
 * ⚠ 등급과 **배타**다(`eventEngine.noticeOf` 가 등급 추첨 밖에 둔다). 한 소식에
 *   둘이 같이 실릴 일이 없으므로 칩도 하나만 그린다.
 */
export const NOTICE_LABEL = "통지";
export function isNotice(lane: string | undefined | null): boolean {
  return lane === "notice";
}
