...
export function entityToProNpcState(entity: EntityRow, seasonYear: number): NpcSaveState {
  const pl = entity.details?.player;
  const isMilitary = entity.militaryStatus === "현역" || pl?.militaryStatus === "현역";
  ...
  const proTeamId = isMilitary ? (clubId ? clubToFirstTeam(clubId) : entity.teamId) : entity.teamId;
...
export async function advanceAllAges(npcs: NpcSaveState[]): Promise<NpcSaveState[]> {
  const namedFlags = new Map(npcs.map((n) => [n.npcId, n.isNamed] as const));
