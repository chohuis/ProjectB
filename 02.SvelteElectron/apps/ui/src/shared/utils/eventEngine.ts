...
export const eventFunnelStats = {
  ...
  tier: {
     ...
     *
     * ⚠ **지금 정본 데이터에는 시즌 상한이 없다**(2026-09-08 ·
     *   `tier_rules.json` `_seasonCapDoc`). 상한을 아무도 안 걸면 이 값은
     *   늘 0 이고 **아무것도 안 지킨다** — 지키는 것이 생기는 것은 규칙
     *   JSON 이 `seasonCap` 을 다시 들 때뿐이다. 그래서
     *   `check:tiercoverage` 는 이걸 규칙 줄에서 뺐다(상한이 선언된
     *   판에서만 말한다).
     */
    capViolation: 0,
