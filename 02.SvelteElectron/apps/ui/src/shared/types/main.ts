...
export interface DecisionEffect {
  ...
  /**
   * 구종 등급 올리기 (상한 5). 없는 구종이면 조용히 넘긴다.
   *
   * 🔴 **`steps` 가 없으면 히든이 유니크와 종류가 같아진다** (2026-09-09 ·
   *   사용자 확정 · B 제보). 보상안 §1 은 히든이 「구종 등급 **두 단계**」인데,
   *   효과 객체는 같은 키를 둘 둘 수 없어서 **한 번에 +1 이 상한**이었다.
   *   그대로 두면 데이터에 두 단계를 적을 방법이 없고, 적어 봐야
   *   **한 단계만 먹고 조용히 넘어간다** — 이 저장소가 반복해 겪은 형태다.
   *
   * ⚠ **없으면 1 이다** — 지금 데이터를 안 고쳐도 그대로 돈다.
   * ⚠ 상한 5 를 넘겨 달라고 해도 5 에서 멈춘다. 그때 **몇 단계가 실제로
   *   올랐는지**는 `__tests__/pitchGradeSteps.test.ts` 가 못박는다.
   */
  pitchGradeUp?: { id: string; steps?: number };
...
export interface TableMetadata {
  ...
  footnote?: string;
  /**
   * 문안의 각주 틀에 채울 값 — 인센티브 정산의 「합계 +{total}만원」이 그
   * 자리다.
   *
   * 🔴 **문장을 소식에 굳혀 보내지 않는다.** 생산부는 숫자만 싣고 틀은
   *    문안이 갖는다 — `lockNote`(「{weeks}주간 재등록 불가」)가 먼저 그은
   *    선이다. `footnote` 를 직접 실어 보내면 그게 이긴다.
   */
  footnoteVars?: Record<string, string | number>;
  /**
   * 표 아래 붙는 **두 번째 표**. 계약 완료 소식이 「조건」(항목·값) 아래
   * 「인센티브」(항목·금액)를 다는 자리다 — 열이 아예 달라 한 표에
