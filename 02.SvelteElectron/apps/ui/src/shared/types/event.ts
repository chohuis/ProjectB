...
export type Condition =
   ...
   *
   * 사용자 지적: 「기존에 배우고 있는 것만 경험치로 뜨거나, 새로 배우는 건
   * 기존에 배우는 게 안 뜨거나 하는 게 있어야 하지 않나」.
   *
   * ⚠ **효과 키로 분기하지 않는다 — 조건으로 후보를 가른다.** 키로 갈랐다면
   *   「배우는 중인데 새 구종을 받아 배우던 게 사라진다」와 「안 배우는데
   *   진행도 점프를 받아 아무 일도 안 난다」가 둘 다 남는다. 후보 자체를
   *   갈라야 어느 쪽도 안 생긴다:
   *
   * ```
   *   pitch_learning: true   → 진행도 점프 · 등급업 이벤트가 후보
   *   pitch_learning: false  → 새 구종 부여 이벤트가 후보
   * ```
   *
   ...
   *
   * 조건 타입 50종을 세었더니 **등판 수를 보는 조건이 전체에서 `stats.g`
   * 단 1회**였다(`num_gte` 로 경로를 직접 짚은 것 하나). 사용자가 말한
   * 「시즌에 경기를 얼마나 뛰었는지」를 **잴 수가 없었다.**
   *
   * 이닝(`season_ip_gte`)으로 대역이 안 된다 — 한 경기 완투와 다섯 경기
   * 짧은 등판이 같은 이닝인데 **「자리를 잡았나」는 정반대**다. 통지가
   * 물어야 하는 것은 뒤쪽이다(콜업·보직·명단은 횟수가 정한다).
   *
   * ⚠ **타자도 읽는다.** `g` 는 투수 표에도 타자 표에도 있다(등판/출전).
   *   `gs`(선발)는 투수에만 있으므로 타자면 늘 거짓이다.
   ...
   * `PLAN_MESSAGE_LANES`). **통지가 서는 다리다.**
   *
   * 지금까지 통지 성격의 이벤트가 조건으로 쓸 수 있는 것은 무대·주차·사기뿐이라
   * 「1군에서 내려왔습니다」가 **조건이 2군소속 + 2주차**였다 — 처음부터 2군인
   * 신인에게도 떴다. 「구단이 창단 첫 우승을 했습니다」는 **순위를 안 봤다.**
   *
   * 신호는 이미 났는데 **읽는 자리가 없었다.** `protagonist.recentOutcomes` 가
   * 그 자리이고 이 조건이 그것을 읽는다.
   *
   * `weeks` 는 **몇 주 안**인가다. 시즌을 넘어도 세도록 `year × 52 + week` 로
   * 잰다 — `ctx.seasonYear` 가 없는 옛 경로에서는 **같은 시즌만** 본다
   * (모르는 해를 지어내는 것보다 좁게 보는 편이 낫다).
   *
   * ⚠ `weeks: 0` 은 「이번 주」다. 통지 대부분이 그것을 원한다.
   */
  | { type: "outcome_within"; outcome: import("./save").OutcomeKind; weeks: number };
 ...
 *
 * `type`은 "어떻게 발동하는가"(달력·조건·확률)이고 이건 **"얼마나 중요한가"**였다.
 ...
 *
 * 🔴 **`important`·`ambient` 는 없어졌다** (2026-09-08). 그 둘이 하던 일
 * (「놓치면 끝」을 먼저 준다 · 「반복되는 것」은 남는 칸)은 **등급 줄기가
 * 통째로 대신한다**(`EventGrade` · §1). 데이터에도 하나도 안 남았다(B 4-2).
 *
 * | 남은 값 | 뜻 |
 * |---|---|
 * | `urgent` | 등급 줄기 **밖** · 주당 1건 상한을 안 탄다. 부상·수술·방출·트레이드 통보 |
 * | `notice` | **통지** — 등급 줄기 밖 · 주당 1건 상한을 안 탄다 · **상태가 부르고 상태를 바꾼다**(아래) |
 *
 * ⚠ **추론도 없어졌다.** 예전엔 비워 두면 `oncePolicy`로 추론했는데
 * (「`repeatable`이면 `ambient`」), 그건 「발동 정책 = 중요도」라는 틀린
 * 전제였다. 지금 등급을 안 적은 규칙은 **등급 줄기를 아예 안 탄다** —
 * 필수 101종이 그렇고, 그 밖에는 있으면 안 된다(`check:tiers` 가 본다).
 */
/**
 * 🔴 **통지 갈래** (2026-09-08 · `PLAN_MESSAGE_LANES_2026-09-08.md` · L3).
 *
 * > **주사위가 부른 것은 상태를 못 바꾸고, 상태가 부른 것만 상태를 바꾼다.**
 *
 * 발단은 테스터 증언이다 — 「2군으로 가라는 메시지가 왔고 **가겠다고 했는데도
 * 안 내려갔다**」. 훑어 보니 무대 이동을 말하는 이벤트 23건 중 **상태를 바꾸는
 ...
 * | 갈래 | 부르는 것 | 고르면 | 등급 |
 * |---|---|---|---|
 * | **이벤트** | 주사위(주 1건 추첨) | **성장**이 바뀐다 | 넷 |
 * | **안내** | 달력·코드 | 없거나 반응만 | 없다 |
 * | **통지** | 세계의 **상태** | **세계**가 바뀐다 | **없다** |
 *
 * ⚠ **`urgent` 는 통지의 옛 이름이다.** 하는 일이 이미 같다 — 등급 줄기 밖 ·
 *   주 1건 상한 밖 · 부상·방출·트레이드처럼 「일어난 것」. 둘을 다른 갈래로
 *   두면 **정본이 둘**이 되어 한쪽만 고쳐진 채 남는다(이 저장소가 반복해 겪은
 *   형태다). 그래서 **레인은 하나**이고 이름만 둘이다 — `noticeOf()` 가 둘 다
 *   참으로 본다. 데이터의 `urgent` 를 `notice` 로 바꾸는 것은 B 몫(L4)이고,
 *   바꾸기 전에도 동작은 같다.
 */
export type EventTier = "urgent" | "notice" | "important" | "ambient";

/** 소식이 탄 갈래 — 소식함에 **스냅샷으로** 실린다(`MessageItem.lane`) */
export type MessageLane = "notice";
...
export interface EventContext {
  ...
  /**
   * 🔴 **내 무대의 시즌이 열렸나** (2026-09-08 · B 제보 ②).
   *
   * 고교 1~7주에 **폴백이 11번** 났다(레어 1~4주 · 유니크 1~7주). 개막 전이라
   * 레어·유니크에 후보가 하나도 없어서 추첨이 매번 한 등급 아래로 내려간 것이다.
   *
   * ⚠ **데이터가 모자란 게 아니다.** 「개막도 안 했는데 유니크가 뜨는」 쪽이
   *   더 이상하다 — 그 주의 레어·유니크는 **없는 것이 맞다**(B 판단 · 동의).
   *   그러니 채울 것은 데이터가 아니라 **그 주에 추첨을 돌리는 쪽**이다.
   *
   * ⚠ `seasonPhase` 로는 못 가른다 — 그 값은 **그 주 일정 항목의 phase** 이고
   *   경기가 없는 주는 항목이 없어 `"season"` 으로 떨어진다(`advanceWeek` 의 `??`).
   *   개막 전이 정확히 그 주들이다.
   *
   * 없으면(구 경로) **열린 것으로 본다** — 모르는 것을 「안 열렸다」로 읽으면
   * 레어·유니크가 통째로 사라진다. 조용히 지우는 쪽이 늘 더 나쁘다.
   */
