...
function around(center: number, spread: number, seed: number, salt: number): number {
  const a = rand01(seed, salt),
    b = rand01(seed, salt + 7919);
...
function pitcherLine(
  ...
  const era = Math.max(1.2, Math.min(9.99, around(4.5 - (ovr - 68) * 0.068, 1.1, seed, 2)));
...
function batterLine(
  ...
  const avg = Math.max(0.15, Math.min(0.4, around(0.26 + (ovr - 68) * 0.0023, 0.03, seed, 2)));
