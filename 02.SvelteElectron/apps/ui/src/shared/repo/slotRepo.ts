...
export const slotRepo = {
  ...
  getStaff: (
    slotId: string,
    filter: { teamId?: string; role?: string; leagueId?: string; status?: string } = {},
  ...
  setProtagonist: (slotId: string, data: unknown) =>
    call<{ ok: true }>("setProtagonist", { slotId, data }),
  ...
  saveHistoryLeague: (p: {
    ...
    year: number;
    leagueId: string;
    kind: "standings" | "leaders" | "postseason" | "awards";
    data: unknown;
  ...
  getTransactions: (p: {
    slotId: string;
    seasonYear?: number;
    category?: RepoTransaction["category"];
    leagueId?: string;
    npcId?: string;
    limit?: number;
