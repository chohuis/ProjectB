<script lang="ts">
  ...
  function swipeNav(node: HTMLElement, onSwipe: (dir: "prev" | "next") => void) {
    ...
    const up = (e: PointerEvent) => {
      const dx = e.clientX - sx,
        dy = e.clientY - sy;
      if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.2) onSwipe(dx < 0 ? "next" : "prev");
    };
    node.addEventListener("pointerdown", down);
    node.addEventListener("pointerup", up);
    return {
      update(fn: typeof onSwipe) {
        onSwipe = fn;
      },
      destroy() {
  ...
  function dragScroll(node: HTMLElement) {
    ...
    const move = (e: MouseEvent) => {
      if (!active) return;
      e.preventDefault();
      node.scrollTop = scrollY - (e.pageY - startY);
    };
    const up = () => {
      active = false;
  ...
  // ── 스케줄 항목 생성 ─────────────────────────────────────────
  // 1. 경기 항목 (주인공 경기만)
  $: gameItems = seasonEntries
    .filter((e) => e.isProtagonistGame)
    .map((entry): ScheduleItem => {
      const date =
        entry.gameDate ?? weekToDateKey(entry.week, seasonYear, entry.isFriendly ? 2 : 5);
      ...
      const result = entry.result
        ? {
            won: entry.result.winnerId === protagonistTeamId,
            score: `${entry.result.homeScore}:${entry.result.awayScore}`,
          }
        : null;
      return {
        id: entry.id,
...
<section class="page">
  <div class="board">
    ...
    <div class="filters">
      ...
      <button class:active={filter === "training"} on:click={() => (filter = "training")}
        >훈련</button
      >
                ...
                {#each allWeeks as week}
                  ...
                  >
                    ...
                    <div class="week-left">
                      ...
                      <span
                        class="phase-tag"
                        class:pre={phase === "preseason"}
                        class:reg={phase === "season"}
                        class:post={phase === "postseason"}
                        class:off={phase === "offseason"}
                      >
                        {PHASE_LABEL[phase]}
                      </span>
                    </div>
                    ...
                    <div class="week-games">
                      {#if wg.officials.length > 0}
                        {#each wg.officials as official}
                          ...
                          {@const opp = teamLabel(
                            isHome ? official.awayTeamId : official.homeTeamId,
                          )}
        ...
        <section class="year-grid">
          {#each monthLabel as label, monthIndex}
            ...
            <button class="month-card" on:click={() => jumpToMonth(monthIndex)}>
              ...
              <div class="chips">
                {#if summary.game > 0}
                  <span class="chip game">경기 {summary.game}</span>{/if}
                {#if summary.training > 0}
                  <span class="chip training">훈련 {summary.training}</span>{/if}
                {#if summary.event > 0}
                  <span class="chip event">이벤트 {summary.event}</span>{/if}
                {#if summary.rest > 0}
                  <span class="chip rest">휴식 {summary.rest}</span>{/if}
              </div>
            </button>
          {/each}
        </section>
            ...
            <li class="detail-item">
              ...
              <div class="detail-body">
                ...
                {#if item.type === "game" && item.meta?.result}
                  <span
                    class="result-badge"
                    class:win={item.meta.result.won}
                    class:lose={!item.meta.result.won}
                  >
                    {item.meta.result.won ? "승" : "패"}
                    {item.meta.result.score}
                  </span>
                {/if}
...
<style>
  ...
  .day-items .game.friendly,
  .week-day li.game.friendly {
  ...
  .day-cell.selected {
    border-color: var(--t-dark);
    box-shadow: inset 0 0 0 1px var(--t-dark);
  }
  .day-cell.outside {
    opacity: 0.3;
  }
  ...
  .week-day.selected {
    box-shadow: inset 0 0 0 2px var(--t-dark);
  }
  .day-title {
    border: 0;
    background: transparent;
  ...
  .game-row {
    display: grid;
    grid-template-columns: 58px 40px minmax(0, 1fr) 64px;
    align-items: center;
    gap: 7px;
    min-width: 0;
  }
