<script lang="ts">
   ...
   * 자리였다** — 스태프는 slot.db에서 오는데 아직 슬롯이 없고, 선수를 담을
   * 마스터 쪽 경로는 아예 없다(`master.db`를 2026-09-04에 접었다 · 그전에도
   * `npc_master`는 Phase 6A 이후 0행이었다).
   *
   ...
   */
  /**
   * 세계 씨앗 — **고정이다** (사용자 확정 2026-08-26).
   *
   * 🔴 예전엔 `Date.now()`라 **새 게임마다 선수·감독·코치가 전부 달랐다.**
   *   그러면 같은 상황을 다시 만들 수 없어 결함을 재현하지 못하고,
   *   공략·제보도 서로 안 맞는다.
   *
   * ⚠ **이 값이 세계 전체를 정한다** — 선수·감독·코치·과거 5년 순위까지.
   *   바꾸면 모든 새 게임의 세계가 통째로 달라진다. 함부로 건드리지 않는다.
   * ⚠ 이어하기는 영향이 없다 — 세이브에 자기 씨앗이 들어 있다.
   */
  const worldSeed = 20260826;
  ...
  async function doStartGame() {
    ...
    // ⚠ **60~90이었다. 시작부터 천장에 붙어 있었다.**
    //
    // `potential_cap_factor`는 `현재스탯 / 잠재력` **비율**로 XP를 깎는다
    // (<0.75 → 1.00 · <0.85 → 0.70 · <0.95 → 0.35 · 그 위 → 0.10).
    // 프리셋 스탯이 57~78인데 잠재력 하한이 60이라, 굴리기에 따라
    // **자기 잠재력보다 높은 능력치로 시작**했다:
    //
    //   시작 시점에 이미 0.35배 이하   21/31 = 68%
    //   시작 시점에 이미 0.10배        12/31 = 39%
    //
    // 실측(24회)이 그대로다 — 1학년 69 · 2학년 70 · 고교말 71 · **최대 75**.
    // 3년에 +3이고 최대값이 잠재력 중앙(75)에 딱 붙는다. 같은 시기 고졸
    // 지명자는 OVR 중앙 74 · 백분위 93이라 주인공이 구조적으로 못 미쳤다.
    //
    // ⚠ 계수를 올려서 덮지 않는다. `developmentRate`는 이미 한 번 그렇게
    // 올렸는데(55~70 → 73~88) 이 비율에 눌려 효과가 반쯤 먹혔다.
    //
    // **하한은 프리셋 최고 스탯(78)보다 위여야 한다** — 그래야 어떤 스탯도
    // 잠재력을 넘은 채로 시작하지 않는다. 뽑기는 그 위에서 갈린다:
