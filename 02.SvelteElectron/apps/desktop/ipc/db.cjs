...
const MAX_SCHEDULE_ENTRIES = 50_000;

function _validateSaveBoundary(season) {
  if (!season) return;
  const lschTotal = Object.values(season.leagueSchedules ?? {})
    .reduce((n, arr) => n + (Array.isArray(arr) ? arr.length : 0), 0);
  const schedTotal = Array.isArray(season.schedule) ? season.schedule.length : 0;
  const total = lschTotal + schedTotal;
  if (total > MAX_SCHEDULE_ENTRIES)
    throw new Error(`[dbSaveSlot] schedule entries (${total}) exceeds cap (${MAX_SCHEDULE_ENTRIES})`);
  for (const e of season.schedule ?? []) {
    if (Array.isArray(e?.result?.playerLines) && e.result.playerLines.length > 500)
      throw new Error(`[dbSaveSlot] schedule entry ${e.id} has ${e.result.playerLines.length} playerLines (max 500); strip before saving`);
  }
}

function dbSaveSlot(db, slotId, game, season) {
  _validateSaveBoundary(season);
  ...
  db.transaction(() => {
    ...
    if (game?.protagonist) {
      ...
      db.prepare(`INSERT INTO protagonist (
        ...
        military_status, military_enlist_year, military_discharge_year,
        military_enlist_week, sports_unit_selected,
        military_hiatus_stage, military_hiatus_university_week, military_defer_penalty,
        ...
        p.militaryStatus ?? '미필', p.militaryEnlistYear ?? null, p.militaryDischargeYear ?? null,
        p.militaryEnlistWeek ?? null, p.sportsUnitSelected ? 1 : 0,
        p.militaryHiatusStage ?? null, p.militaryHiatusUniversityWeek ?? null, p.militaryDeferPenalty ?? 0,
...
function dbLoadSlot(db, slotId) {
  ...
  if (p) {
    ...
    const protagonist = {
      ...
      militaryStatus: p.military_status ?? '미필',
      militaryEnlistYear: p.military_enlist_year ?? null,
      militaryDischargeYear: p.military_discharge_year ?? null,
      militaryEnlistWeek: p.military_enlist_week ?? null,
      sportsUnitSelected: !!p.sports_unit_selected,
      militaryHiatusStage: p.military_hiatus_stage ?? null,
      militaryHiatusUniversityWeek: p.military_hiatus_university_week ?? null,
      militaryDeferPenalty: p.military_defer_penalty ?? 0,
