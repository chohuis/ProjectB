...
///
/// 라운드 직선의 앵커(78.0, 아래 `round` 식)는 그대로 두고 문턱만 낮췄다.
/// `draftRelative.test.ts`가 두 값의 일치를 잠갔던 것은 예전 설계이고,
/// 지금은 둘이 다른 게 의도다 — 68~78점 구간은 `round` 계산값이 11 위로
/// 나와 그대로 클램프되어 "11R 턱걸이"로 뽑힌다.
///
/// 🔴 **68.0 — 5단계 D 계측으로 확정 (2026-09-10, `docs/BALANCE_PROPOSAL_101.md` ①).**
/// ⑭(시작 능력치 일괄 −9) 뒤 고졸 직행 지명이 8.3%(목표 30~40%)로 떨어졌다.
/// ⑭는 되돌리지 않고 이 문턱만 내려 재측정했다 — 같은 12칸(4프리셋×3씨앗·
/// 성장 페르소나)에서 78.0일 때 1/12(8.3%), 68.0일 때 4/12(33.3%)로
/// 목표 안에 들어왔다(예외·정지 0, 폴백 비율 불변). 사용자 확정.
const UNDRAFTED_SCORE: f64 = 68.0;
