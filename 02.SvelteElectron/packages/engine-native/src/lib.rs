...
#[napi] pub fn week_calc_npc_injuries_native(p: String) -> String { let params: week_engine::NpcInjuriesPayload = match serde_json::from_str(&p) { Ok(v) => v, Err(e) => return parse_err("weekCalcNpcInjuriesNative", e) }; serde_json::to_string(&week_engine::calc_npc_injuries(params)).unwrap_or_else(|e| parse_err("weekCalcNpcInjuriesNative/s", e)) }
