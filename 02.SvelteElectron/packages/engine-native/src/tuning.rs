...
// ── 구종 반복 · 타자 노림수 (결정 ⑩ · 2026-09-08) ────────────────────
//
// 🔴 **타자가 상대 구종 이력을 아예 안 보고 있었다.** `BatterStats` 에도
//   `swing_decision` 에도 `resolve_contact` 에도 「직전에 뭘 던졌나」가 없다.
//   그래서 다양성의 값어치가 전부 **투수 쪽 반복 페널티**(아래 PITCH_REPEAT_*)
//   에서만 나왔고, 그건 「타자가 읽었다」가 아니라 「투수가 알아서 손해본다」다.
//   구종을 다섯 배울 산식상 이유가 없었다.
//
// 🔴 **같은 일을 두 곳에서 세지 않는다.** 「같은 구종을 이어 던지면 불리하다」는
//   한 가지 사실이다. 투수 쪽 페널티를 그대로 두고 타자 쪽 가산을 얹으면
//   반복이 **두 배로** 벌을 받는다. 그래서 **투수 쪽을 절반으로 내리고
//   그 몫을 타자 쪽으로 옮겼다** (−1/−2/−4 → −0.5/−1/−2 · 새 구종 +1 → +0.5).
//
//   투수 쪽에 절반을 남긴 이유: 반복은 타자가 못 읽어도 손해다(같은 팔스윙·
//   같은 궤적이면 타이밍이 저절로 맞는다). 다만 **얼마나 손해인가는 타자가
//   정한다** — 그 몫이 아래 BATTER_READ_* 이고, 눈·선구안을 탄다.
//
// ⚠ 크기가 갈리는 자리를 적어 둔다(투수몫 + 눈 50 타자의 읽기몫):
//     직전과 같은 구종   −0.5 + −2.0 = −2.5  (예전 −1.0)
//     두 번 연속        −1.0 + −2.0 = −3.0  (예전 −2.0)
//     세 번 이상        −2.0 + −2.0 = −4.0  (예전 −4.0)
//     최근 3구에 두 번   0.0 + −1.5 = −1.5  (예전 0.0 — **새로 생긴 자리**)
//     최근 3구에 없음   +0.5        = +0.5  (예전 +1.0)
//   눈 90 타자는 읽기몫이 1.4배, 눈 20 타자는 0.6배다.

/// 구종 반복 페널티 — **절반이 타자 쪽(BATTER_READ_*)으로 갔다.**
/// 정본은 여기다. 예전엔 `match_engine::pitch_pattern_modifier` 안에 숫자로
/// 박혀 있었는데, 그러면 두 결정의 크기를 나란히 못 본다.
pub const PITCH_REPEAT_1: f64 = -0.5;
pub const PITCH_REPEAT_2: f64 = -1.0;
pub const PITCH_REPEAT_3: f64 = -2.0;
/// 최근 3구에 안 나온 구종 — **arsenal 이 넓을수록 자주 걸린다.**
/// 구종 2개로 번갈아 던지면 이 가산이 한 번도 안 붙는다(직전 3구에 늘 있다).
pub const PITCH_FRESH_BONUS: f64 = 0.5;

/// 노림수를 켠다 (1.0). **0 이면 결정 ⑩ 이전과 완전히 같다** — 투수 쪽
/// 페널티도 예전 크기(−1/−2/−4 · +1)로 되돌아간다. 계측용 `PB_BATTER_READ`.
pub const BATTER_READ_MODE: f64 = 1.0;
pub fn batter_read_mode() -> f64 {
    match std::env::var("PB_BATTER_READ") {
        Ok(v) => v.parse::<f64>().unwrap_or(BATTER_READ_MODE),
        Err(_) => BATTER_READ_MODE,
    }
}

/// ⑩ 이 꺼졌을 때 쓰는 예전 값 — **되돌리기를 한 자리에 모아 둔다.**
pub const PITCH_REPEAT_1_LEGACY: f64 = -1.0;
pub const PITCH_REPEAT_2_LEGACY: f64 = -2.0;
pub const PITCH_REPEAT_3_LEGACY: f64 = -4.0;
pub const PITCH_FRESH_BONUS_LEGACY: f64 = 1.0;

/// 타자가 보는 창 — **던지는 이 공까지 세어 3구다.**
///
/// 🔴 **이 공을 안 세면 번갈아 던지기가 안 잡힌다.** 처음엔 「앞 3구」로
///   뒀는데, 구종 둘을 A,B,A,B 로 번갈아 던지면 앞 3구가 늘 [B,A,B] 라
///   지금 던지는 A 가 **한 번밖에** 안 세어져 노림수가 영원히 0 이었다
///   (검사 `구종이_적으면_읽힌다` 가 잡았다). 「최근 3구에 두 번」은
///   사람 말로도 **이 공을 포함한** 3구다.
/// ⚠ 그래서 실제로 뒤져 보는 것은 앞 2구다(`WINDOW - 1`).
/// ⚠ 투수 쪽 `pitch_pattern_modifier` 의 「최근 3구에 없던 구종」은 이 공을
///   빼고 센다 — 그쪽이 묻는 것은 「이 공이 새것인가」라 이 공이 분모가
///   아니다. 잣대가 어긋난 게 아니라 **묻는 것이 다르다.**
pub const BATTER_READ_WINDOW: usize = 3;

/// 직전과 같은 구종 — 타자가 제일 크게 읽는다
pub const BATTER_READ_SAME_AS_LAST: f64 = 2.0;
/// 직전은 아니지만 최근 3구에 두 번 이상 나온 구종 (번갈아 던지기가 여기 걸린다)
pub const BATTER_READ_TWICE_IN_WINDOW: f64 = 1.5;

/// 눈·선구안이 읽기 크기를 가른다. 눈 6 : 선구안 4 — **눈이 주다**
/// (선구안은 「안 휘두른다」는 쪽이고 노림수는 「친다」는 쪽이라 덜 준다).
/// 50 이 기준(1.0)이고 0.6~1.4 로 막는다 — 밴드 폭이 7~8 이라 그 이상은 과하다.
pub fn batter_read_scale(eye: f64, discipline: f64) -> f64 {
    let skill = eye * 0.6 + discipline * 0.4;
    (1.0 + (skill - 50.0) / 100.0).clamp(0.6, 1.4)
}

// ── 결정구 (결정 ⑪ · 2026-09-08) ────────────────────
//
// 지금 2스트라이크 가산(`count_modifier` 0-2 +5)은 **「그 카운트라서」**지
// 「결정구를 잘 골랐다」가 아니다 — 무엇을 던지든 같은 +5 다. 그래서
// 「주무기를 다듬어 승부구로 쓴다」가 산식에 없다.
//
// ⚠ **최고 등급이 여럿이면 안 준다.** 전 구종이 3등급인 투수에게 주면
//   그건 결정구 보너스가 아니라 그냥 카운트 보정이 한 번 더 붙는 것이다.
//   「남들보다 잘 다듬은 공」이 있어야 결정구다 — `is_putaway_pitch` 참고.
// ⚠ **작게 둔다.** ⑩ 과 겹친다 — 2스트라이크에 결정구를 이어 던지는 것이
//   AI 기본 행동이다(`pick_from_arsenal` 이 70% 로 같은 공을 고른다).
//   반복이면 ⑩ 이 −2.0 을 먹이므로 순증은 −1.0 이고, 「같은 결정구만
//   되풀이하면 읽힌다」가 성립한다.
/// 결정구를 켠다 (1.0). 0 이면 결정 ⑪ 이전과 완전히 같다. 계측용 `PB_PUTAWAY`.
pub const PUTAWAY_MODE: f64 = 1.0;
pub fn putaway_mode() -> f64 {
    match std::env::var("PB_PUTAWAY") {
        Ok(v) => v.parse::<f64>().unwrap_or(PUTAWAY_MODE),
        Err(_) => PUTAWAY_MODE,
    }
}

/// 2스트라이크에 **최고 등급 구종**을 던졌을 때 품질 가산
pub const PUTAWAY_BONUS: f64 = 1.0;
/// 그 공이 **존 밖인데 타자가 따라 나왔을 때** 콘택트 품질에 얹는 벌.
/// ⚠ 이미 `chase_penalty`(존 밖 +12 · 경계 +5)가 있다 — 그 위에 얹는 몫이라
///   작아야 한다. 「유인구에 속았다」를 결정구가 한 겹 더 벌하는 자리다.
pub const PUTAWAY_CHASE_BONUS: f64 = 1.5;
...
// ── 로케이션 품질 — **착탄 기준 하나뿐이다** (결정 ⑦ · 확정 2026-09-08) ──────
//
// 🔴 **모드가 둘이었고, 켜는 자리가 코드에 없었다.** `location_intent_mode()`
//    가 환경변수 `PB_LOC_INTENT` 를 읽었는데 그 변수를 심는 것은 계측 스크립트
//    뿐이라, 게임은 **늘 착탄 기준으로만** 돌면서 코드에는 안 쓰이는 두 번째
//    산식이 얹혀 있었다. 그 상태를 끝낸 것이 이 결정이다 — 환경변수를 없애고
//    쓰지 않는 갈래를 지웠다. 이제 바꾸려면 이 파일을 고치고 다시 빌드해야 한다.
//
// **어느 쪽으로 정했나: 착탄 기준.** `location_q = -4 + dist x 5`.
//
// 의도 기준(지운 쪽)은 이랬다 — 겨냥한 곳의 난이도에서 빗나간 만큼을 뺐다:
//    let t = decision.target;  let t_dist = |t|;  let miss = |landing - t|;
//    location_q = LOCATION_CENTER_PENALTY + t_dist * LOCATION_DISTANCE_SCALE
//                                         - miss   * 6.0
//
// **왜 지웠나 — 그걸 만든 결함이 사라졌다.**
//   의도 기준을 만든 이유는 하나였다(2026-08-11): 착탄 기준은 착탄점의 중심
//   거리만 보는데 `pick_target` 은 대개 존 안을 겨냥하므로, 제구가 나빠 흩어질
//   수록 일부가 가장자리로 가서 품질이 **올라갔다** — 제구가 좋을수록 손해였다.
//   그때는 OVR 55 → 80 에서 ERA 가 오히려 나빠졌고 능력치 배수를 8배로 밀어도
//   방향이 안 바뀌었다.
//
//   2026-09-08 에 다시 쟀더니(`npm run probe:a:cmd` · 씨앗 셋 × 300경기 ·
//   구속·무브를 62 로 묶고 command·control 만 움직임) **뒤집힘이 없다**:
//     착탄 기준  제구 40 .334 → 62 .276 → 80 .242   (40→80 −0.093)
//     의도 기준  제구 40 .363 → 62 .298 → 80 .253   (40→80 −0.110)
//   둘 다 제구가 듣는다. 그 사이에 잡음 축소·`pitch_skill_scale`·결정 ⑧⑨ 가
//   들어왔고, 그중 무엇이 고쳤는지는 못 가렸다 — 다만 **고칠 이유가 없어졌다.**
//
// **그래서 무엇으로 갈랐나 — 리그 지표다** (`npm run probe:d:loc` · 씨앗 셋 ·
//   결정 ⑧⑨ 를 켠 위에서 · 고교→드래프트→프로 1시즌):
//     착탄 기준(0)  리그 ERA 5.24 / — / 5.25   리그 타율 .277 / — / .278
//     의도 기준(1)  리그 ERA 5.75 / 5.83 / 5.55  리그 타율 .290 / .291 / .286
//   (씨앗 777 의 착탄 판은 RESULT 를 못 읽어 빠졌다 — 남은 둘이 5.24·5.25 로
//    붙어 있고 의도 쪽 셋이 전부 그 위라 방향은 갈린다)
//   목표는 KBO 수준(타율 .270대 · 사용자 확정 2026-08-02)이고 **착탄 쪽이 그
//   안에 있다.** 의도 기준은 리그 ERA 를 +0.5, 타율을 +0.012 밀어 목표에서
//   멀어진다. 기준선 §3 이 결정 ⑧⑨ 전에 잰 값(6.51 → 6.93)과 **같은 방향·
//   같은 크기**다 — ⑧⑨ 가 들어와도 이 모드의 성격은 안 바뀌었다.
//
// ⚠ `audit:engine` ② 절의 띠는 **두 모드 다 통과했다**(피안타비중 0.460 대
//   0.489 · 띠 0.45~0.62). 그래서 감사만으로는 못 고르고, 위 리그 지표가
//   갈랐다. 감사는 내부 정합성을 보증할 뿐 리그 밸런스를 안 정한다는 이
//   파일의 다른 경고와 같은 이야기다.
// ⚠ 착탄 기준으로 정했으므로 **커맨드 계수는 그대로 둔다.** 「원하는 곳에
//   넣는다」는 의도 기준을 켤 때만 따라오는 숙제였다.
// ⚠ 다시 열려면: 위 산식을 `calculate_pitch_quality` 의 `location_q` 에
//   되살리고 `probe:a:cmd` 와 `probe:d:loc` 를 나란히 다시 잰다.
