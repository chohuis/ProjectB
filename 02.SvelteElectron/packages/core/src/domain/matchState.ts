...
export function createInitialMatchState(options: MatchStartOptions = {}): MatchState {
  ...
  const entryTrigger: EntryTrigger = options.entryTrigger ?? { type: "inning_start", inning: 1 };
  ...
  const fallbackOpponentLineup =
    options.opponentLineup ?? Array.from({ length: 9 }, () => createBatter(batterMean));
  const fallbackMyLineup =
    options.myTeamLineup ?? Array.from({ length: 9 }, () => createBatter(batterMean));
  ...
  const isImmediateEntry = entryTrigger.type === "inning_start" && entryTrigger.inning <= 1;
