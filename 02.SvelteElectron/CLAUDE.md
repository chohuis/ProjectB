...
# ⚠ 이 프로젝트는 동결됐다 (2026-08-13)

**개발 대상은 `04.GodotOnePitch/`로 옮겼다.** Godot 4.6 · GDScript 단독.

여기는 **읽기 전용**이다. 고치지 않는다.

## 왜 옮겼나

사용자 확정: 웹 기반(Svelte + Electron)이 아니라 Godot 기반으로 간다.
PC(Steam)가 1차 목표이고 안드로이드는 나중이다.

Rust 코어(24k줄)를 GDExtension으로 살리는 안과 C# 안도 검토했으나,
**GDScript 단독**으로 정했다 — 모바일 배포가 쉽고 언어가 하나이며,
성능 게이트를 실측으로 통과했다(시즌 시뮬 2,124경기 0.659초).

## 여기서 무엇을 읽나

**① 동작 명세.** 어떤 규칙이 왜 그 값인지가 커밋 메시지와 주석에 있다.
드래프트 앵커(score 88 → 7R · 문턱 78), 수상 자격선(다승·탈삼진 40 ·
방어율 45), 잠재력 범위(80~99), 교체 규칙(12 + 스태미나/99 × 15 아웃) —
전부 실측 근거와 함께 적혀 있다.

**② 검사 53개.** 비싸게 배운 것이 코드로 박혀 있다. 이주에서 **로직보다
먼저 옮긴다.**

## 마지막 상태

밸런스 작업은 `b29263b51`에서 동결했다. 미해결로 남긴 것:
