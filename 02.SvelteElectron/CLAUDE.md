...
⚠ 04 문서의 "02 검사 53개"는 **파일 수**다. 실제 검사는 **613건**이다.
...
| 5 | **FA 미계약자가 야구를 그만둔다** — 아래 참고 | `draft.rs` Placer | 중간 |

> **FA 미계약자 결말** — 실측 2026-08-27 · 사용자가 "나중에"로 미뤘다
>
> FA를 못 구한 선수의 **약 3분의 2가 야구를 아예 그만둔다**(`quit_baseball`).
> 은퇴가 아니다 — 27세 프로 5년차가 갈 곳이 없어 그만두는 것이다.
>
> ```
...
> ```
>
> ⚠ **문턱 탓이 아니다.** 비율이 거의 같다 — 원래 그랬고, 문턱 80(2026-08-27
> 사용자 확정)이 **걸리는 인원을 2.7배로** 늘렸을 뿐이다. 지금은 FA 신청자의
> 41%가 매년 야구를 그만둔다.
>
> 🔴 **가설 둘을 세웠고 둘 다 실측으로 틀렸다. 다시 세우지 마라:**
>
> - ~~"정원이 차서 못 받는다"~~ → KBL 1군 평균 30.3명/상한 34, 꽉 찬 팀 1/10
> - ~~"독립리그 나이 상한(31세)에 걸린다"~~ → 미계약자의 **32세 이상은 1%**다.
>   31세 이하 6,256건 중 68%가 포기하고, 32세 이상 42건 중에서는 19%다.
>   **어리고 갈 곳 없는 쪽이 그만둔다.**
>
> 아직 안 본 것: 독립리그가 **10팀(총 정원 450)**뿐이고 미지명 고졸·대졸과
> 같은 자리를 겨룬다 — `draft.rs`의 `Placer`가 2군 → 독립 순으로 자리를 찾는데
> 그 앞에 미지명자가 먼저 들어와 있을 수 있다. **확인 안 했다.**
>
