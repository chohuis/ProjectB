...
1. **아무것도 안 고쳤다.** 위는 전부 계측이다
2. 어디를 먼저 손댈지는 **사용자 판단** — 후보 셋
   - ⓐ 빈 메시지 둘을 conditional에서 빼기 (37주 회수 · 제일 싸다)
   - ⓑ conditional 주당 상한 1 → n (밸런스 변경이다 — 동결 중)
   - ⓒ priority 재배치 / 단계별 큐 분리
3. ⚠ **계측 흔들림이 남아 있다.** 같은 씨앗 두 실행이 272주·292주로 갈렸다
   (A 트랙이 추적 중). 다만 **conditional 발동률 12.6%는 두 실행이 같았다** —
