# CLAUDE.md 의 역사 — 규칙이 왜 생겼나 (2026-09-12 에 갈라냄)

> `CLAUDE.md` 가 807줄이 되어 **규칙과 역사가 섞였다**(프로젝트 평가 개선 11). 규칙은 `CLAUDE.md` 에 남기고, **왜 그 규칙이 생겼는지** — 날짜 · 실측 · 틀렸던 가설 · 해시 — 는 전부 여기로 옮겼다. **아래는 2026-09-12 시점의 `CLAUDE.md` 원문 그대로다.** 한 줄도 안 지웠다.
> 읽는 법: `CLAUDE.md` 의 규칙 옆에 「→ 역사 §제목」이 붙어 있으면 여기서 그 제목을 찾는다.
> ⚠ 여기 적힌 숫자(줄 수 · 검사 수 · 상한)는 **그때 값**이다. 지금 값은 명령으로 잰다.
...
### 🔴 「연도만으로 유일」은 **한 번만 난다는 전제**다 (2026-09-06 · 실사용자 사고)

`msg-tour-my-<대회>-r<라운드>-<연도>` 는 위 표의 「○ 대상 ID + 연도」인데도
**겹쳤다.** 전제가 깨졌기 때문이다 — 그 라운드가 **주마다 다시 확정**됐다.

원인은 소식 쪽이 아니었다: 대회 본선 한 경기가 무승부로 끝나 승자가 안 찍혔고
(`gameSimulator` 가 대회 경기에 정규시즌 연장 상한을 걸었다), 라운드가 영영
안 닫혔다. 화면은 통째로 굳었다(탭 전환도 안 됨). 자세한 것은
`utils/scheduleView.ts` 의 `knockoutMatchIds` 머리말.

거기서 배운 것 둘:

- **id 에 「무엇이 그 소식을 한 번만 나게 하는가」를 적어 둔다.** 대회 소식
  다섯에는 이제 주차가 들어간다 — 또 두 번 나면 조용히 버려지는 대신
  두 통이 남아 눈에 띈다.
- **막는 자리는 세는 자리이기도 해야 한다.** `dedupeMailbox` 가 사본을
  걷어내되 `mailboxDupStats` 로 센다. 안 세면 증상만 없어지고 만드는 쪽의
  결함은 남는다. 게이트: `npm run check:msgdupid`
  (`check:msgdup` 과 다르다 — 그쪽은 「같은 id 로 **다른 소식**」,
  이쪽은 「같은 id 가 **두 번**」이다)

## 🔴 좁게 읽었으면 좁게 써라 — 저장 왕복 검사 셋 (2026-09-06)

실사용자 세이브를 죽인 형태는 **좁게 읽고 넓게 되쓴 것**이었다:
`getAllNpcs` 가 은퇴자를 칼럼을 골라 읽고, `syncNpcs` 가 스토어 전체를
되쓰면서 **안 읽은 칼럼이 삽입 폴백으로 덮였다**(은퇴자 869명의
`military_status`·`salary`·능력치가 왕복마다 지워졌다). 막는 자리 셋:

| 검사 | 무엇을 보나 | 언제 |
|---|---|---|
| `npm run check:savecolumns` | R2 좁은 읽기 목록이 **표를 다 덮나**(`PRAGMA` 로 DB 에 묻는다) · R1 한 명 왕복 · R3 결과에 `undefined` 가 없나 | `test:v3` 안에서 매번 |
| `npm run check:roundtrip` | 세계 전체를 **새 프로세스로** 불러왔다 다시 저장해 한 칸씩 대조 | 저장·로드를 건드렸을 때 |
| `npm run check:msgdupid` | 같은 소식 id 가 두 번 났나 | 진행 로직을 건드렸을 때 |

- **손으로 적은 칼럼 목록은 반드시 뒤처진다.** 그래서 `check:savecolumns` 는
  `RETIRED_NPC_COLUMNS` 를 **표와** 맞춘다 — 칼럼이 새로 생기면 빨개지고,
  그때 "읽을 것인가"를 결정하게 된다.
- **`check:roundtrip` 은 반드시 프로세스를 나눈다.** 같은 프로세스로 다시
  저장하면 스토어가 메모리에 남아 있어 **유실이 안 보인다.**
- **`undefined` 와 `null` 은 다르다.** `JSON.stringify` 는 `undefined` 키를
  통째로 빼고, Rust 의 옵션 아닌 필드가 그걸 `missing field` 로 거부한다.
