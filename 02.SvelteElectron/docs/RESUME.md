...
**Phase 5-3까지 완료.** 172팀 세계가 켜졌고 **고교 8권역 주말리그가 돈다**(1,020경기).
...
**고교 전국대회 5종.** 주말리그 위에 얹는 토너먼트 — 개나리기·장미기(32강) ·
무궁화기(48강) · 국화기(128대진) · 패왕기(24팀).

- 리그와 달리 **탈락 구조**라 라운드로빈 코드를 재사용할 수 없다. 시드 배정 +
  대진표 생성이 새로 필요하다.
- 주말리그(2~45주)와 **주차가 겹치지 않게** 배치해야 한다 — 현재 주말리그가
  한 주에 최대 2일만 쓰므로 평일은 비어 있다.

이후 5-5(대학) → 5-6(독립) → 5-7(프로 PO) → 5-8(투구수·반경).
전부 끝나면 `main`에 병합.

## 검증 명령

```bash
npm run test:v3          # 6개 스위트 (migration·slotdb·rostergen·newgame·draftpool·regional)
...
1~3 정리·조사·기획통합 ✅ · **4** 데이터 기반+하네스 ✅ · **5** 리그·대회 🔨(5-3까지)
