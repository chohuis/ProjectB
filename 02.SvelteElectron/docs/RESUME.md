...
**Phase 5-2까지 완료.** 172팀 세계가 켜졌고 데이터·테스트는 전부 통과하지만,
**고교·대학·독립 리그에 일정이 없는 의도된 미완 상태**다. 브랜치를 판 이유가 이것.

- `DEFAULT_LEAGUE_CONFIGS`에서 고교·대학·독립을 **일부러 뺐다** — 8권역/5조/4단계
  생존리그는 `cycles` 모델로 표현 불가. 그냥 두면 50팀 라운드로빈이 6,000경기를 만든다.
- 프로만 정확히 표현된다 (1군 10팀 144경기 · 2군 10팀 99경기).

## 다음에 할 것 — Phase 5-3

**8권역 주말리그 스케줄러.** Rust `schedule_engine.rs`에 권역 단위 생성을 넣는다.

- 편성 기준은 이미 데이터에 있다: `leagueTeams.generated.ts`의 **`HS_REGIONS`**
  (고교 102팀이 거점구장 공유로 정확히 8권역으로 나뉜다 — 5-2에서 확인)
- 목표: **팀당 20경기**. 권역 크기 편차(6~20팀)는 **라운드로빈 바퀴 수로 보정**한다.
- 현행 `cycles` 모델을 **목표 경기수 모델**로 바꿔야 한다.

이후 5-4(고교 대회 5종) → 5-5(대학) → 5-6(독립) → 5-7(프로 PO) → 5-8(투구수·반경).
전부 끝나면 `main`에 병합.

## 검증 명령

```bash
npm run test:v3          # 5개 스위트 (migration·slotdb·rostergen·newgame·draftpool)
...
1~3 정리·조사·기획통합 ✅ · **4** 데이터 기반+하네스 ✅ · **5** 리그·대회 🔨(5-2까지)
