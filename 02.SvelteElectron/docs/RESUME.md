...
2. 프로파일 → 진범 특정 → **그것만** 수정
3. 은퇴자를 `getAllNpcs`에서 빼기 (7-1 D-4b 이월) — 근거는 `npm run measure:slotsize`

### Phase 8 들어가기 전에 알아둘 것

- **현재 성능은 측정된 적이 없다.** Phase 7이 주간 처리에 계속 얹혔다 —
  승강(매주 10구단) · 국가대표 · 재정 · 캠퍼스 이벤트 · 문장 뱅크가 전부
  `advanceWeek`을 지난다
- `advanceWeek.ts`가 **1,900줄을 넘었다.** Phase 8의 주요 후보이기도 하다
- OnePitch는 같은 규모를 Rust 단독 1시즌 24.75초로 처리했고 **최대 개선 요인은
  인덱스 부재 해소**였다(73s→24.75s). 현행은 주간 처리 상당부분이 TS다
