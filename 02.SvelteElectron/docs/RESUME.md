...
**Phase 5-7까지 완료.** 국내 전 리그의 정규시즌·대회·포스트시즌이 전부 돈다
(고교 1,020+233 · 대학 225+85 · 독립 152 · 프로 1군 720 · 2군 495).
...
## 다음에 할 것 — Phase 5-8 (Phase 5 마지막)

**투구수 상한 + 의무 휴식표 · 반경 게이트 재정의.**

- 상한: 고교 105구 / 대학·독립·프로 120구 (DESIGN §7.2).
  현행 매치엔진에 soft/hard cap이 따로 있으니 **중복·충돌부터 확인**할 것.
- 의무 휴식표는 **일 단위**다 — 5-3~5-7에서 리그별 요일을 다르게 깐 이유가 이것.
  `playerConditions.lastPitchedWeek`가 주 단위라 날짜 기준으로 바꿔야 할 수 있다.
- 반경 게이트: DESIGN §2가 "국내 풀 / 해외만 드리프트"로 바뀌었는데 코드에
  구 Lite 반경 로직이 남아 있는지 훑는다.
...
```bash
npm run test:v3          # 10개 스위트 (…·tournament·groupstage·survival·propo)
...
1~3 정리·조사·기획통합 ✅ · **4** 데이터 기반+하네스 ✅ · **5** 리그·대회 🔨(5-7까지)
