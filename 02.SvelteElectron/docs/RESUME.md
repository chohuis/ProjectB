...
feat/promotion        (최신)     ← Phase 7-2 승강 + 7-3 병역. feat/draft 위에 8커밋
...
```bash
npm run test:v3          # 20개 스위트 (전부 ALL PASS여야 한다)
cd packages/engine-native && cargo test --release   # Rust 유닛 105개
