...
```
main                  b4ecb1a3f  Phase 6 병합 완료
fix/domestic-rosters  c336d335e  ← 여기서 작업 중. main보다 5커밋 앞섬
feat/people                      Phase 6 작업 브랜치 — 병합됨, 삭제 무방
feat/172teams                    Phase 5 작업 브랜치 — 병합됨, 삭제 무방
```

**Phase 6 완료** (관계도까지 main 병합). `fix/domestic-rosters`에 실행 스모크에서
나온 수정 5건이 쌓여 있다 — **사용자 확인 후 main 병합 대기**:

1. 국내 전 리그 로스터를 새 게임에서 생성 (빈 로스터 경기가 0-0 홈팀승이었다)
2. 로스터 현실 규모 확대 + 8포지션 두 바퀴 보장
3. npcId 팀 해시 충돌 (대학 두 팀이 같은 ID를 만들고 있었다)
4. 유령 팀 31개 제거 (refs에 없는 v1 팀이 목록에 섞여 있었다)
5. 해외 리그 1차 출시 제외 게이트
...
1. **실행 스모크** — 새 게임 → 팀·리그 화면에 해외가 안 보이는지, 국내 로스터가
   30~34명씩 차 있는지 확인
2. **`fix/domestic-rosters` → `main` 병합**
3. **Phase 7-1 — 11월 통합 드래프트**부터. 분해와 순서는 [BACKLOG.md §1](BACKLOG.md)
...
cd packages/engine-native && cargo test --release   # Rust 유닛 45개
