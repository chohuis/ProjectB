...
**Phase 5-5까지 완료.** 고교 8권역 주말리그(1,020경기)·전국대회 5종, 대학 5조
정규리그(225경기)·대회 3종이 돈다.
...
**독립 10팀 4단계 생존리그.** 단계마다 컷오프(10→8→4), 35경기 + PO.

- 리그도 대회도 아닌 **제3의 형식**이다 — 단계가 진행되면서 팀이 탈락하고
  남은 팀끼리 다시 리그를 돈다. `group_stage.rs`의 "예선 리그 → 진출 판정"
  구조가 가장 가깝다. 그걸 다단계로 반복하는 형태.
- 상무(TEAM_IND_SANGMU)가 10번째 팀이고 매 시즌 로스터가 40~50% 갈린다 —
  생존 판정이 전력 변동과 맞물린다.

이후 5-7(프로 5강 사다리·2군 축약 PO) → 5-8(투구수 상한·휴식표·반경 게이트).
전부 끝나면 `main`에 병합.
...
```bash
npm run test:v3          # 8개 스위트 (…·regional·tournament·groupstage)
...
1~3 정리·조사·기획통합 ✅ · **4** 데이터 기반+하네스 ✅ · **5** 리그·대회 🔨(5-5까지)
