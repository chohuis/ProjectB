...
통 수는 실측이고(§7 · `measure:messagekinds` 6시즌), 종류 수와 표시 형태는
코드 전수다.
...
## 7. 통 수 — 실측 (`measure:messagekinds --seasons 6` · 2026-09-03)

6시즌(고교 3 → 독립 1 → 프로 2) 한 판. **메일함이 아니라 생산 시점에서** 세고
시즌 경계마다 차분한다 — 상한에 밀려 사라진 뒤에 세면 이미 없어진 종류가 0으로
보이기 때문이다(스크립트 머리 주석).

```
총 생산 1,974건 · 메일함 1,500/1,500 · **밀려남 474건**
단계별 시즌당   고교 321 · 독립 220 · 프로 396
```

🔴 **474건이 상한에 밀려 사라진다.** 여섯 시즌에 생산의 24%다. 대시보드로
바꿀 종류를 고를 때 **밀려나는 쪽부터 보는 게 값이 크다** — 어차피 안 읽히는
통을 예쁘게 만들어도 소용이 없다.
...
| 종류 | 고교 | 독립 | 프로 | 표시 |
|---|---|---|---|---|
| `evt-` 이벤트 | 90.7 | 62.0 | 131.0 | 텍스트 |
| `msg-train` 주간 훈련 | 45.3 | 32.0 | 52.0 | **대시보드** |
| `msg-rel` 관계 변화 | 29.0 | 15.0 | 26.5 | 텍스트 |
| `msg-league-results` 리그 결과 | 21.3 | 16.0 | 28.0 | 텍스트 |
| `msg-official-result` 공식 경기 | — | — | 24.5 | 텍스트 |
| `msg-demote` 강등 | — | — | 22.0 | 텍스트 |
| `msg-injury` 월간 부상(NPC) | 11.3 | 8.0 | 13.0 | **대시보드** |
| `msg-top10-pitcher` 유망주 | 11.3 | — | — | **대시보드** |
| `msg-npc-trade` 트레이드 | 11.0 | 8.0 | 9.5 | 텍스트 |
| `msg-friendly-result`·`-plan` 연습경기 | 10.3 · 9.3 | 7.0 · 8.0 | — | 텍스트 |
| `msg-mybody` 몸 상태 | 10.0 | 7.0 | — | 🔴 텍스트(§4) |
| `msg-digest` 다이제스트 | 9.7 | 7.0 | 11.0 | 텍스트 |
| `msg-coach-report` 코치 리포트 | 8.0 | — | 9.0 | 텍스트 |
| `msg-tour-*` 대회 | 13.7+ | 25.0+ | 21.0+ | 텍스트 |

### 이 표가 말하는 것 둘

**① 대시보드 넷은 통 수로는 작다.** 훈련(시즌당 45~52)만 크고 나머지 셋을
합쳐도 이벤트 하나(90~131)에 못 미친다. **눈에 보이는 소식의 대부분이 텍스트다.**

**② 텍스트 쪽 상위가 전부 값을 말한다** — 리그 결과 · 공식 경기 · 강등 ·
트레이드 · 다이제스트 · 대회. §5 후보와 같은 목록이 실측에서도 위로 온다.

⚠ 다이제스트 통합 대상 넷(`msg-neighbor`·`msg-myrank`·`msg-hs-digest`·
`msg-standings`)은 **전 단계에서 0건**이다. 이미 `msg-digest` 하나로 합쳐진
뒤라 그 표는 지금 뜻이 없다 — 스크립트 쪽이 낡았다.
