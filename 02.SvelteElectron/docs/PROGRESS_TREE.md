...
**완료 19 / 20 = 95%** (09-04 09:0x · 남은 것은 **18 회신 결함**뿐 — 빌드·포장 눈확인에서 나온다). 🔴 A 가 출시를 막을 결함 하나를 잡았다: `_manifest.json`(gitignore)이 낡아 이벤트 18종이 안 실린 채 포장될 뻔했다(6c33079e0). 옛 줄: 17 재회 훅 — A 가 「빌드가 이벤트 목록을 안 다시 만들던 것」을 고침(6c33079e0) → D 새 빌드로 12종 도달률 재측 뒤 ✅ · 20 — D 빨강은 낡은 빌드 + 프로브가 외국인 「면제」를 군필로 센 탓(19e6c94bf) → D 재덤프 뒤 ✅ · 18 회신 결함(빌드 눈확인 뒤) · protransition 은 A 재실행 초록). C 남은 일 = 단위 9 눈확인 · 묶음 3·4 대시보드 눈확인 · 「본문+패널」 표시 · rankList/timeline 문안 읽기. 1.0.0 고정 = 20 전부 ✅ · 그 뒤 BALANCE_BACKLOG 조정 → 1.0.1.
