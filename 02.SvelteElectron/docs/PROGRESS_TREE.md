...
├─ 11. ✅ 빌드 산출물 — `release/win-unpacked/OnePitch.exe`(09-03 02:45 pack) 첫 실행 → 새 게임 → W5(자동 저장 `slot3_slot_1.db`) → 종료 → 재실행 「이어하기」 → 슬롯 → **W5 그대로** → W8 진행 · 1366×768 소식/리그/나 · 병역·이벤트 모달·결산은 dev 우회 없인 못 닿음(asar 에 문자열은 들어 있다) (HANDOFF_C_TO_A 9차 · `c11-*.png`)
