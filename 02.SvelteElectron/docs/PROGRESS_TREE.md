...
| 17 | 병영 재회 이벤트 — 축소판(위생 검사 값 기준 · 조건 키 셋 · 관계 종류 unitmate · 이름 렌더러 없음) | A → B → A | 🔄 **D 실측(ffe0a00ed) 도달 0/12 — 풀에는 있으나 전역 뒤 조건 이벤트 훅이 안 걸려 있다(A 트리 14 「⬜ 재회 훅」) → A 배선 뒤 D 재측** · A 01b4d5803 · B 16553ea9f(12종 풀 적재 · 605종 · 초안 파일 삭제 · topRelations NUM 경로는 unitmate 와 같은 축이라 미사용) |
...
**완료 18 / 20 = 90%** (09-04 04:5x · 남은 것: 17 재회 훅 — A 가 「빌드가 이벤트 목록을 안 다시 만들던 것」을 고침(6c33079e0) → D 새 빌드로 12종 도달률 재측 뒤 ✅ · 20 — D 빨강은 낡은 빌드 + 프로브가 외국인 「면제」를 군필로 센 탓(19e6c94bf) → D 재덤프 뒤 ✅ · 18 회신 결함(빌드 눈확인 뒤) · protransition 은 A 재실행 초록). C 남은 일 = 단위 9 눈확인 · 묶음 3·4 대시보드 눈확인 · 「본문+패널」 표시 · rankList/timeline 문안 읽기. 1.0.0 고정 = 20 전부 ✅ · 그 뒤 BALANCE_BACKLOG 조정 → 1.0.1.
