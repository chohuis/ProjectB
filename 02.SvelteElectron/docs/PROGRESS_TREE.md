...
| 2-c | 🔴 **사본을 만드는 쪽은 아직이다** — 대회 라운드 소식 id 에 **주차가 없어**(`msg-tour-my-<대회>-r<라운드>-<연도>`) `advanceWeek` 의 대회 루프가 같은 라운드를 다시 확정하면 같은 id 를 또 낸다(`weekPhases/tournamentNews.ts:134`). 화면은 이제 안 죽지만 **소식 한 통이 조용히 버려진다.** 왜 같은 라운드가 두 번 확정되는지까지 | ⬜ **A** |
