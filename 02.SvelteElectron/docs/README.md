...
| 문서 | 무엇 | 상태 |
|---|---|---|
| `../CLAUDE.md` | **코드 규칙만** — 한 줄씩 · 옆에 「→ 역사 §제목」 (2026-09-12 에 갈랐다) | 정본 |
| `CLAUDE_HISTORY.md` | 그 규칙이 **왜** 생겼나 — 갈라내기 전 `CLAUDE.md` 원문 807줄 그대로(날짜 · 실측 · 틀렸던 가설 · 해시) | 역사 · 숫자는 그때 값 |
...
| `BALANCE_PROPOSAL_102.md` | 1.0.2 제안 둘 — 대학 입학 문턱(분포+후보값) · 군 목표표 정본 자리 | 진행 중 (D · `track/measure`) |
...
| `SIM_REPORT_2026-09-11_final.md` | 12판 최종 — 1.0.1 의 값 | 1.0.1 「전」 값 |
| `SIM_REPORT_2026-09-13_stage4.md` | 1.0.2 4단계 12판 — 신인 2군 출발 · 대학 D 문턱 | 24판이 대신한다 |
| **`SIM_REPORT_2026-09-14_final24.md`** | 🔴 **1.0.2 계측 24판** — 사다리 · 성향별 지원 · 잣대 물음 둘(ⓐⓑ) | ⚠ 대학·독립 칸은 09-18 배선 뒤 옛 값 |
| `SIM_102_YARDSTICK_2026-09-18.md` | ⓐ 대학 합격은 Rust 확률 게이트(28/22/8%) · ⓑ 군 복무 주는 등급 계수기를 안 탄다 | 정본 |
| `SIM_102_MILITARY_COUNTER_2026-09-18.md` | 군 전용 계수기 — 6판 25.8/시즌(목표 24~30 안) | 정본 |
| `SIM_REPORT_2026-09-19_final24b.md` | 배선 뒤 24판 — 대학 합격 24/24 · 무대 도달 4/24 · 성장형 군 계수기 0 | **09-20 이 대신한다** · 대학·군 칸은 `SIM_102_UNIV_MIL` 의 드라이버 수정 전 값 |
| `SIM_102_UNIV_MIL_2026-09-19.md` | 대학·군 물음 둘 — ① 안전형이 대학을 안 다녔다(드라이버 · 고쳤다) · ② 체육부대를 안 세고 있었다(잣대 · 고쳤다) | 정본 |
| `SIM_REPORT_2026-09-20_final24c.md` | 1.0.2 계측 24판 세 번째. 세 판(09-14·09-19·09-20) 나란히 · 안전형 대학 재학 확인(3/8) · 군 부대별 목표 둘 다 범위 안 | **09-21 이 대신한다** — 잣대 열넷째(못 접은 해가 등급 이벤트 자체를 버림)로 등급 빈도가 낮게 잡혔다 |
| `SIM_REPORT_2026-09-21_final24d.md` | 1.0.2 계측 24판 네 번째. 워커 연도 라벨 고침(`cef688b5a`) 뒤 재계측 · 네 판 나란히 · `report-simruns.cjs` 자체의 무대-접힘 잣대 결함을 찾아 고침(고졸직행 87.5%→33.3%) · 고졸직행 처음 범위 안 · 군 24/24 완주 · 폴백 -21%(B 고교 레어 수정) | **09-25 가 대신한다** — 프로 정원이 구단마다 갈리며(γ=0.20) 재계측 |
| **`SIM_REPORT_2026-09-25_final24e.md`** | 🔴 **1.0.2 계측 마감 정본 — 24판 다섯 번째.** 프로 정원 압축 사상(γ=0.20·`rosterMin` 29) 뒤 재계측 · 다섯 판 나란히 · 콜업 속도는 인구 감소에 안 반응(25→25) · 고졸직행 처음 목표 아래(33.3%→25%, growth 성향만) · 성장:안전 처음 8:0 쏠림 · 폴백 계속 개선(0.76%→0.73%) | 정본 |
| **`SIM_102_JSON_CTRL_2026-09-26.md`** | 🔴 09-25 #8 의 `matchToSimResultNative` JSON 제어문자 예외 — 데이터 가설은 실측으로 틀렸고, JSON 을 손으로 이은 자리 열여섯을 걷었다 · 그 한 건은 못 재현 · 다음엔 메시지가 어느 칸인지 말한다(`cargo test json_ctrl` · `npm run probe:a:jsonctrl`) | 정본 |
...
| **`SIM_102_DRAFT_GAMMA_2026-09-26.md`** | 🔴 고졸 직행 하락(33.3%→25%)이 정원 압축(γ=0.20)의 효과인지 씨앗 우연인지 — 씨앗 다섯 20짝 중 4짝이 한 방향으로만 갈렸다(효과) · 단 드래프트 총량(KBL 110명)은 γ 와 무관하게 고정 | 정본 |
...
| `PATCH_NOTES_v1.0.2.md` | v1.0.1 → v1.0.2 테스터용 패치 노트 |
