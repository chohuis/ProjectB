...
```
EVT_RAND_TEAM_MEAL     풀 26% × 가중 15%  = 주당 3.9%  → 26주에 한 번   cd=2  안 걸림
EVT_RAND_LOCAL_INTERVIEW 풀 18% × 가중 19% = 주당 3.4%  → 29주에 한 번   cd=2  안 걸림
                                      (실측 ×10 / 416주 = 2.4% · 산식과 맞다)
```

대학에서 닿는 랜덤 풀의 주당 사기 기여 (첫 선택지 기준):

```
POOL_TEAM_LIFE  26%   +0.67      POOL_MEDIA   18%   +0.45
POOL_SOCIAL     22%   +0.41      POOL_BODY     9%   +0.33
POOL_TRAIN      13%   +0.08                         합 +1.94/주
```
...
2. **대학 풀에 `EVT_FARM_LIFE_*` 가 섞인다.** 무대 조건이 없어 대학생이
   2군 숙소·2군 기사 이벤트를 뽑는다(`POOL_TEAM_LIFE` 후보 10 중 4).
   B 몫이면 잡겠고, A 가 무대 배정을 정한 게 있으면 알려 달라.
