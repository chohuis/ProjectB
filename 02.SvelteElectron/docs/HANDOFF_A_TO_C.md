...
군 복무 중 전 리그 0                → 세상이 돈다 (고교만 0 — A 확인 중)
...
날짜별 C 일감은 [PLAN_RELEASE §2.1](PLAN_RELEASE_2026-09-28.md) C 표다. **9/3 순위표 4종
→ 9/4 재정·투자·2군 탭** 순이다. 결함은 매일 `HANDOFF_C_TO_A.md` 에 — A 가 아침에 읽는다.
...
## 3. ✅ 코드 프리즈 9/23(화) — 사용자 확정

그 뒤 C 는 **결함 수정만** 한다. 새 화면 없음. W3 까지 눈확인을 끝내야
프리즈 뒤 일감이 결함뿐이 된다.
