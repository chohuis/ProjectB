...
⚠ **소식 id 에 연도를 넣는다** — `msg-role-{year}`. 주차는 시즌마다 1로
돌아가고, 무대 이동·콜업으로 **한 해에 두 번 물을 수 있다**(§4 ①②③).
그때 겹치면 Svelte 목록이 죽고 세이브가 안 열린다(CLAUDE.md). 팀 id 까지
넣은 `msg-role-{year}-{teamId}` 가 안전하다 — `lastRoleChoiceKey` 와 같은 꼴이다.
