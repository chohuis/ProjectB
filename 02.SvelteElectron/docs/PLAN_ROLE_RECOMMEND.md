...
### 문안 — B 안 (제안 · 2026-09-03)

문안은 B 소유라 위 §4·§5 의 글을 B 기준으로 다듬어 둔다. **뜻은 안 바꿨다** —
부제·대시 설명을 빼고 한 줄로 줄인 것이다. 골라 쓰면 된다.

**묻는 소식** — 제목·본문·버튼

```
제목   {연도}시즌 보직
본문   (감독 이름)이 올해 자리를 이렇게 봤다.
       {추천 한 줄}
버튼   선발   중계   마무리        ← 추천에만 표시 하나
```

추천 한 줄 — 보직마다 하나씩 (감독의 말 · 숫자 없음):

```
선발    긴 이닝을 맡길 만하다.
중계    한 이닝을 짧게 끊는 게 낫다.
마무리  마지막 한 이닝을 맡기고 싶다.
```
...
⚠ `ahead` 가 **0 일 때가 있다**(자리가 비었는데 추천이 아닌 경우 · §3 의 자리
배정에서 나온다). 그때 "0명 있다" 는 말이 안 된다 — 갈래를 하나 둔다:
...
```
제목   {연도}시즌 보직 — {보직}
본문   추천대로     올해는 {보직}으로 간다.
       추천과 다름  감독은 {추천}을 권했지만 {보직}을 택했다.
                    기회는 스스로 만들어야 한다.
```

⚠ **소식 id 는 `msg-role-{year}-{teamId}-w{week}` 다** (확정 · §7).
연도만 쓰면 겹친다 — 무대 이동·콜업으로 한 해에 두 번 물을 수 있다(§4 ①②③).
팀 id 를 붙여도 **같은 팀에서 두 번**이 남는다(콜업 → 강등 → 콜업). 그래서
주차까지 세 조각이다. 겹치면 Svelte 목록이 죽고 세이브가 안 열린다(CLAUDE.md).
근거와 가드(`lastRoleChoiceKey`)와의 짝은 §7 「소식 id」에 적었다.

⚠ 감독 이름이 없으면 "코칭스태프" 다(§4). 그때 본문 첫 줄은
"코칭스태프가 올해 자리를 이렇게 봤다." 로 읽힌다 — 어색하지 않다.
...
⚠ **추천을 눌러도 같은 줄이 뜬다**(확정 12). 추천이라고 문장을 빼면 "추천은
자리가 있다"는 뜻이 되는데, 셋 다 밀리는 경우가 실제로 있다(§8 ⑫).
