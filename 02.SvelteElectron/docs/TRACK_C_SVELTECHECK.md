...
TRACK_C ③. **오류 34 → 0 · 경고 37 → 0.**

```
1주차 시작   오류 34 · 경고 37 · 문제 파일 12
1주차 끝     오류 15                     (목표 34 → 15 달성)
2주차 끝     오류 13 · 경고  0           (C 가 손댈 수 있는 것은 다 했다)
3주차 시작   오류  0 · 경고  0 · 문제 파일 0    ✅
tsc          깨끗
vitest       177파일 1,564건 통과
```

마지막 13건은 **A 가 타입 셋을 늘리고**(§6) **C 가 죽은 갈래 하나를 지워**(§7)
0 이 됐다.
...
| # | 무엇 | 결과 |
|---|---|---|
| 1 | `PitchUnlockRule` 에 `"multi_stat"` · `conditions` | ✅ 6건 |
| 3 | `InteractiveMatchResult.protagonistEntered` | ✅ 1건 |
| 4 | `severity` 에서 `"surgery"` 제거 + 우회 캐스팅도 삭제 | ✅ 1건 |
| 5 | 타자 계약 평가 | ✅ 1건 — **C 가 본 것보다 컸다** |

⚠ 5번: 같은 `SeasonStats` 를 **제시액 계산도** 썼다. 타자 주인공은 표시만이
아니라 **계약 금액 자체가 객체**였다. 가드가 두 자리라 제시액만 고치면
화면 평점이 50 으로 굳는다 — A 가 실측으로 잡았다. 산식은 새로 안 만들고
NPC 정본 `calcNpcPerfScore` 의 타자식을 옮겼다.

### 소유권 — `shared/types/` 는 **A 다** (C 요청 반려)

> 타입은 게임 로직의 계약이라 화면 편의로 넓히면 로직이 못 지키는 모양이
> 생긴다. 이번 다섯 중 둘이 그 이유다 — 하나는 넓히면 안 됐고(§7),
> 하나는 좁혀야 했다(`"surgery"`).

맞는 판단이다. **필요하면 인계서에 적고 A 가 같은 턴에 처리한다.**

---

## 7. 🔴 남은 4건 — **내 진단은 맞고 처방이 틀렸다**

C 는 *"`sportsMilitaryPassed` 를 만드는 코드가 없다 → 타입을 늘려달라"* 고
넘겼다. A 가 재 보니 **그 위쪽이 통째로 죽어 있었다.**

```
CareerApplications.sportsMilitaryApplied 를 true 로 만드는 코드   0건
...
C 가 적은 *"눌러도 늘 불합격"* 은 틀렸다 — **누를 수가 없었다.**
...
그리고 진짜 경로는 `advanceWeek` 이 `protagonist.sportsUnitApplied` 로
**이미 제대로 돈다.** 타입을 늘렸으면 죽은 갈래를 키우는 것이었다.

→ `CareerResultsModal` 에서 그 갈래를 지웠다. **오류 4 → 0.**

⚠ 지우면서 `{#if hasSports}` 의 닫는 태그를 잘못 잘라 마크업이 깨졌다
(8칸 `{/if}` 패턴이 12칸 줄에 걸렸다). `svelte-check` 가 바로 잡았다.

검사 5건(`career/__tests__/noSportsUnitInResults.test.ts`) · 변이 2건.
**양쪽을 다 막는다** — 죽은 갈래가 되살아나도, 진짜 경로가 지워져도 실패한다.

### 배운 것

**"쓰는 곳이 없다"를 찾았으면 "만드는 곳도 없나"까지 본다.**
C 는 읽는 쪽만 보고 타입을 늘려달라고 했다. 한 칸 위를 봤으면
지우자고 했을 것이다.
