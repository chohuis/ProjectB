...
### ① `draft_picked` — A 가 넣었고 화면이 그대로 받았다

`{ year, eventType: "draft_picked", toTeamId, toLeagueId, detail: "N라운드 M순위" }`.
주요 사건 절이 손댈 것 없이 그린다 — 라벨표에도 이미 있었다.

### ② 🔴 엔딩 뒤가 없었다 — 커리어 고리를 닫았다

결산을 닫으면 **은퇴한 주인공인 채로 메인에 남았다.** 게임 안에 타이틀로
돌아가는 길이 하나도 없었다. `App.svelte` 부터 배선은 이어져 있었는데
`SeasonEndModal.onExit` 에서 끊겨 아무도 안 불렀다.

사용자 확정: **둘 다 준다** — `둘러보기`(메인에 남는다) · `마치기`(타이틀로).
자세한 것은 [TRACK_C_SVELTECHECK.md §5](TRACK_C_SVELTECHECK.md).
...
1. **👁 눈으로 봐야 한다.** 이 저장소의 검사는 전부 소스 문자열 대조라
   **컴포넌트를 띄우지 않는다.** 실제로 은퇴까지 가서 봐야 한다.
   ...
   비어 있다 — 띄워도 빈 화면만 보인다
2. 사건이 많은 커리어(15~20시즌)에서 절이 길어지는지 — 필요하면 접기
