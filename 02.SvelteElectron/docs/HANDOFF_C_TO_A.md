...
> `b018a2cf6`·`c7a706a11`(검사 24) · `6b1934c3d`(실물에서 잡은 붙은 줄).
> `extract-modals` 를 병합해 얹었다.
> engine-native 도장 `f122e4326d26`.
> **vitest 242파일 2,602건 중 2,601 통과**(빨간 하나는 병합이 들고 온 매니페스트 · §0.60-3) ·
> `check:svelte` 774파일 **0 오류 0 경고** ·
...
| ⑥ | 위기 표시 | `isCrisis` · 칩 안 | ✅ 코드 · ⚠ 데이터가 7종뿐이라 눈으로는 못 봤다(§0.60-4b·§0.60-6) |
...
### 0.60-4 「종류만 힌트」를 유니크 전수에 걸어 봤다 — 숫자 0건

눈으로 한 화면만 보고 넘어갈 자리가 아니라, **지금 데이터의 유니크 전부**에
힌트 생성기를 걸어 세었다(`resource/data/master/events` + `decision_templates`).

| 잰 것 | 값 |
|---|---:|
| 유니크·히든 이벤트 | 41 (히든 **0** — B 4-4 미착수) |
| 그 선택지 | 94 |
| 🔴 숫자가 샌 힌트 | **0** |
| 종류를 못 세워 빈 힌트 | **0** |
| 두 갈래가 **같은 말**이 되는 이벤트 | **1** |

- 대가(`cost`)가 없는 유니크·히든도 **0** 이다 — `check:tiers` 와 같은 값이다.
- 실제로 나오는 꼴:
  `EVT_HS_COMMON_RETIRE_GIFT` → 대가 「관계를 내준다」 ·
  갈래1 「큰 것을 얻는다 · 사람을 얻는다 · 돈을 내준다」 /
  갈래2 「사람을 얻는다 · 마음이 놓인다 · 돈을 내준다」.

**🔴 B 에게 하나** — `EVT_HS_COMMON_FORM_FIX` 는 두 갈래가 **종류까지 똑같다**
(둘 다 「큰 것을 얻는다 · 몸을 갈아 넣는다」). 숫자를 감추면 **고를 근거가
사라진다** — 노말의 「갈래 둘이 종류가 다른가」 규칙(§7-4)이 유니크에도 필요한
자리다. 효과 키·부호를 한쪽만 바꾸면 닫힌다. 밸런스가 아니라 **선택지 설계**다.

### 0.60-4b 위기 표시는 코드로만 확인했다

`theme: body` 인 레어·유니크는 지금 **7종뿐**이다(레어 3 · 유니크 4). 한 시즌
헤드리스로는 잡히기 어려워 눈으로 못 봤고, 대신 `isCrisis` 를 검사로 못박았다
(노말·히든은 위기가 아니다 · 몸이 아닌 결은 위기가 아니다). B 가 `body` 를 더
달면 그때 화면에서 같이 본다.

### 0.60-5 눈확인이 네 판 만에 돌았다 — `drive.mjs` 에서 배운 것 넷

새 게임 헤드리스로 등급 이벤트를 만나는 데 **네 번 다시 짰다.** 다음 사람이
같은 데를 밟지 않게 적어 둔다(고칠 자리는 네 몫이라 `drive.mjs` 는 안 건드렸다).

**㉮ `.go` 는 pending 이 있어도 disabled 가 아니다.** `TopHeader.handleAdvance`
가 `$hasPendingAction` 이면 **버튼을 잠그는 대신** `onOpenPending()` 으로 그
화면을 연다. 그래서 「`.go` 가 비활성이면 막힌 것」이라는 전제로 짜면 같은
pending 만 계속 다시 열며 `GO` 를 63번 뱉고 **한 주도 안 간다**(실측).
막는 것을 다 치운 **뒤**에 밀어야 한다 — `advance` 는 이미 그 차례로 돼 있다.

**㉯ 보직 확인 버튼이 `.btn.go` 라 주 진행 버튼과 겹친다.**
`document.querySelector('.go')` 는 헤더 것을 먼저 집는다. `RoleChoicePanel` 의
확인은 `.confirm .btn.go` 로 따로 짚어야 하고 헤더 것은 `.go:not(.btn)` 이다.
`advance --auto` 가 **첫 판에서 W1 을 못 넘긴** 이유가 이것이다 —
`STUCK` 도 안 뱉고 매주 12번 헛돌다 조용히 다음 주로 넘어간다.

**㉰ 페이지 안에서 `await sleep()` 으로 도는 루프는 못 쓴다.** Chromium 이
감춰진 창의 타이머를 조인다(intensive throttling) — 그렇게 짠 둘째 판이
**15분을 돌고도 한 줄도 안 뱉었다.** 기다리는 것은 드라이버(Node) 몫이라
`eval` 이 **한 걸음만** 옮기게 짰다(`scripts/c62-tierchip.txt`).

**㉱ `advance --auto` 로는 「숫자를 감췄나」를 못 본다.** `button.opt` 을 보이는
족족 눌러서 40주를 돌고 소식함을 열면 **미결이 하나도 안 남는다** — 고른 뒤
화면(`.dec-done`)은 설계상 숫자를 그대로 보여 주는 자리다(§2 「결과는 고른 뒤
소식으로」). 그래서 c62 는 **미결 유니크·히든에서 멈춘다.**

### 0.60-6 실물 확인 — 새 게임 헤드리스 (`scripts/c62-tierchip.txt`)

새 게임(고교 1학년 북악고) → W12 에서 **미결 유니크**를 만나 멈춤 → 그대로 찍고
→ 시즌 결산까지 밀었다. 스크린샷 셋: `docs/screens/c62-pending-graded.png` ·
`c62-season-tally.png` · `c62-list-chips.png`.

**① 칩** — 소식 목록과 상세 머리 둘 다에 「유니크」가 뜬다(보라). 같은 화면의
노말 소식엔 아무것도 안 붙는다.

**② 숫자 감춤** — 유니크 `EVT_HS_COMMON_MINOR_INJURY`(「훈련 중 발목 접질림」)에서

```
힌트        ["큰 것을 얻는다 · 몸을 갈아 넣는다", "큰 것을 얻는다 · 몸이 편해진다"]
훈련효율꼬리 []
숫자샌힌트   []          ← 🔴 여기가 0 이어야 한다
```

같은 판에서 **고른 뒤**의 유니크(「투구 자세 교정 제안」)는 `.dec-done` 에
「제구 즉시 +1, 피로 +8」이 **숫자 그대로** 뜬다 — §2 「결과는 고른 뒤 소식으로」.

**③ 대가** — 「이 선택에는 대가가 따른다 — 관계를 내준다」가 갈래 위에 한 줄.

**④ 집계** — 2026 시즌 결산 시즌 탭: **레어 6 · 유니크 2 · 히든 0**.
⚠ **레어 6·유니크 2 는 `tier_rules.json` 의 시즌 상한과 같은 값이다** — 고교
1학년 한 시즌에 둘 다 **상한까지 찼다**. 등급이 흔한 건지 상한이 낮은 건지는
D 4-6 이 잴 일이고, 화면은 있는 값을 그대로 보인다.

**⑥ 위기** — 이 판에선 안 떴고, **그게 맞다**(§0.60-4b + 아래 제보).

#### 🔴 B 에게 — `theme` 가 몸 이야기와 어긋난 레어·유니크 셋

칩이 「유니크」만 뜨고 「위기」가 안 붙어서 화면을 의심했는데, **데이터가
그렇게 적혀 있었다.** 본문에 몸 이야기가 있는데 `theme` 가 `body` 가 아닌
레어·유니크는 셋이다:

| 등급 | 지금 theme | id | 본문 단서 |
|---|---|---|---|
| 유니크 | `social` | `EVT_HS_COMMON_MINOR_INJURY` | 「발목을 살짝 **접질**렸다」 |
| 유니크 | `social` | `EVT_HS_Y3_SHOULDER` | 어깨 **통증** |
| 레어 | `team` | `EVT_HS_Y2_ACE_OUT` | 에이스 **부상** |

앞 둘은 **주인공 자기 몸**이라 `body` 가 맞아 보인다. 셋째는 동료 부상이라
`team` 이 맞을 수 있다 — 판단은 B·사용자 몫이고, 화면은 `theme` 만 본다.
지금 `theme: body` 인 레어·유니크는 **7종뿐**(레어 3 · 유니크 4)이라 위기 칩이
거의 안 뜬다.
